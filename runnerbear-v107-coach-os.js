/* RunnerBear v10.19 · locked Design Direction 1.0 with rolling plan and coach logic preserved
...
(function(){
  ...
  const state={selectedDs:sessionStorage.getItem(K.selected)||'',planView:savedPlanView==='done'?'done':'plan',planLens:['week','focus','long'].includes(savedPlanLens)?savedPlanLens:'week',openWeek:null,moveOpen:false,adaptOpen:false,completedId:'',doneScroll:0,goalManagerOpen:false,goalEditor:''};
  ...
  function todayHtml(){
    ...
    return `<div id="rb107Today" class="rb107-surface rb119b rb119b-today"><div class="rb107-shell">${appBarHtml()}${raceMetaHtml()}${viewTitleHtml(done?'Dagens resultat':'Dagens økt',done?'Gjennomført og analysert':'')}
      ${done?`${resultCardHtml(base,'today')}${changeNoticeHtml(base)}`:pending?pendingResultHtml(base):`${workoutHeroHtml(base)}${changeNoticeHtml(base)}${todayCoachHtml(base,d,suggest)}${readinessCardHtml()}${suggest?`<button class="rb119b-suggestion" data-rb107-apply-suggestion>Bruk coachens anbefaling ${icon('arrowRight')}</button>`:''}${todayActionsHtml(base,d,suggest)}`}
  ...
  function planListHtml(week,lens){
    if(lens==='long'){
      const weeks=[...new Set(effectiveSchedule().map(p=>p.week))],at=Math.max(0,weeks.indexOf(week));return `<div class="rb119b-long-list">${weeks.slice(at,at+4).map((n,index)=>{const rows=weekRows(n),s=weekStats(n);return `<button data-rb107-day="${rows[0]?.ds||''}"><span><small>${index===0?'Denne uken':index===1?'Neste uke':'Foreløpig'}</small><b>Uke ${n}</b></span><span>${s.km} km · ${s.quality} kvalitet</span>${icon('chevronRight')}</button>`}).join('')}</div>`;
    }
    const rows=weekRows(week),visible=lens==='focus'?rows.filter(p=>p.type==='quality'||p.type==='race'||/langtur/i.test(p.title||'')):rows;return `<div class="rb119b-plan-list">${(visible.length?visible:rows).map(planRowHtml).join('')}</div>`;
  }
  ...
  function planHtml(){
    ...
    return `<div id="rb107Plan" class="rb107-surface rb119b rb119b-plan"><div class="rb107-shell">${appBarHtml()}${viewTitleHtml(state.planView==='done'?'Gjennomførte økter':'Din plan',state.planView==='done'?'Resultater og coachanalyser':subtitle)}
      ${state.planView==='done'?`<button class="rb119b-back" data-rb107-plan-view="plan">${icon('arrowLeft')} Tilbake til plan</button>${completedHtml()}`:`<div class="rb119b-week-controls"><div><button data-rb113-week-step="-1" aria-label="Forrige uke">${icon('chevronLeft')}</button><b>Uke ${week} · ${esc(range)}</b><button data-rb113-week-step="1" aria-label="Neste uke">${icon('chevronRight')}</button></div><button data-rb119b-plan-lens="long">${icon('calendar')}<span>Oversikt</span></button></div><div class="rb119b-segments" role="tablist" aria-label="Planvisning"><button class="${state.planLens==='week'?'active':''}" data-rb119b-plan-lens="week">Uke</button><button class="${state.planLens==='focus'?'active':''}" data-rb119b-plan-lens="focus">Fokus</button><button class="${state.planLens==='long'?'active':''}" data-rb119b-plan-lens="long">Langsiktig</button></div>${planListHtml(week,state.planLens)}${planOverviewHtml(week)}${selected?dayDetailHtml(selected):''}<button class="rb119b-completed-link" data-rb107-plan-view="done">Gjennomførte økter og coachanalyser ${icon('chevronRight')}</button>`}
    </div></div>`;
  }
  ...
  function goalHeroFidelityHtml(goal,g,pred){
    ...
    return `<section class="rb119b-goal-hero"><span class="rb119b-hero-icon">${icon('flag')}</span><div><small>${esc(title)}</small><h2>${esc(distance)}</h2></div><footer>${icon('calendar')}<b>${esc(date)}</b><i></i><span>${goal?`${goalDays(goal)} dager igjen`:`~${roundHalf(policy().anchorKm||50)} km normaluke`}</span></footer></section>`;
  }
  ...
  function ensureCoachStyles(){
    ['runnerbear-v1013-coach-ui.css','runnerbear-v1014-premium-polish.css','runnerbear-v1016-goals-more.css','runnerbear-v1017-ux.css','runnerbear-v1018-plan-preferences.css','runnerbear-v1019-design-system.css','runnerbear-v1019b-screen-fidelity.css'].forEach(name=>{const link=qs(`link[href*="${name}"]`);if(link)document.head.appendChild(link)});
  }
  ...
  function bind(){
    ...
    qsa('[data-rb119b-plan-lens]').forEach(b=>b.onclick=()=>{state.planLens=b.dataset.rb119bPlanLens;sessionStorage.setItem(K.planLens,state.planLens);renderAll()});
  ...
  window.RunnerBearCoachOS={version:'10.19b',effectiveSchedule,planFor,forecast,thresholdEvidence,matches:allMatches,analysisFor,sessionAssessment,workoutStructure,garminQueue,tredictPlanQueue,goalState,trainingPreferences,render:renderAll,moveWorkout,toggleLock,setChoice,adapt};
