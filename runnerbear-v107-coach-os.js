/* RunnerBear v10.20 · coaching and interaction clarity within locked Design Direction 1.0
...
(function(){
  ...
  const clarityModel=()=>window.RunnerBearV1020||null;
  ...
  function trackEvent(name,detail={}){
    const payload={event:name,runnerbearVersion:'10.20',...detail};
  ...
  function coachWatchHtml(){
    const h=healthSignal(),r=h.r||{},t=trend30(),rec=recoveryScore(r),weekCheck=localStorage.getItem(`runfest26_weekcheck_${weekForToday()}`)||'',bodyLabel=weekCheck==='fresh'?'Frisk':weekCheck==='tired'?'Litt sliten':weekCheck==='heavy'?'Tung':'Ikke meldt',tone=h.tone==='red'?'obs':h.tone==='yellow'?'watch':'good';
    const rows=[
  ...
  function coachBasis(){
    const raw=decision(),h=healthSignal(),r=h.r||{},trend=trend30(),positives=[],watch=[],sources=new Set(),hasRecoverySignals=!!(r.hrv?.value||r.sleep?.value||r.rhr?.value);
    ...
    const severeSignal=/akilles|hælfeste|flere avvik/i.test(`${raw.headline||''} ${raw.message||''}`);
    const status=clarityModel()?.coachDecision?.({rawLevel:raw.level,healthTone:h.tone,hasRecoverySignals,severeSignal,message:raw.message})||{key:raw.level==='red'?'adjust_day':raw.level==='yellow'?'plan_margin':'plan_stands',headline:raw.headline,message:raw.message,readiness:{score:raw.level==='red'?4:raw.level==='yellow'?6:8,label:raw.level==='red'?'Juster':raw.level==='yellow'?'Med margin':'Klar',copy:raw.message}};
  ...
  function todayCoachHtml(base,d,suggest){
    const coach=coachBasis(),text=suggest?'RunnerBear anbefaler en roligere dag og venter på godkjenning før planen endres.':coach.message;
    return `<article class="rb119b-card rb119b-coach-card"><header><h2>Coachens vurdering</h2><span class="rb119b-quote">“</span></header><div><span class="rb119b-coach-avatar" aria-hidden="true">RB</span><p><b>${esc(coach.headline)}</b>${esc(text)}</p></div><button type="button" data-rb107-open-why>Hvorfor? ${icon('chevronRight')}</button></article>`;
  }
  ...
  function todayActionsHtml(base,d,suggest){
    return `<div class="rb119b-primary-actions rb1020-secondary-actions"><button class="rb119b-secondary-action" data-rb107-toggle-adapt>${icon('spark')} Tilpass dagen</button></div>
      ${state.adaptOpen?`<div class="rb119b-adapt-panel"><span>Hva har endret seg?</span><div><button data-rb107-adapt="tired" data-base-ds="${base.baseDs}">${icon('moon')}Litt sliten</button><button data-rb107-adapt="time" data-base-ds="${base.baseDs}">${icon('clock')}Dårlig tid</button><button data-rb107-adapt="achilles" data-base-ds="${base.baseDs}">${icon('shield')}Akilles / hælfeste</button></div></div>`:''}
      `;
  }
  ...
  function garminTrainingAvailable(){return window.RunnerBearGarminTraining?.available===true&&typeof window.RunnerBearGarminTraining?.send==='function'}
  function workoutDetailModalHtml(p){
    if(!p)return'';const x=prescription(p),a=activityFor(p),done=!!a,status=done?'Gjennomført':x.type==='rest'?'Hviledag':'Planlagt',metrics=a?resultMetrics(a,p):[];
    return `<div class="rb1020-modal" data-rb1020-modal-backdrop="workout"><section class="rb1020-sheet rb1020-workout-sheet" role="dialog" aria-modal="true" aria-labelledby="rb1020WorkoutTitle"><header><div><span>${esc(status)} · ${esc(formatDate(p.ds,{weekday:'long',day:'numeric',month:'long'}))}</span><h2 id="rb1020WorkoutTitle">${esc(x.title)}</h2><p>${esc(typeLabel(x.type))}</p></div><button type="button" data-rb1020-modal-close="workout" aria-label="Lukk øktdetaljer">${icon('close')}</button></header><div class="rb1020-sheet-body">${done?`<section class="rb1020-detail-section"><span>Gjennomført</span><div class="rb1020-result-grid">${metrics.map(row=>`<div><small>${esc(row[0])}</small><b>${esc(row[1])}</b></div>`).join('')}</div></section>`:''}<section class="rb1020-detail-section"><span>Øktstruktur</span>${simpleStructureHtml(p)}</section>${workoutIntensityHtml(p)}<section class="rb1020-coach-key"><span>Coachens nøkkelråd</span><p>${esc(keyAdvice(p))}</p></section>${x.shoe||x.fuel?`<section class="rb1020-detail-section rb1020-practical"><span>Praktisk</span>${x.shoe?`<p><b>Sko</b>${esc(x.shoe)}</p>`:''}${x.fuel?`<p><b>Energi</b>${esc(x.fuel)}</p>`:''}</section>`:''}${flexHtml(p)}${!canOneOff(p)&&!flexible(p)?matchPickerHtml(p):''}${garminTrainingAvailable()&&!done?`<button class="rb107-button rb1020-garmin-send" data-rb1020-send-garmin="${esc(p.ds)}">Send til Garmin</button>`:''}</div></section></div>`;
  }
  function coachReasonModalHtml(){
    const coach=coachBasis(),positives=coach.positives.length?coach.positives:['Belastningsbildet gir ikke grunnlag for en offensiv endring.'];
    return `<div class="rb1020-modal" data-rb1020-modal-backdrop="coach"><section class="rb1020-sheet rb1020-reason-sheet" role="dialog" aria-modal="true" aria-labelledby="rb1020ReasonTitle"><header><div><span>Hvorfor står planen?</span><h2 id="rb1020ReasonTitle">${esc(coach.headline)}</h2></div><button type="button" data-rb1020-modal-close="coach" aria-label="Lukk coachvurderingen">${icon('close')}</button></header><div class="rb1020-sheet-body"><p class="rb1020-reason-lead">${esc(coach.message)}</p><section><h3>Det som taler for økten</h3><ul>${positives.map(x=>`<li>${esc(x)}</li>`).join('')}</ul></section><section><h3>Det vi følger med på</h3><ul>${coach.watch.map(x=>`<li>${esc(x)}</li>`).join('')}</ul></section><footer><span>Datagrunnlag</span><b>${esc(coach.sources.length?coach.sources.join(' · '):'plan og tilgjengelig treningshistorikk')}</b></footer></div></section></div>`;
  }
  function todayHtml(){
    ...
    return `<div id="rb107Today" class="rb107-surface rb119b rb119b-today"><div class="rb107-shell">${appBarHtml()}${raceMetaHtml()}${viewTitleHtml(done?'Dagens resultat':'Dagens økt',done?'Gjennomført og analysert':'')}
      ${workoutHeroHtml(base)}${changeNoticeHtml(base)}${done?resultCardHtml(base,'today'):`${pending?pendingResultHtml(base):''}${todayCoachHtml(base,d,suggest)}${readinessCardHtml()}${coachWatchHtml()}${suggest?`<button class="rb119b-suggestion" data-rb107-apply-suggestion>Bruk coachens anbefaling ${icon('arrowRight')}</button>`:''}${todayActionsHtml(base,d,suggest)}`}
  ...
  function compactResultLabel(p){const a=activityFor(p);if(!a)return'';return`Fullført · ${verdictFor(p,a).badge}`}
  ...
  function planRowHtml(p){
    const x=prescription(p),done=isDone(p),changed=planChange(p),isToday=p.ds===today(),tone=isToday&&!done?'key':done?'done':changed?'changed':'',status=done?'Fullført':changed?'Endret':isToday?'I dag':x.type==='quality'||x.type==='race'?'Nøkkel':'Planlagt';
    return `<button class="rb119b-plan-row ${tone}" data-rb107-day="${p.ds}"><time><span>${esc(formatDate(p.ds,{weekday:'short'}).replace('.',''))}</span><b>${dateFrom(p.ds).getDate()}</b></time><span class="rb119b-plan-type">${icon(workoutIconName(p))}</span><span class="rb119b-plan-copy"><b>${esc(x.title)}</b><small>${esc(x.desc||targetFor(p).main||typeLabel(x.type))}</small></span><span class="rb119b-plan-status"><small>${esc(status)}</small>${done?icon('check'):`<i aria-hidden="true"></i>`}</span></button>`;
  }
  ...
  function monthCalendarHtml(){
    ...
    const days=Array.from({length:42},(_,i)=>{const d=new Date(start);d.setDate(d.getDate()+i);const ds=localIso(d),p=plans.get(ds),outside=d.getMonth()!==month,done=p&&isDone(p),kind=p?(p.type==='quality'||p.type==='race'?'high':p.type==='rest'?'rest':'planned'):'',stateLabel=done?'gjennomført':p?.type==='quality'||p?.type==='race'?'kvalitetsøkt':p?.type==='rest'?'hviledag':'planlagt';return`<button class="${outside?'outside ':''}${ds===today()?'today ':''}${ds===state.selectedDs?'selected ':''}${kind} ${done?'done':''}" data-rb119c-calendar-day="${ds}" ${p?`aria-label="${esc(formatDate(ds))} · ${stateLabel}"`:`disabled aria-label="${esc(formatDate(ds))} · ingen økt i planen"`}><span>${d.getDate()}</span>${p?`<i aria-hidden="true"></i>`:''}</button>`}).join('');
  ...
  function dayActivity(p){
    const matched=activityFor(p);if(matched)return matched;
  ...
  function planDayState(p,a){
    return clarityModel()?.dayState?.({date:p?.ds,today:today(),planType:prescription(p)?.type,hasActivity:!!a,deviates:activityDeviates(p,a)})||{code:a?'completed':p?.ds<today()?'missed':'planned',label:a?'Gjennomført':p?.ds<today()?'Ikke gjennomført':'Planlagt'};
  }
  function dayViewHtml(p){
    ...
    const actual=a?`<div class="rb1020-day-actual"><span>Gjennomført</span><b>${esc(a.title||sportLabel(a,p))}</b><small>${esc(metrics.map(row=>row[1]).slice(0,3).join(' · '))}</small></div>`:'';
    let body='';
    if(day.code==='completed_deviation')body=`<div class="rb1020-day-comparison">${planned}${actual}</div><p class="rb1020-day-coach">Dagens aktivitet avvek fra planen. Dette tas med i vurderingen av resten av uka, uten treningsgjeld.</p><button class="rb1020-day-link" data-rb109-open-completed="${esc(a.id)}">Se gjennomføringen ${icon('chevronRight')}</button>`;
    ...
    else if(day.code==='missed')body=`${planned}<p class="rb1020-day-coach">Ingen aktivitet ble matchet med denne dagen. Planen går videre uten moraliserende treningsgjeld.</p><button class="rb1020-day-link" data-rb1020-day-workout="${esc(p.ds)}">Se den planlagte økten ${icon('chevronRight')}</button>`;
    ...
    else body=`${planned}<div class="rb1020-day-metrics"><div><span>Type</span><b>${esc(typeLabel(x.type))}</b></div><div><span>Mengde</span><b>${esc(plannedAmount)}</b></div></div><button class="rb1020-day-link" data-rb1020-day-workout="${esc(p.ds)}">Åpne øktdetaljer ${icon('chevronRight')}</button>`;
  ...
  function intensityCardHtml(){
    ...
    return `<article class="rb119b-card rb1020-intensity"><header><div><h2>Intensitetsstyring</h2><p>Slik styrer RunnerBear treningen din nå.</p></div><span>${profile.valid?'Personlig':'Kontroller data'}</span></header><div class="rb1020-intensity-list">${rows.map(x=>`<div class="${x.key}"><div><b>${esc(x.label)}</b><small>${esc(x.description)}</small></div><span><i style="width:${x.width}%"></i></span><strong>${esc(rangeLabel(x))}</strong></div>`).join('')}</div><footer><p>Gråsonen er synlig for å begrense unødvendig mellomintensitet – ikke fordi den alltid er feil.</p><button type="button" data-rb1020-intensity-explain>Hvordan beregnes dette? ${icon('chevronRight')}</button></footer></article>`;
  }
  function intensityExplanationModalHtml(){
    const p=intensityProfile();return `<div class="rb1020-modal" data-rb1020-modal-backdrop="intensity"><section class="rb1020-sheet rb1020-intensity-sheet" role="dialog" aria-modal="true" aria-labelledby="rb1020IntensityTitle"><header><div><span>Personlig treningsmodell</span><h2 id="rb1020IntensityTitle">Hvordan beregnes dette?</h2></div><button type="button" data-rb1020-modal-close="intensity" aria-label="Lukk forklaringen">${icon('close')}</button></header><div class="rb1020-sheet-body"><p class="rb1020-reason-lead">RunnerBear tilpasser intensitetsområdene etter terskel, nylige representative økter og pulsrespons. Områdene kan derfor endre seg når formen utvikler seg.</p><section><h3>Grunnlaget akkurat nå</h3><ul><li>Individuell terskel: ${esc(p.thresholdHr)} bpm${p.thresholdDate?` · ${esc(formatDate(p.thresholdDate,{day:'numeric',month:'short'}))}`:''}</li><li>Makspuls i treningsprofilen: ${esc(p.maxHr)} bpm</li><li>Terskelkilde: ${esc(p.thresholdSource)}</li></ul></section><p class="rb1020-method-note">Frontend mottar ferdige, sammenhengende områder fra coachmodellen. Ingen verdi kan tilhøre to viste områder.</p></div></section></div>`;
  }
  ...
  function decorateBrand(){
    ...
    brand.innerHTML=`<div class="rb"><img src="runnerbear-brand-mark-flat.svg?v=1019" alt=""></div><div><b>RunnerBear</b><span>Bakken-coach</span></div>`;
  }
  function ensureCoachStyles(){
    ['runnerbear-v1013-coach-ui.css','runnerbear-v1014-premium-polish.css','runnerbear-v1016-goals-more.css','runnerbear-v1017-ux.css','runnerbear-v1018-plan-preferences.css','runnerbear-v1019-design-system.css','runnerbear-v1019b-screen-fidelity.css','runnerbear-v1019c-final-fidelity.css','runnerbear-v1020-coaching-clarity.css'].forEach(name=>{const link=qs(`link[href*="${name}"]`);if(link)document.head.appendChild(link)});
  }
  ...
  function decorateNav(){
    ...
    qsa('.navbtn[data-tab]').forEach(b=>{const x=map[b.dataset.tab];if(!x)return;b.innerHTML=`<span>${icon(x[1])}</span>${x[0]}`;b.onclick=()=>{const target=b.dataset.tab==='goals'&&$('goals')?'goals':b.dataset.tab;qsa('.view').forEach(v=>v.classList.toggle('active',v.id===target));qsa('.navbtn').forEach(n=>n.classList.toggle('active',n.dataset.tab===b.dataset.tab));window.scrollTo({top:0,behavior:'smooth'})}});
  }
  function renderAll(){
    if(!engine()||!clarityModel())return;logAutomaticAdjustments();mount('today',todayHtml());mount('plan',planHtml());mount($('goals')?'goals':'race',goalsHtml());mount('more',moreHtml());decorateNav();decorateBrand();decorateHeader();ensureCoachStyles();document.documentElement.classList.add('rb107-ready');document.documentElement.classList.remove('rb108-booting');document.body.classList.toggle('rb109-modal-open',state.goalManagerOpen||state.workoutDetailOpen||state.coachReasonOpen||state.intensityExplanationOpen);bind();
  }
  ...
  function bind(){
    ...
    qsa('[data-rb1020-send-garmin]').forEach(b=>b.onclick=async()=>{const p=planFor(b.dataset.rb1020SendGarmin);if(!p||!garminTrainingAvailable())return;b.disabled=true;b.textContent='Sender…';try{await window.RunnerBearGarminTraining.send(garminWorkout(p));toast('Økten er sendt til Garmin');state.workoutDetailOpen=false;renderAll()}catch(error){toast(error?.message||'Kunne ikke sende økten til Garmin');b.disabled=false}});
  ...
  function openPlanOnToday(){if(state.planView==='plan'){state.selectedDs=planFor(today())?.ds||state.selectedDs;state.monthKey=monthKeyFor(state.selectedDs||today());state.planDetailOpen=false;state.planDayViewOpen=false;sessionStorage.setItem(K.selected,state.selectedDs)}state.moveOpen=false;setTimeout(renderAll,0)}
  function init(){
    if(!engine()||!clarityModel())return setTimeout(init,50);migrateDocumentedThreshold();migrateTrainingPreferences();runAutopilot();renderAll();
  ...
  window.RunnerBearCoachOS={version:'10.20',effectiveSchedule,planFor,forecast,thresholdEvidence,matches:allMatches,analysisFor,sessionAssessment,workoutStructure,garminQueue,tredictPlanQueue,goalState,trainingPreferences,intensityProfile,coachBasis,heroBank:HERO_BANK,render:renderAll,moveWorkout,toggleLock,setChoice,adapt};
