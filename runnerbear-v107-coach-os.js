/* RunnerBear v10.19c · locked Design Direction 1.0 with rolling plan and coach logic preserved
...
(function(){
  ...
  const state={selectedDs:sessionStorage.getItem(K.selected)||'',planView:savedPlanView==='done'?'done':'plan',planLens:['week','focus','long'].includes(savedPlanLens)?savedPlanLens:'week',openWeek:null,monthKey:'',planDetailOpen:false,moveOpen:false,adaptOpen:false,completedId:'',doneScroll:0,goalManagerOpen:false,goalEditor:''};
  ...
  function coachWatchHtml(){
    ...
    return `<article class="rb119b-card rb119c-coach-watch"><header><div><h2>Coachen følger med</h2><p>${esc(h.copy)}</p></div><button type="button" data-rb107-open-why>Se grunnlaget ${icon('chevronRight')}</button></header><div class="rb119c-health-grid">${rows.map(x=>`<div><span>${icon(x.icon)}</span><small>${esc(x.label)}</small><b>${esc(x.value)}</b><em class="${tone}"><i></i>${esc(x.status)}</em></div>`).join('')}</div><footer><span>Kroppssjekk</span><b>${esc(bodyLabel)}</b><small>${esc(t.title)} · ${esc(t.threshold)}</small></footer></article>`;
  }
  ...
  function workoutHeroHtml(base){
    const p=prescription(base),target=targetFor(base),structure=workoutStructure(base),focus=p.type==='quality'?'Terskel':p.type==='race'?'Løp':/langtur/i.test(p.title||'')?'Utholdenhet':p.type==='easy'?'Rolig':typeLabel(p.type),main=structure?.model?.mainLabel||target.main||p.desc||p.title,recovery=structure?.recovery||p.detail||'',total=structure?.estimate||target.total||'Fleksibel total',distance=Number(p.km||0)?`${String(roundHalf(p.km)).replace('.',',')} km`:'Fleksibel';
    return `<article class="rb119b-workout-hero" ${heroStyle(heroNameForWorkout(base))}><span class="rb119b-hero-icon">${icon(workoutIconName(base))}</span><div class="rb119b-workout-copy"><h2>${esc(p.title)}</h2><p>${esc(main)}</p>${recovery?`<small>${esc(recovery)}</small>`:''}</div><div class="rb119b-hero-metrics"><div>${icon('clock')}<span><small>Total tid</small><b>${esc(total)}</b></span></div><div>${icon('pin')}<span><small>Distanse</small><b>${esc(distance)}</b></span></div><div>${icon('goal')}<span><small>Fokus</small><b>${esc(focus)}</b></span></div></div></article>`;
  }
  function todayCoachHtml(base,d,suggest){
    const text=suggest?'RunnerBear anbefaler en roligere dag og venter på godkjenning før planen endres.':d.message;
    return `<article class="rb119b-card rb119b-coach-card"><header><h2>Coachens vurdering</h2><span class="rb119b-quote">“</span></header><div><span class="rb119b-coach-avatar" aria-hidden="true">RB</span><p><b>${esc(d.headline)}</b>${esc(text)}</p></div><button type="button" data-rb107-open-why>Se grunnlaget ${icon('chevronRight')}</button></article>`;
  }
  function readinessCardHtml(){
    const d=decision(),health=healthSignal(),levels={green:8,yellow:6,red:4,neutral:7},score=Math.min(levels[d.level]||7,levels[health.tone]||7),label=score>=8?'Klar':score>=6?'Med margin':'Juster',copy=score>=8?'Du er klar for dagens plan.':score>=6?'Hold kontroll og behold litt ekstra margin.':'Kroppen og planen tilsier en roligere dag.';
  ...
  function todayActionsHtml(base,d,suggest){
    const p=prescription(base);return `<div class="rb119b-primary-actions"><button class="rb119b-primary-button" data-rb107-toggle-details><span>Start økt</span>${icon('arrowRight')}</button><button class="rb119b-secondary-action" data-rb107-toggle-adapt>${icon('spark')} Tilpass dagen</button></div>
      ...
      <details class="rb119b-card rb119b-workout-details" id="rb107TodayDetails"><summary><span>Øktstruktur og coachgrunnlag</span>${icon('chevronRight')}</summary><div>${workoutStructureHtml(base)}<div class="rb107-detail-row"><b>Hensikt</b><span>${esc(purposeFor(p))}</span></div><div class="rb107-detail-row"><b>Gjennomføring</b><span>${esc(p.detail||p.desc||'Følg kontrollert belastning.')}</span></div>${p.shoe?`<div class="rb107-detail-row"><b>Sko</b><span>${esc(p.shoe)}</span></div>`:''}${p.fuel?`<div class="rb107-detail-row"><b>Energi</b><span>${esc(p.fuel)}</span></div>`:''}${flexHtml(base)}${!canOneOff(base)&&!flexible(base)?matchPickerHtml(base):''}</div></details>`;
  }
  function todayHtml(){
    ...
    return `<div id="rb107Today" class="rb107-surface rb119b rb119b-today"><div class="rb107-shell">${appBarHtml()}${raceMetaHtml()}${viewTitleHtml(done?'Dagens resultat':'Dagens økt',done?'Gjennomført og analysert':'')}
      ${done?`${resultCardHtml(base,'today')}${changeNoticeHtml(base)}`:pending?pendingResultHtml(base):`${workoutHeroHtml(base)}${changeNoticeHtml(base)}${todayCoachHtml(base,d,suggest)}${coachWatchHtml()}${readinessCardHtml()}${suggest?`<button class="rb119b-suggestion" data-rb107-apply-suggestion>Bruk coachens anbefaling ${icon('arrowRight')}</button>`:''}${todayActionsHtml(base,d,suggest)}`}
  ...
  function planHtml(){
    ...
    return `<div id="rb107Plan" class="rb107-surface rb119b rb119b-plan"><div class="rb107-shell">${appBarHtml()}${viewTitleHtml(state.planView==='done'?'Gjennomførte økter':'Din plan',state.planView==='done'?'Resultater og coachanalyser':subtitle)}
      ${state.planView==='done'?`<button class="rb119b-back" data-rb107-plan-view="plan">${icon('arrowLeft')} Tilbake til plan</button>${completedHtml()}`:`<div class="rb119b-week-controls"><div><button data-rb113-week-step="-1" aria-label="Forrige uke">${icon('chevronLeft')}</button><b>Uke ${week} · ${esc(range)}</b><button data-rb113-week-step="1" aria-label="Neste uke">${icon('chevronRight')}</button></div><button data-rb119c-month-focus>${icon('calendar')}<span>Månedsoversikt</span></button></div>${monthCalendarHtml()}<div class="rb119b-segments" role="tablist" aria-label="Planvisning"><button class="${state.planLens==='week'?'active':''}" data-rb119b-plan-lens="week">Uke</button><button class="${state.planLens==='focus'?'active':''}" data-rb119b-plan-lens="focus">Fokus</button><button class="${state.planLens==='long'?'active':''}" data-rb119b-plan-lens="long">Langsiktig</button></div>${planListHtml(week,state.planLens)}${state.planDetailOpen&&selected?`<div class="rb119c-plan-detail"><button data-rb119c-detail-close>${icon('close')} Lukk øktdetalj</button>${dayDetailHtml(selected)}</div>`:''}${planOverviewHtml(week)}<button class="rb119b-completed-link" data-rb107-plan-view="done">Gjennomførte økter og coachanalyser ${icon('chevronRight')}</button>`}
    </div></div>`;
  }
  ...
  function zonesCardHtml(){
    const zones=[['Sone 1','125–140 bpm',45],['Sone 2','130–148 bpm',32],['Sone 3','160–166 bpm',18],['Sone 4','165–172 bpm',11],['Sone 5','173+ bpm',5]];return `<article class="rb119b-card rb119b-zones"><h2>Soner</h2>${zones.map(x=>`<div><b>${x[0]}</b><span><i style="width:${x[2]}%"></i></span><small>${x[1]}</small>${icon('chevronRight')}</div>`).join('')}</article>`;
  }
  ...
  function ensureCoachStyles(){
    ['runnerbear-v1013-coach-ui.css','runnerbear-v1014-premium-polish.css','runnerbear-v1016-goals-more.css','runnerbear-v1017-ux.css','runnerbear-v1018-plan-preferences.css','runnerbear-v1019-design-system.css','runnerbear-v1019b-screen-fidelity.css','runnerbear-v1019c-final-fidelity.css'].forEach(name=>{const link=qs(`link[href*="${name}"]`);if(link)document.head.appendChild(link)});
  }
  ...
  function bind(){
    ...
    qsa('[data-rb119b-plan-lens]').forEach(b=>b.onclick=()=>{state.planLens=b.dataset.rb119bPlanLens;state.planDetailOpen=false;sessionStorage.setItem(K.planLens,state.planLens);renderAll()});
    qsa('[data-rb107-day]').forEach(b=>b.onclick=()=>{const ds=b.dataset.rb107Day;if(!planFor(ds))return;state.selectedDs=ds;state.planDetailOpen=true;sessionStorage.setItem(K.selected,ds);state.openWeek=planFor(ds).week;state.monthKey=monthKeyFor(ds);state.planView='plan';sessionStorage.setItem(K.planView,'plan');renderAll()});
    ...
    qsa('[data-rb119c-calendar-day]').forEach(b=>b.onclick=()=>{const ds=b.dataset.rb119cCalendarDay,p=planFor(ds);if(!p)return;state.selectedDs=ds;state.openWeek=p.week;state.planLens='week';state.planDetailOpen=false;sessionStorage.setItem(K.selected,ds);sessionStorage.setItem(K.planLens,'week');renderAll();requestAnimationFrame(()=>qs(`[data-rb107-day="${ds}"]`)?.scrollIntoView({block:'center',behavior:'smooth'}))});
    qsa('[data-rb119c-week-start]').forEach(b=>b.onclick=()=>{const ds=b.dataset.rb119cWeekStart,p=planFor(ds);if(!p)return;state.selectedDs=ds;state.openWeek=p.week;state.monthKey=monthKeyFor(ds);state.planLens='week';state.planDetailOpen=false;sessionStorage.setItem(K.selected,ds);sessionStorage.setItem(K.planLens,'week');renderAll()});
  ...
  function openPlanOnToday(){if(state.planView==='plan'){state.selectedDs=planFor(today())?.ds||state.selectedDs;state.monthKey=monthKeyFor(state.selectedDs||today());state.planDetailOpen=false;sessionStorage.setItem(K.selected,state.selectedDs)}state.moveOpen=false;setTimeout(renderAll,0)}
  function init(){
  ...
  window.RunnerBearCoachOS={version:'10.19c',effectiveSchedule,planFor,forecast,thresholdEvidence,matches:allMatches,analysisFor,sessionAssessment,workoutStructure,garminQueue,tredictPlanQueue,goalState,trainingPreferences,heroBank:HERO_BANK,render:renderAll,moveWorkout,toggleLock,setChoice,adapt};
