/* RunnerBear v10.13 · coach-first mobile experience and trustworthy session analysis
...
(function(){
  ...
  function allMatches(){
    const acts=activities(),plans=effectiveSchedule(),signature=`${acts.map(a=>`${a.id}:${a.ds}:${a.distance}:${a.duration}:${a.heartrate}:${a.detail?.analysis?.confidence||''}:${a.detail?.analysis?.workBlocks?.length||0}:${a.detail?.analysis?.workHr||0}:${a.detail?.analysis?.workPace||0}:${a.detail?.analysis?.hrDrift||0}`).join('|')}#${plans.map(p=>`${p.baseDs}:${p.ds}:${p.type}:${p.title}:${p.km}:${p.bRace?.id||''}:${choiceFor(p)}`).join('|')}`;
  ...
  function nextWorkoutHtml(base){
    const next=effectiveSchedule().find(p=>p.ds>base.ds&&!isDone(p));if(!next)return'';const x=prescription(next);
    return `<section class="rb107-card rb113-next"><span>Neste</span><div><b>${esc(x.title)}</b><small>${esc(formatDate(next.ds,{weekday:'long'}))} · ${esc(typeLabel(x.type))}</small></div><button data-rb113-open-next="${esc(next.ds)}" aria-label="Åpne ${esc(x.title)}">›</button></section>`;
  }
  function todayHtml(){
    ...
    return `<div id="rb107Today" class="rb107-surface"><div class="rb107-shell">
      ...
      ${done?`${resultCardHtml(base,'today')}${nextWorkoutHtml(base)}`:`${pending?pendingResultHtml(base):`<section class="rb107-card rb107-decision ${d.level}"><div class="rb107-decision-mark">${icon(d.level==='green'?'check':'info')}</div><div><span class="rb107-overline">Coachens beslutning</span><h2>${esc(d.headline)}</h2><p>${esc(suggest?'RunnerBear anbefaler å ta dagens kvalitet ut, men venter på deg.':d.message)}</p></div>${suggest?`<button class="rb107-decision-link" data-rb107-apply-suggestion>Bruk anbefalingen</button>`:`<button class="rb107-decision-link" data-rb107-open-why>Hvorfor?</button>`}</section>`}${pending?'':plannedWorkoutHtml(base,d,suggest)}`}
      ${todaySignalsHtml()}
      ${done||d.level==='green'?'':`<section class="rb107-card rb107-silent alert"><div><span class="rb107-overline">Coachsignal</span><b>Ett signal fortjener oppmerksomhet</b><p>${esc(d.message)}</p></div><span>${icon('info')}</span></section>`}
    </div></div>`;
  }

  function weekStripHtml(week){
    const rows=weekRows(week),stats=weekStats(week),selected=selectedPlan();
    return `<section class="rb107-card rb107-week-strip"><div class="rb107-week-strip-head"><button data-rb113-week-step="-1" aria-label="Forrige uke">‹</button><div><span class="rb107-overline">Uke ${week}</span><b>${esc(rows[0]&&rows.at(-1)?`${formatDate(rows[0].ds,{day:'numeric',month:'short'})} – ${formatDate(rows.at(-1).ds,{day:'numeric',month:'short'})}`:'')}</b></div><button data-rb113-week-step="1" aria-label="Neste uke">›</button></div><div class="rb107-days">${rows.map(p=>`<button class="rb107-day-chip ${p.ds===today()?'today':''} ${p.ds===selected?.ds?'active':''} ${isDone(p)?'done':''}" data-rb107-day="${p.ds}"><span>${formatDate(p.ds,{weekday:'short'}).replace('.','')}</span><b>${dateFrom(p.ds).getDate()}</b><i class="${p.type}"></i></button>`).join('')}</div></section>`;
  }
  ...
  function fourWeeksHtml(current){
    ...
    return `<div class="rb113-four-weeks">${visible.map(n=>{const rows=weekRows(n),s=weekStats(n);return`<section class="rb107-card"><header><div><span>${esc((window.RUNFEST_WEEKS||[]).find(w=>w.n===n)?.phase||'Treningsuke')}</span><b>Uke ${n}</b></div><strong>${s.km} km</strong></header><div>${rows.map(p=>`<button data-rb107-day="${p.ds}"><time>${esc(formatDate(p.ds,{weekday:'short',day:'numeric'}).replace('.',''))}</time><span>${esc(prescription(p).title)}</span><i class="${p.type}"></i></button>`).join('')}</div></section>`}).join('')}</div>`;
  }
  function planHtml(){
    ...
    return `<div id="rb107Plan" class="rb107-surface"><div class="rb107-shell"><header class="rb107-section-head"><div><span class="rb107-overline">Planen åpner på i dag</span><h1>Plan</h1><p>Coachen viser neste beslutning først.</p></div><div class="rb107-segment"><button class="${state.planView==='plan'?'active':''}" data-rb107-plan-view="plan">Uke</button><button class="${state.planView==='overview'?'active':''}" data-rb107-plan-view="overview">4 uker</button></div></header>
      ${state.planView==='done'?`<button class="rb113-back" data-rb107-plan-view="plan">← Tilbake til plan</button>${completedHtml()}`:state.planView==='overview'?`${fourWeeksHtml(selected?.week||current)}<button class="rb113-completed-link" data-rb107-plan-view="done">Gjennomførte økter og coachanalyser</button>`:`${weekStripHtml(selected?.week||current)}<div class="rb107-week-summary"><div><span>Totalt</span><b>${stats.km} km</b></div><div><span>Kvalitet</span><b>${stats.quality}</b></div><div><span>Løpedager</span><b>${stats.runDays}</b></div></div>${dayDetailHtml(selected)}<button class="rb113-completed-link" data-rb107-plan-view="done">Gjennomførte økter og coachanalyser</button>`}
    </div></div>`;
  }
  ...
  function goalsHtml(){
    ...
    const hero=goal?`<section class='rb107-card rb107-goal-hero rb109-goal-hero'><div><span class='rb107-overline'>${esc(distanceMeta(goal.distance).label)} · ${esc(formatDate(goal.date,{day:'numeric',month:'long',year:'numeric'}))}</span><h2>${esc(goal.name)}</h2><p>Løpsfarten velges av terskelrespons, kontinuitet og de spesifikke øktene.</p></div><div class='rb107-goal-now'><span>Løpskapasitet nå</span><b>${fmtTime(f.current)}</b><small>Sannsynlig område ${fmtTime(f.low)}–${fmtTime(f.high)}</small></div><div class='rb107-race-range'><div><span>Måltid</span><b>${goal.targetSeconds?fmtTime(goal.targetSeconds):'Utvikling'}</b></div><div><span>Til løpet</span><b>${goalDays(goal)} dager</b></div></div></section>`:`<section class='rb107-card rb107-goal-hero rb109-goal-hero base'><div><span class='rb107-overline'>${g.mode==='transition'?'Overgangsperiode':'Bakken-prinsippene · uten løpsdato'}</span><h2>${g.mode==='transition'?'Bygg kroppen opp igjen':'Bygg form uten nedtelling'}</h2><p>${g.mode==='transition'?'Volum og kvalitet holdes lavere en kort periode før normal rytme fortsetter.':'RunnerBear utvikler terskel, kapasitet og løpsøkonomi videre uten et kunstig konkurransepress.'}</p></div><div class='rb107-goal-now'><span>Halvmaratonkapasitet</span><b>${pred?.half?.seconds?fmtTime(pred.half.seconds):'–'}</b><small>${esc(formDirection())} formutvikling</small></div><div class='rb107-race-range'><div><span>Normaluke</span><b>~${roundHalf(policy().anchorKm||50)} km</b></div><div><span>Løpedager</span><b>min. ${policy().profile.minRunDays||5}</b></div></div></section>`;
    ...
    return `<div id='rb107Goals' class='rb107-surface'><div class='rb107-shell'><header class='rb107-section-head rb109-goals-head'><div><span class='rb107-overline'>Fremgang, ikke pynt</span><h1>Mål</h1><p>Et stabilt kapasitetsbilde – dagsform hører hjemme under I dag.</p></div><button class='rb107-button secondary' data-rb109-goal-open>Administrer mål</button></header>${hero}
      <section class='rb107-card rb107-predictions rb109-predictions'><div class='rb107-prediction-head'><b>Løpsprognoser</b><span class='rb107-overline'>Kapasitet · ikke dagsform</span></div>${predictionRows.map(x=>`<div class='rb107-prediction-row rb109-prediction-row ${goal?.distance===x.key?'active':''}'><span>${esc(x.label)}</span><b>${x.value?fmtTime(x.value):'–'}</b><div><small>Grunnlag</small><strong class='rb109-foundation ${x.foundation.code}'>${esc(x.foundation.level)}</strong></div></div>`).join('')}<p class='rb107-note'>${esc(f.foundation.copy)} Dette er et utjevnet coachestimat; Concept2 gir aerob støtte, men teller aldri som falske løpskilometer.</p></section>${status}
  ...
  function moreHtml(){
    ...
    return `<div id="rb107More" class="rb107-surface"><div class="rb107-shell"><header class="rb107-section-head"><div><span class="rb107-overline">Innstillinger og sporbarhet</span><h1>Mer</h1><p>Ryddig som en innstillingsside. Åpne bare det du trenger.</p></div></header><section class="rb107-card rb113-coach-contract"><div><span class="rb107-overline">Din Bakken-coach</span><h2>Kontroll før intensitet</h2><p>RunnerBear bruker Garmin-data til å ta neste treningsbeslutning – ikke til å bygge et nytt Garmin-dashboard.</p></div><ul><li><b>Skadevern</b><span>Akilles- og hælfestesignal kan erstatte løping med 45–60 min rolig Zwift.</span></li><li><b>Ingen treningsgjeld</b><span>Tapte kilometer eller kvalitetsøkter tas aldri igjen.</span></li><li><b>Respons trumfer ego</b><span>Puls, pust og opplevd belastning tolkes konservativt når de spriker.</span></li></ul></section><div class="rb108-settings-list">
  ...
  function ensureCoachStyles(){
    const link=qs('link[href*="runnerbear-v1013-coach-ui.css"]');if(link&&link!==document.head.lastElementChild)document.head.appendChild(link);
  }
  ...
  window.RunnerBearCoachOS={version:'10.13',effectiveSchedule,planFor,forecast,thresholdEvidence,matches:allMatches,analysisFor,sessionAssessment,workoutStructure,garminQueue,tredictPlanQueue,goalState,render:renderAll,moveWorkout,toggleLock,setChoice,adapt};
