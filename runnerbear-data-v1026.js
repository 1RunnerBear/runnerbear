/* RunnerBear 10.26.0 lazy Cloud/data runtime · generated by scripts/build-v1026.mjs */
...
/* canonical source: runnerbear-cloud-v1026.js */
/* RunnerBear v10.26 · Coach Loop browser client and compatibility bridge. */
(function(){
  'use strict';
  const BUILD='10.26.0',CACHE='runnerbear_coach_loop_snapshot_v1',SHADOW_COUNT='runnerbear_coach_loop_shadow_success_v1';
  let snapshot=null,lastSignature='',syncTimer=0,starting=false;
  ...
  function constraints(){const prefs=window.RunnerBearCoachOS?.trainingPreferences?.()||{};return{runDays:prefs.runDays||[],qualityDays:prefs.qualityDays||[],longRunDay:prefs.longRunDay,alternativeDays:prefs.alternativeDays||[],maxRunDays:(prefs.runDays||[]).length||5,weeklyKmCap:Number(prefs.maxKm||55)}}
  function legacyInput(){
    const os=window.RunnerBearCoachOS,plan=os?.effectiveSchedule?.()||[],prefs=os?.trainingPreferences?.()||{},goal=os?.goalState?.()||{},explicitModes=read('runnerbear_v118_day_modes',{});
    const primary=goal.primary||{};return{timezone:Intl.DateTimeFormat().resolvedOptions().timeZone||'Europe/Oslo',profile:{baseKm:Number(prefs.baseKm||50)},constraints:constraints(),goal:{mode:goal.mode||'race',name:primary.name||'',date:primary.date||'',distance:primary.distance||'half',targetSeconds:Number(primary.targetSeconds||0)},effectivePlan:plan.map(row=>{const workoutType=row.workoutType||row.type||'easy',structure=window.RunnerBearCoachOS?.workoutStructure?.(row)||{},legacy={baseDs:row.baseDs||row.ds,sourceLabel:row.sourceLabel||row.label||'',desc:row.desc||'',detail:row.detail||'',shoe:row.shoe||'',fuel:row.fuel||''};return{workoutId:row.workoutId||row.baseDs,lineageId:row.lineageId||row.workoutId||row.baseDs,localDate:row.ds,slotIndex:Number(row.slotIndex||0),status:row.status||(row.terminal?'completed':'scheduled'),sport:row.sport||(['easy','quality','race'].includes(workoutType)?'running':workoutType),workoutType,title:row.title||'Planlagt økt',intent:row.intent||row.stimulus||'',prescription:{version:1,...structure,legacy},plannedDurationSeconds:row.plannedDurationSeconds??null,plannedDistanceM:Number(row.plannedDistanceM||Number(row.km||0)*1000),plannedLoad:row.plannedLoad||{},source:row.source||'legacy-v10.25.1',lockLevel:row.lockLevel||(row.locked?'user':'none'),flexible:row.flexible===true,explicitChoice:Object.hasOwn(explicitModes,row.baseDs)}})};
  }
  ...
  async function start(){if(starting)return;starting=true;try{await refresh('home');const input=legacyInput();lastSignature=window.RunnerBearV1026?.stable?.({plan:input.effectivePlan,profile:input.profile,constraints:input.constraints,goal:input.goal})||JSON.stringify(input);window.addEventListener('runnerbear:state-dirty',scheduleSync);await applySafeAuto()}catch(error){console.error(JSON.stringify({event:'coach_loop_bootstrap_error',build:BUILD,code:error.code||'',message:error.message}));const cached=read(CACHE,null);if(cached?.flags?.coach_loop_read===true)try{install(cached)}catch{}}finally{starting=false}}
  window.RunnerBearCloudV1026={build:BUILD,start,refresh,reconfigure,resolveDecision,applySafeAuto,undoPlan,submitFeedback,shadowCompare,snapshot:()=>snapshot,decision:()=>snapshot?.coachDecision||null,toLegacyDecision,proposalFor};
