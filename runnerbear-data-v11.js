/* RunnerBear 12.6.0 lazy Cloud/data runtime · generated by scripts/build-v11.mjs */
...
(function(){
  'use strict';
  const BUILD=window.RunnerBearRelease?.build||'12.4.0',CACHE='runnerbear_coach_loop_snapshot_v1',SHADOW_COUNT='runnerbear_coach_loop_shadow_success_v1';
  let fullRefreshPromise=null,refreshSerial=0,snapshot=null,lastSignature='',syncTimer=0,startPromise=null,fullLoadedRevision='',syncWatchTimers=[];
  const read=(key,fallback={})=>{try{return JSON.parse(localStorage.getItem(key)||'')??fallback}catch{return fallback}};
  const api=async(path,{timeoutMs,...options}={})=>{const controller=new AbortController(),timer=(timeoutMs||(options.method||'GET')==='GET')?setTimeout(()=>controller.abort(),timeoutMs||15000):null;try{const response=await fetch(path,{credentials:'same-origin',cache:'no-store',signal:controller.signal,...options,headers:{Accept:'application/json',...(options.body?{'Content-Type':'application/json'}:{}),...(options.headers||{})}});let body={};try{body=await response.json()}catch(error){if(controller.signal.aborted)throw error}if(!response.ok)throw Object.assign(new Error(body.message||body.error||`RunnerBear HTTP ${response.status}`),{code:body.code,status:response.status});return body}finally{clearTimeout(timer)}};
  ...
  function planIdentity(rows=[],{fromDate='',throughDate=''}={}){return(rows||[]).filter(row=>{const date=row.localDate||row.ds||'';return(!fromDate||date>=fromDate)&&(!throughDate||date<=throughDate)}).map(row=>[String(row.workoutId||row.baseDs||'').replace(/^wo-/,''),row.localDate||row.ds||'',row.workoutType||row.type||'',Number(row.plannedDistanceM||Number(row.km||0)*1000)]).sort((a,b)=>String(a[0]).localeCompare(String(b[0])))}
  function shadowCompare(data){
    const legacy=legacyInput().effectivePlan,canonical=data?.activePlan?.items||[],dates=canonical.map(row=>row.localDate).filter(Boolean).sort(),bounds={fromDate:dates[0]||data?.generatedFromDate||'',throughDate:data?.config?.goal?.date||dates.at(-1)||''},legacyIdentity=planIdentity(legacy,bounds),canonicalIdentity=planIdentity(canonical,bounds),left=JSON.stringify(legacyIdentity),right=JSON.stringify(canonicalIdentity),match=left===right,count=match?Math.max(0,Number(localStorage.getItem(SHADOW_COUNT)||0))+1:0,legacyKeys=new Set(legacyIdentity.map(row=>row.slice(0,2).join(':'))),canonicalKeys=new Set(canonicalIdentity.map(row=>row.slice(0,2).join(':')));
    localStorage.setItem(SHADOW_COUNT,String(count));
    const report={match,planRevisionId:data?.planRevisionId||'',legacyCount:legacyIdentity.length,canonicalCount:canonicalIdentity.length,fromDate:bounds.fromDate,throughDate:bounds.throughDate,legacyOnly:[...legacyKeys].filter(key=>!canonicalKeys.has(key)).slice(0,5),canonicalOnly:[...canonicalKeys].filter(key=>!legacyKeys.has(key)).slice(0,5)};console[match?'info':'warn'](JSON.stringify({event:match?'coach_loop_shadow_match':'coach_loop_shadow_mismatch',build:BUILD,...report,consecutiveSuccesses:count}));api('/api/v2/shadow/report',{method:'POST',headers:{'Idempotency-Key':window.RunnerBearV1026?.idempotency?.('shadow')||`shadow:${Date.now()}`},body:JSON.stringify(report)}).catch(()=>{});
    return{match,count};
  }
  ...
  async function refresh(scope='home'){
    ...
    if(scope==='home'&&fullLoadedRevision&&document.querySelector('.view.active')?.id!=='today')scope='full';
    let data=await api(`/api/v2/bootstrap?scope=${scope}`);
    if(serial!==refreshSerial)return snapshot||data;
    ...
    if(serial!==refreshSerial)return snapshot||data;
    const integrity=window.RunnerBearV1026?.assertRevision?.(data);
    if(!data.ok||!integrity?.ok)throw new Error('Gjeldende datagrunnlag kunne ikke verifiseres.');
    fullLoadedRevision=scope==='full'?data.planRevisionId||'':'';
    if(data.flags?.coach_loop_read!==true){if(data.flags?.coach_loop_shadow===true){shadowCompare(data)}snapshot=data;localStorage.removeItem(CACHE);document.documentElement.classList.remove('rb1026-active');window.RunnerBearCloud?.hydrateState?.(data.clientState||{});window.RunnerBearCoachOS?.renderView?.(document.querySelector('.view.active')?.id||'today');return data}
    return install(data);
  }
  ...
  function scheduleSync(event){if(window.RunnerBearClientMigrating===true)return;const key=String(event?.detail?.key||'');if(!/(runfest26_week_adjustments|runnerbear_v107_plan_moves|runnerbear_v107_plan_locks|runnerbear_v109_goals|runfest26_training_profile_v10|runnerbear_v118_day_modes)/.test(key))return;clearTimeout(syncTimer);const goalChanged=/goals/.test(key),preferences=/training_profile/.test(key);syncTimer=setTimeout(()=>reconfigure({reason:goalChanged?'goal-change':preferences?'training-preferences':'user-change',trigger:goalChanged?'goal_changed':preferences?'training_preferences_changed':'plan_adjustment',confirm:!preferences,goalChanged}).catch(error=>console.warn(JSON.stringify({event:'coach_loop_write_failed',build:BUILD,code:error.code||'',message:error.message}))),900)}
  async function previewUserWorkout(workoutId,patch,key){
    if(!snapshot?.flags?.coach_loop_write||snapshot.storage?.writeAvailable===false)throw new Error('Planendringer er ikke tilgjengelige akkurat nå.');
    const before=snapshot.activePlan?.items||[],source=before.find(row=>row.workoutId===workoutId);
    if(!source||source.status!=='scheduled'||source.lockLevel==='user')throw new Error('Økten er endret eller låst. Åpne den på nytt.');
    const items=before.map(row=>row!==source?row:{...row,title:patch.title,workoutType:patch.type,sport:patch.sport||row.sport,plannedDurationSeconds:patch.plannedDurationSeconds,plannedDistanceM:patch.plannedDistanceM,prescription:{...(patch.prescription||{version:1,main:{kind:patch.type==='rest'?'recovery':'continuous',seconds:patch.plannedDurationSeconds}}),legacy:{...row.prescription?.legacy,desc:patch.desc,detail:patch.detail}},explicitChoice:true});
    const preview=await api('/api/v2/plan/preview',{method:'POST',timeoutMs:15000,headers:{'Idempotency-Key':key+':preview'},body:JSON.stringify({expectedPlanRevisionId:snapshot.planRevisionId,items,reason:'day-choice-preview',trigger:'day_choice'})});
    if(!preview.previewId)throw new Error('Forslaget passer ikke med treningsrammene. Velg en annen tilpasning.');
    return{...preview,key,before};
  }
  async function commitUserWorkout(preview){
    if(!preview?.previewId||snapshot?.planRevisionId!==preview.expectedPlanRevisionId)throw new Error('Planen har endret seg. Hent et nytt forslag.');
    const saved=await api('/api/v2/plan/commit',{method:'POST',timeoutMs:20000,headers:{'Idempotency-Key':preview.key+':commit'},body:JSON.stringify({expectedPlanRevisionId:preview.expectedPlanRevisionId,previewId:preview.previewId,reason:'day-choice-preview'})});
    if(saved.ok!==true)throw new Error('Lagringen er ikke bekreftet. Prøv igjen.');
    try{await refresh('full');if(snapshot?.flags?.coach_loop_sync)watchSync()}catch{if(saved.activePlan)snapshot={...snapshot,planRevisionId:saved.planRevisionId,activePlan:saved.activePlan,oneDecision:null};saved.refreshPending=true;window.dispatchEvent(new CustomEvent('runnerbear:saved-refresh-pending'))}
    return saved;
  }
  ...
  async function submitPaceCorrection(input,key){
    const saved=await api('/api/v2/pace-corrections',{method:'POST',timeoutMs:15000,headers:{'Idempotency-Key':key},body:JSON.stringify(input)});if(saved.ok!==true||!saved.correction?.eventId)throw new Error('Korrigeringen er ikke bekreftet lagret.');
    const merge=()=>{if(!snapshot)return;const c=saved.correction,rows=snapshot.paceCorrections||[];if(rows.some(r=>r.eventId===c.eventId))return;snapshot={...snapshot,paceCorrections:[c,...rows.map(r=>r.activitySource===c.activitySource&&r.activityId===c.activityId?{...r,rank:r.rank+1}:r)]}};merge();try{await refresh('full')}catch{}merge();return saved;
  }
  async function submitWorkoutResponse(input,key){
    const result=await api('/api/v2/workout-responses',{method:'POST',timeoutMs:15000,headers:{'Idempotency-Key':key},body:JSON.stringify(input)});
    if(result.ok!==true||!result.eventId)throw new Error('Svaret er ikke bekreftet lagret. Prøv igjen.');
    if(snapshot)snapshot={...snapshot,responseEvents:[{event_id:result.eventId,event_type:'feedback:workout',occurred_at:result.savedAt,local_date:input.localDate,payload:{...input,sourceId:key}},...(snapshot.responseEvents||[]).filter(r=>r.event_id!==result.eventId)]};
    try{await refresh('home')}catch(error){console.warn(JSON.stringify({event:'response_saved_refresh_pending',build:BUILD,code:error.code||''}))}
    return result;
  }
  ...
  async function loadFullForView(event){const view=String(event?.detail?.view||'');if(!['plan','goals','race','more'].includes(view)||!snapshot?.planRevisionId||fullLoadedRevision===snapshot.planRevisionId)return;try{if(!fullRefreshPromise)fullRefreshPromise=refresh('full').finally(()=>{fullRefreshPromise=null});await fullRefreshPromise}catch(error){console.warn(JSON.stringify({event:'coach_loop_history_refresh_failed',build:BUILD,code:error.code||'',message:error.message}))}}
  function verifiedCache(){const cached=read(CACHE,null),check=window.RunnerBearV1026?.assertRevision?.(cached);return cached?.flags?.coach_loop_read===true&&check?.ok?cached:null}
  function start(){
    ...
    startPromise=(async()=>{try{const data=await refresh('home'),input=legacyInput();lastSignature=window.RunnerBearV1026?.stable?.({plan:input.effectivePlan,profile:input.profile,constraints:input.constraints,goal:input.goal})||JSON.stringify(input);window.addEventListener('runnerbear:state-dirty',scheduleSync);window.addEventListener('runnerbear:view',loadFullForView);setTimeout(()=>applySafeAuto().then(()=>applyRealignment()).catch(error=>console.warn(JSON.stringify({event:'coach_loop_auto_update_failed',build:BUILD,code:error.code||''}))),0);document.documentElement.classList.remove('rb10312-cached-first-paint');return snapshot||data}catch(error){console.error(JSON.stringify({event:'coach_loop_bootstrap_error',build:BUILD,code:error.code||'',message:error.message}));if(cached)return snapshot;throw error}})();return startPromise
  }
  window.RunnerBearCloudV11={build:BUILD,start,refresh,refreshSync,retrySync,verifySyncRepair,reconfigure,previewUserWorkout,commitUserWorkout,resolveDecision,applySafeAuto,applyRealignment,undoPlan,submitFeedback,submitPaceCorrection,submitWorkoutResponse,submitCheckIn,shadowCompare,snapshot:()=>snapshot,decision:()=>snapshot?.coachDecision||null,oneDecision:()=>snapshot?.oneDecision||null,toLegacyDecision,proposalFor};
