/* RunnerBear 10.22 canonical UI · generated by scripts/build-v1022.mjs */
/* RunnerBear v10.22 · canonical, targeted coaching UI within locked Design Direction 1.0
...
(function(){
  ...
  const K={
    ...
    profile:'runfest26_training_profile_v10',legacyProfile:'runnerbear_v7_profile',dayModes:'runnerbear_v118_day_modes',readiness:'runnerbear_v1022_daily_readiness'
  };
  let eventScope=null;
  ...
  function trackEvent(name,detail={}){
    const payload={event:name,runnerbearVersion:'10.22',...detail};
  ...
  function todayCoachHtml(base,d,suggest){
    const coach=coachBasis(),reported=coach.reported||readinessState(),stateLabel={fresh:'Frisk',tired:'Litt sliten',heavy:'Tung / svært trøtt',unknown:'Ikke meldt'}[reported.state]||'Ikke meldt',reasonLabels={poor_sleep:'dårlig søvn',fatigue:'generell tretthet',heavy_legs:'tunge bein',stress:'stress',illness:'sykdomsfølelse',achilles:'akilles / hælfeste'},reportedReasons=(reported.reasons||[]).map(x=>reasonLabels[x]).filter(Boolean),planned=coach.planned||rawPlanFor(base),proposed=coach.proposed||planned,choice=reported.choice||'pending';
  ...
  function todayHtml(){
    const base=planFor(today())||effectiveSchedule().find(p=>p.ds>=today())||effectiveSchedule().at(-1),d=decision(),sync=syncState(),done=isDone(base),coach=coachBasis(),suggest=coach.requiresChoice&&coach.reported?.choice==='pending',pending=!done&&manualCandidates(base).length>0;
    return `<div id="rb107Today" class="rb107-surface rb119b rb119b-today"><div class="rb107-shell">${appBarHtml()}${raceMetaHtml()}${viewTitleHtml(done?'Dagens resultat':'Dagens økt',done?'Gjennomført og analysert':'')}
      ${workoutHeroHtml(base)}${changeNoticeHtml(base)}${done?resultCardHtml(base,'today'):`${pending?pendingResultHtml(base):''}${todayCoachHtml(base,d,suggest)}${readinessCardHtml()}${coachWatchHtml()}${todayActionsHtml(base,d,suggest)}`}
  ...
  function setFormToday(value){
    const p=planFor(today()),stateValue=['fresh','tired','heavy'].includes(value)?value:'unknown',current=readinessState(),reasons=stateValue==='fresh'?[]:current.reasons;clearReadinessAdjustment(p);saveReadinessState({state:stateValue,reasons,choice:'pending'});addLog(`Form i dag: ${stateValue==='fresh'?'frisk':stateValue==='tired'?'litt sliten':'tung / svært trøtt'}.`,'manual');trackEvent('daily_readiness_changed',{state:stateValue});renderAll();
  }
  function toggleReadinessReason(reason){
    const current=readinessState(),reasons=current.reasons.includes(reason)?current.reasons.filter(x=>x!==reason):[...current.reasons,reason],p=planFor(today());clearReadinessAdjustment(p);saveReadinessState({state:current.state==='unknown'?'tired':current.state,reasons,choice:'pending'});trackEvent('daily_readiness_reason_changed',{reason,selected:reasons.includes(reason)});renderAll();
  }
  function acceptReadinessRecommendation(){
    ...
    const before=snapshot(K.adjustments),all=read(K.adjustments,{}),x=coach.proposed;all[p.sourceLabel]={created:new Date().toISOString(),source:'runnerbear-v10.22',reason:'readiness-v1022',type:x.type,title:x.title,desc:x.desc,detail:x.detail,km:Number(x.km||0),shoe:x.shoe||'',fuel:x.fuel||''};write(K.adjustments,all);saveReadinessState({choice:'accepted'});addLog(`${coach.planned.title} er tilpasset etter Form i dag: ${x.title}. Ingen treningsgjeld.`,'manual',before);trackEvent('daily_readiness_recommendation',{choice:'accepted',severity:coach.severity});toast('Coachens anbefaling er brukt');renderAll();
  }
  function keepPlannedWorkout(){
    const p=planFor(today()),coach=readinessDecision(p);if(!p)return;const before=snapshot(K.adjustments);clearReadinessAdjustment(p);saveReadinessState({choice:'keep'});addLog(`${coach.planned?.title||p.title} beholdes etter brukerens valg, med forsiktighetsråd.`,'manual',before);trackEvent('daily_readiness_recommendation',{choice:'keep',severity:coach.severity});toast('Planlagt økt beholdes med margin');renderAll();
  }
  ...
  function init(){
    ...
    if(!engine()||!clarityModel()){
      console.error(JSON.stringify({event:'runnerbear_fatal_init',build:'10.22',reason:'canonical core unavailable'}));
