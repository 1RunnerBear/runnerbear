...
test('v11.4.1 locks retry UX, direct sync action and read-only health',async()=>{
  ...
  assert.match(ui,/data-rb1141-coach-retry/);assert.match(ui,/Åpne kontroll/);assert.match(css,/min-height:44px/);assert.ok(manifest.styles.includes('runnerbear-v1141-reliability.css'));assert.match(entry,/healthReadOnly:true/);assert.match(config,/src\/index-v1141\.js/);assert.match(workflow,/cloudBuild!=="11\.4\.1"/);assert.doesNotMatch(healthBlock,/repairAccidentalGoalState|repairBakkenV11Plan|processPendingSync|reconcileActiveSyncProjection/);
});
...
test('v11.4.1 preserves locked coach authority and bounded adjustment',()=>{
  ...
  assert.match(entry,/planWritesByAi:false/);assert.match(entry,/maximumReductionPercent:20/);assert.match(entry,/emptyResponsesAccepted:false/);assert.match(entry,/retryableTurnState:true/);assert.doesNotMatch(coach,/(INSERT INTO|UPDATE|DELETE FROM) rb_plan_/);
});
