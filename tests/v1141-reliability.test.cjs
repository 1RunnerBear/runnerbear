...
test('v11.6 removes chat retry UX while preserving direct sync action and read-only health',async()=>{
  ...
  assert.doesNotMatch(ui,/data-rb1141-coach-retry|Coach Live|coachLive|coach-live/);assert.match(ui,/Åpne kontroll/);assert.match(css,/min-height:44px/);assert.ok(manifest.styles.includes('runnerbear-v116-contextual-coach.css'));assert.match(entry,/healthReadOnly:true/);assert.match(config,/src\/index-v116\.js/);assert.match(workflow,/verify-v116-health\.mjs/);assert.doesNotMatch(healthBlock,/repairAccidentalGoalState|repairBakkenV11Plan|processPendingSync|reconcileActiveSyncProjection/);
});
