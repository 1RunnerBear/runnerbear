...
test('v11.6 removes chat retry UX while preserving direct sync action and read-only health',async()=>{
  ...
  assert.doesNotMatch(ui,/data-rb1141-coach-retry|Coach Live|coachLive|coach-live|data-rb108-publish-plan/);assert.match(ui,/Kalenderen oppdateres automatisk/);assert.match(css,/min-height:44px/);assert.ok(JSON.parse(fs.readFileSync('tests/fixtures/v121-css-pruning.json','utf8')).sources.includes('runnerbear-v116-contextual-coach.css'));assert.match(entry,/healthReadOnly:true/);assert.match(config,/src\/index-v118\.js/);assert.match(workflow,/verify-v116-health\.mjs/);assert.doesNotMatch(healthBlock,/repairAccidentalGoalState|repairBakkenV11Plan|processPendingSync|reconcileActiveSyncProjection/);
});
