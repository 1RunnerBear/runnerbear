...
test('v11.4.1 locks retry UX, direct sync action and read-only health',async()=>{
  ...
  assert.match(ui,/data-rb1141-coach-retry/);assert.match(ui,/Åpne kontroll/);assert.match(css,/min-height:44px/);assert.ok(manifest.styles.includes('runnerbear-v115-premium-ux.css'));assert.match(entry,/healthReadOnly:true/);assert.match(config,/src\/index-v115\.js/);assert.match(workflow,/cloudBuild!=="11\.5\.0"/);assert.doesNotMatch(healthBlock,/repairAccidentalGoalState|repairBakkenV11Plan|processPendingSync|reconcileActiveSyncProjection/);
});
