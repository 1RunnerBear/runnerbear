...
test('release metadata and production health gate agree on v11.5',()=>{
  const bridgeWorkflow=read('.github/workflows/deploy-tredict-bridge.yml');
  assert.equal(JSON.parse(read('runnerbear-version.json')).build,'11.5.0');
  assert.match(read('site.webmanifest'),/v11500/);
  assert.match(read('cloud/runnerbear-cloud/src/index-v115.js'),/const BUILD='11\.5\.0'/);
  assert.match(read('cloud/runnerbear-cloud/src/index-v982.js'),/const BUILD='10\.25\.1'/);
  assert.match(read('cloud/runnerbear-cloud/wrangler.jsonc'),/src\/index-v115\.js/);
  assert.match(read('.github/workflows/runnerbear-cloud-deploy.yml'),/cloudBuild!==\"11\.5\.0\"/);
