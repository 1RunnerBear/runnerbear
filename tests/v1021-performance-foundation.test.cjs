...
test('release metadata and production health gate agree on v10.28.1',()=>{
  const bridgeWorkflow=read('.github/workflows/deploy-tredict-bridge.yml');
  assert.equal(JSON.parse(read('runnerbear-version.json')).build,'10.28.1');
  assert.match(read('site.webmanifest'),/v10281/);
  assert.match(read('cloud/runnerbear-cloud/src/index.js'),/const BUILD = '10\.28\.1'/);
  ...
  assert.match(read('.github/workflows/runnerbear-cloud-deploy.yml'),/cloudBuild!==\"10\.28\.1\"/);
