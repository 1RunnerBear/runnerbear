...
test('production entrypoint binds Workers AI and cannot mutate the canonical plan',()=>{
  ...
  assert.match(config,/src\/index-v112\.js/);assert.match(config,/"binding": "AI"/);assert.match(config,/@cf\/zai-org\/glm-4\.7-flash/);assert.match(entry,/coachLiveInference:!!env\.AI/);assert.match(routes,/env\.AI\.run/);assert.doesNotMatch(routes,/(INSERT INTO|UPDATE|DELETE FROM) rb_plan_/);assert.match(workflow,/coachLiveAudit\?\.ok!==true/);assert.match(workflow,/schemaVersion!==4/);
});
