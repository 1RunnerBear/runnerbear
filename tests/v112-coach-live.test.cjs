...
test('Concept 1 UI replaces Coach Live with contextual guidance and four navigation tabs',()=>{
  ...
  assert.match(ui,/function contextualCoach\(/);assert.match(ui,/function contextualHealthStripHtml\(/);assert.match(ui,/function contextualWorkoutCoachHtml\(/);assert.match(ui,/Dagens oppgave/);assert.match(ui,/Dette betyr for uka/);assert.doesNotMatch(ui,/Coach Live|coachLive|coach-live|data-rb112|rb112/);assert.match(css,/Contextual Coach/);assert.match(css,/The coach speaks once, in context/);assert.match(css,/@media\(max-width:680px\)/);assert.ok(JSON.parse(fs.readFileSync('tests/fixtures/v121-css-pruning.json','utf8')).sources.includes('runnerbear-v116-contextual-coach.css'));assert.doesNotMatch(html,/data-tab="coach"|>Coach Live<\/button>/);
});

test('production retires Coach Live routes, inference and plan mutation authority',()=>{
  const config=fs.readFileSync('cloud/runnerbear-cloud/wrangler.jsonc','utf8'),entry=fs.readFileSync('cloud/runnerbear-cloud/src/index-v116.js','utf8'),contextual=fs.readFileSync('cloud/runnerbear-cloud/src/v116/contextual-coach.js','utf8'),workflow=fs.readFileSync('.github/workflows/runnerbear-cloud-deploy.yml','utf8');
  assert.match(config,/src\/index-v118\.js/);assert.doesNotMatch(config,/"binding":\s*"AI"|COACH_LIVE_MODEL|@cf\/zai-org/);assert.match(entry,/retiredCoachLiveResponse/);assert.match(entry,/coachLive:false/);assert.match(entry,/coachLiveRoutes:false/);assert.match(contextual,/COACH_LIVE_REMOVED/);assert.match(contextual,/status:410/);assert.match(contextual,/planWritesByAi:false/);assert.match(contextual,/maximumReductionPercent:20/);assert.doesNotMatch(contextual,/(INSERT INTO|UPDATE|DELETE FROM) rb_plan_|ai\.run/);assert.match(workflow,/coach_live_removed/);assert.match(workflow,/verify-v116-health\.mjs/);
});
