...
test('Concept 1 UI adds contextual Coach Live without a fifth navigation tab',()=>{
  const ui=fs.readFileSync('runnerbear-ui-v11-source.js','utf8'),css=fs.readFileSync('runnerbear-v115-premium-ux.css','utf8'),html=fs.readFileSync('index.html','utf8'),manifest=JSON.parse(fs.readFileSync('runnerbear-v11-assets.json','utf8'));
  assert.match(ui,/function coachLiveCardHtml/);assert.match(ui,/function coachLiveModalHtml/);assert.match(ui,/aria-labelledby="rb112CoachTitle"/);assert.match(ui,/coachLiveContextButtonHtml\('workout'/);assert.match(ui,/coachLiveContextButtonHtml\('body_response'/);assert.match(ui,/Ingen plan endres her/);assert.match(css,/Concept 1/);assert.match(css,/min-height:48px/);assert.match(css,/@media\(max-width:680px\)/);assert.ok(manifest.styles.includes('runnerbear-v115-premium-ux.css'));assert.doesNotMatch(html,/data-tab="coach"|>Coach Live<\/button>/);
});
...
test('production entrypoint binds Workers AI and cannot mutate the canonical plan',()=>{
  ...
  assert.match(config,/src\/index-v115\.js/);assert.match(config,/"binding": "AI"/);assert.match(config,/@cf\/zai-org\/glm-4\.7-flash/);assert.match(entry,/coachLiveInference:!!env\.AI/);assert.match(routes,/runCoachInference\(env\.AI/);assert.match(routes,/ai\.run/);assert.doesNotMatch(routes,/(INSERT INTO|UPDATE|DELETE FROM) rb_plan_/);assert.match(workflow,/coachLiveAudit\?\.ok!==true/);assert.match(workflow,/schemaVersion!==4/);
});
