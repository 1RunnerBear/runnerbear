...
const app=fs.readFileSync(path.join(root,'runnerbear-ui-v1027.js'),'utf8');
...
function assertCanonicalStyle(source){
  assert.match(html,/runnerbear-v1027\.css\?v=10280/);
...
test('v10.13 mounts one coach-first brand and navigation system',()=>{
  ...
  assert.match(app,/version:'10\.28\.0'/);
...
test('PWA and live build identify the same production release',()=>{
  assert.equal(manifest.start_url,'/?app=v10280');
  assert.equal(version.build,'10.28.0');
