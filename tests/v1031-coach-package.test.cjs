...
test('v11.3 wraps the locked v11.2 entrypoint and preserves accessible review and repair UI',()=>{
  const config=fs.readFileSync('cloud/runnerbear-cloud/wrangler.jsonc','utf8'),current=fs.readFileSync('cloud/runnerbear-cloud/src/index-v113.js','utf8'),previous=fs.readFileSync('cloud/runnerbear-cloud/src/index-v112.js','utf8'),entry=fs.readFileSync('cloud/runnerbear-cloud/src/index-v11.js','utf8'),base=fs.readFileSync('cloud/runnerbear-cloud/src/index-v11-base.js','utf8'),ui=fs.readFileSync('runnerbear-ui-v11-source.js','utf8'),manifest=JSON.parse(fs.readFileSync('runnerbear-v11-assets.json','utf8'));
  assert.match(config,/index-v113\.js/);assert.match(current,/\.\/index-v112\.js/);assert.match(previous,/\.\/index-v11\.js/);assert.match(entry,/v11\/routes\.js/);assert.match(base,/v11\/routes\.js/);assert.match(ui,/function weeklyReviewCardHtml/);assert.match(ui,/role="dialog" aria-modal="true" aria-labelledby="rb1031ReviewTitle"/);assert.match(ui,/data-rb1031-sync-verify/);assert.ok(manifest.styles.includes('runnerbear-v1031-coach-package.css'));
});
...
test('startup paints verified cache before revalidation and legacy plan publishing yields to canonical sync',()=>{
  ...
  assert.match(canonical,/const cached=verifiedCache\(\);\s*if\(cached\)\{install\(cached\)/);assert.match(legacy,/function canonicalSyncOwner\(\)/);assert.match(legacy,/if\(!IS_CLOUD\|\|canonicalSyncOwner\(\)\)return/);assert.match(html,/runnerbear-data-v11\.js\?v=11300" defer/);assert.doesNotMatch(ui,/canonicalSyncBannerHtml\(\)[^\n]+data-rb1028-sync-retry/);
});
