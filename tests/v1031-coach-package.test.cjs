...
test('v11 uses a new entrypoint and preserves accessible review and repair UI',()=>{
  ...
  assert.match(config,/index-v11\.js/);assert.match(entry,/v11\/routes\.js/);assert.match(base,/v11\/routes\.js/);assert.match(ui,/function weeklyReviewCardHtml/);assert.match(ui,/role="dialog" aria-modal="true" aria-labelledby="rb1031ReviewTitle"/);assert.match(ui,/data-rb1031-sync-verify/);assert.ok(manifest.styles.includes('runnerbear-v1031-coach-package.css'));
});
...
test('startup paints verified cache before revalidation and legacy plan publishing yields to canonical sync',()=>{
  ...
  assert.match(canonical,/const cached=verifiedCache\(\);\s*if\(cached\)\{install\(cached\)/);assert.match(legacy,/function canonicalSyncOwner\(\)/);assert.match(legacy,/if\(!IS_CLOUD\|\|canonicalSyncOwner\(\)\)return/);assert.match(html,/runnerbear-data-v11\.js\?v=11000" defer/);assert.doesNotMatch(ui,/canonicalSyncBannerHtml\(\)[^\n]+data-rb1028-sync-retry/);
});
