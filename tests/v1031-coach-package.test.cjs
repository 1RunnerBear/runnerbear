...
test('v11.6 preserves the locked wrapper chain and accessible review and repair UI',()=>{
  ...
  assert.match(config,/index-v118\.js/);assert.match(contextual,/\.\/index-v115\.js/);assert.match(ux,/\.\/index-v1141\.js/);assert.match(release,/\.\/index-v114\.js/);assert.match(current,/\.\/index-v113\.js/);assert.match(oneDecision,/\.\/index-v112\.js/);assert.match(previous,/\.\/index-v11\.js/);assert.match(entry,/v11\/routes\.js/);assert.match(base,/v11\/routes\.js/);assert.match(ui,/function weeklyReviewCardHtml/);assert.match(ui,/role="dialog" aria-modal="true" aria-labelledby="rb1031ReviewTitle"/);assert.doesNotMatch(ui,/data-rb108-publish-plan/);assert.ok(JSON.parse(fs.readFileSync('tests/fixtures/v121-css-pruning.json','utf8')).sources.includes('runnerbear-v1031-coach-package.css'));
});
...
test('rolling sync uses one durable binding and moves the same external workout id',async()=>{
  const {projectRollingSync,stableExternalId,canonicalWorkoutProjection}=await import('../cloud/runnerbear-cloud/src/v11/sync-projection.js'),item=row('quality-stable','2026-08-27',{workoutType:'quality'}),desired=canonicalWorkoutProjection(item,planRevisionId,'rb-plan-primary'),binding={workout_id:item.workoutId,status:'confirmed',confirmed_date:'2026-08-26',remote_fingerprint:desired.fingerprint},operations=projectRollingSync([item],planRevisionId,'2026-08-24','tredict',[binding],'rb-plan-primary');
  assert.equal(operations.length,1);assert.equal(operations[0].operationType,'move');assert.equal(operations[0].payload.previousDate,'2026-08-26');assert.equal(stableExternalId(item.workoutId),'rb-workout-quality-stable');assert.equal(projectRollingSync([item],planRevisionId,'2026-08-24','tredict',[{...binding,confirmed_date:item.localDate}],'rb-plan-primary').length,0);
});
...
test('startup paints verified cache before revalidation and legacy plan publishing yields to canonical sync',()=>{
  ...
  assert.match(canonical,/const cached=verifiedCache\(\);\s*if\(cached\)\{install\(cached\)/);assert.match(legacy,/function canonicalSyncOwner\(\)/);assert.match(legacy,/if\(!IS_CLOUD\|\|canonicalSyncOwner\(\)\)return/);assert.match(html,/runnerbear-data-v11\.js\?v=12201" defer/);assert.doesNotMatch(ui,/canonicalSyncBannerHtml\(\)[^\n]+data-rb1028-sync-retry/);
});
