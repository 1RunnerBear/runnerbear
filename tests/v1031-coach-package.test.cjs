...
test('v11.6 preserves the locked wrapper chain and accessible review and repair UI',()=>{
  ...
  assert.match(config,/index-v118\.js/);assert.match(contextual,/\.\/index-v115\.js/);assert.match(ux,/\.\/index-v1141\.js/);assert.match(release,/\.\/index-v114\.js/);assert.match(current,/\.\/index-v113\.js/);assert.match(oneDecision,/\.\/index-v112\.js/);assert.match(previous,/\.\/index-v11\.js/);assert.match(entry,/v11\/routes\.js/);assert.match(base,/v11\/routes\.js/);assert.match(ui,/function weeklyReviewCardHtml/);assert.match(ui,/role="dialog" aria-modal="true" aria-labelledby="rb1031ReviewTitle"/);assert.doesNotMatch(ui,/data-rb108-publish-plan/);assert.ok(manifest.styles.includes('runnerbear-v1031-coach-package.css'));
});
...
test('startup paints verified cache before revalidation and legacy plan publishing yields to canonical sync',()=>{
  ...
  assert.match(canonical,/const cached=verifiedCache\(\);\s*if\(cached\)\{install\(cached\)/);assert.match(legacy,/function canonicalSyncOwner\(\)/);assert.match(legacy,/if\(!IS_CLOUD\|\|canonicalSyncOwner\(\)\)return/);assert.match(html,/runnerbear-data-v11\.js\?v=11800" defer/);assert.doesNotMatch(ui,/canonicalSyncBannerHtml\(\)[^\n]+data-rb1028-sync-retry/);
});
