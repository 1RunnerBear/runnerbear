...
test('v10.27 API reflows preferences atomically while preserving plan, activity, feedback and matching history',async()=>{
  ...
  for(const flag of ['coach_loop_shadow','coach_loop_read','coach_loop_ui','coach_loop_write'])db.sqlite.prepare("INSERT INTO rb_feature_flags(user_id,flag,enabled,payload_json,updated_at) VALUES('primary',?,1,'{}',?)").run(flag,new Date().toISOString());
