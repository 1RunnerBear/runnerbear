...
test('canonical Cloud load performs no parallel legacy bootstrap or client sync timer',()=>{
  ...
  const window={RunnerBearRelease:{build:'12.4.0'},RunnerBearCloudV11:{refresh:async()=>({})},RunnerBearV1025:{},addEventListener:(name,fn)=>{listeners[name]=fn},dispatchEvent(){}};
  ...
  assert.equal(window.RunnerBearCloud.build ,'12.4.0');
...
test('v12 release, cache and state hydration use one current contract',()=>{
  ...
  assert.equal(manifest.build ,'12.4.0');assert.equal(version.build ,'12.4.0');
  ...
  assert.match(html,/runnerbear-core-v11\.js\?v=12401/);assert.match(html,/runnerbear-data-v11\.js\?v=12401/);
