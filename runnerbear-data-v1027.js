/* RunnerBear 10.29.1 lazy Cloud/data runtime · generated by scripts/build-v1027.mjs */
...
(function(){
  'use strict';
  const BUILD='10.29.1',CACHE='runnerbear_coach_loop_snapshot_v1',SHADOW_COUNT='runnerbear_coach_loop_shadow_success_v1';
  ...
  async function refreshSync(){if(!snapshot?.flags?.coach_loop_sync)return snapshot;const result=await api('/api/v2/sync/status');snapshot={...snapshot,sync:result.operations||[]};localStorage.setItem(CACHE,JSON.stringify(snapshot));window.RunnerBearCoachOS?.renderView?.(document.querySelector('.view.active')?.id||'today');return snapshot}
