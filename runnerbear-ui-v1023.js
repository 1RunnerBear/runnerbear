/* RunnerBear 10.23 canonical UI · generated by scripts/build-v1023.mjs */
/* RunnerBear v10.23 · canonical, targeted coaching UI within locked Design Direction 1.0
...
(function(){
  ...
  const K={
    ...
    profile:'runfest26_training_profile_v10',legacyProfile:'runnerbear_v7_profile',dayModes:'runnerbear_v118_day_modes',readiness:'runnerbear_v1022_daily_readiness',garminSync:'runnerbear_v1023_garmin_sync'
  };
  let eventScope=null;
  ...
  const planIntegrity=()=>window.RunnerBearV1023||null;
  ...
  function trackEvent(name,detail={}){
    const payload={event:name,runnerbearVersion:'10.23',...detail};
    if(Array.isArray(window.dataLayer))window.dataLayer.push(payload);
    try{window.dispatchEvent(new CustomEvent('runnerbear:analytics',{detail:payload}))}catch{}
  }
  let garminSyncService=null;
  function garminSync(){
    if(garminSyncService)return garminSyncService;
    garminSyncService=planIntegrity()?.createGarminSyncService?.({storage:localStorage,stateKey:K.garminSync,transport:()=>window.RunnerBearGarminTraining||null,onEvent:(name,detail)=>{trackEvent(name,detail);try{window.dispatchEvent(new CustomEvent('runnerbear:state-dirty',{detail:{key:K.garminSync}}))}catch{}},debounceMs:600})||null;
  ...
  function allMatches(){
    ...
    replacements.forEach((replacement,baseDs)=>{const p=plans.find(row=>row.baseDs===baseDs),id=`workout_replaced:${baseDs}:${replacement.activity.id}`,first=!read(K.seen,{})[id];addLog(`Planlagt ${p?.title||'økt'} er erstattet av ${sportLabel(replacement.activity,p)}. Planen videre står.`,'activity',null,id);if(first){queueGarminMutation('plan:workout-replaced',[p],{previousDate:p?.ds,reason:'activity-sync'});trackEvent('workout_replaced',{planType:p?.type||'',activityKind:sportKind(replacement.activity,p)})}});
  ...
  function undoEntry(id){
    ...
    queueGarminMutation('plan:workout-adjusted',effectiveSchedule().filter(p=>p.ds>=today()),{previousDates,reason:'coach-log-undo'});
  ...
  function queueGarminMutation(type,plans=[],detail={}){
    const service=garminSync(),integrity=planIntegrity();if(!service||!integrity)return;
  ...
  function adapt(p,reason){
    ...
    write(K.adjustments,all);const changed=basePlan(p.baseDs);queueGarminMutation(reason==='skip'?'plan:workout-cancelled':'plan:workout-adjusted',[changed],{previousDate:p.ds,reason});addLog(`${p.title} er tilpasset: ${reason==='time'?'kortere tid':reason==='tired'?'sliten kropp':reason==='achilles'?'akillesrespons':'økten utgår'}.`,'manual',before);toast('Dagen er tilpasset uten treningsgjeld');renderAll();
  }
  ...
  function setChoice(p,mode){
    ...
    const changed=basePlan(p.baseDs);queueGarminMutation('plan:workout-adjusted',[changed],{previousDate:p.ds,reason:`day-choice-${mode}`});addLog(`${formatDate(p.ds,{weekday:'long'})}: ${mode==='run'?'rolig jogg':mode==='row'?'Concept2':mode==='bike'?'Zwift':'hvile'} er valgt.`,'manual',before);renderAll();
  }
  ...
  function garminStatusFor(p){const service=garminSync(),externalId=planIntegrity()?.stableExternalId?.(p)||`rb-workout-${p?.baseDs||''}`,item=service?.status?.(externalId)||{status:'not_synced'};return{...item,available:service?.available?.()===true,externalId}}
  function garminStatusHtml(p){
    const stateValue=garminStatusFor(p),labels={synced:'Garmin synkronisert',pending:'Synkroniserer med Garmin…',syncing:'Synkroniserer med Garmin…',error:'Garmin-synk feilet',not_synced:stateValue.available?'Venter på Garmin-synk':'Garmin kalenderpublisering venter på Training API-tilgang.'},label=labels[stateValue.status]||labels.not_synced;
  ...
  function moreHtml(){
    ...
    const service=garminSync(),garminState=service?.all?.()||{items:{},queue:[]},garminItems=Object.values(garminState.items||{}),garminError=garminItems.some(x=>x.status==='error'),garminBusy=garminItems.some(x=>['pending','syncing'].includes(x.status)),garminAvailable=service?.available?.()===true,garminLabel=garminError?'Synkfeil':garminBusy?'Synkroniserer':garminAvailable?'Automatisk':'Venter API',garminCopy=garminError?'Garmin-synk feilet. RunnerBear-planen er fortsatt lagret og riktig.':garminBusy?'Planendringer er lagret. Garmin oppdateres i bakgrunnen.':garminAvailable?'Planendringer synkroniseres automatisk med Garmin.':'Garmin kalenderpublisering venter på Training API-tilgang. Plan, kø, stabile ID-er og idempotens er klare.';
  ...
  function restoreMove(p){const before=snapshot(K.moves),moves=read(K.moves,{}),partner=Object.keys(moves).find(key=>moves[key]===p.baseDs),previousDates={[p.baseDs]:p.ds};if(partner)previousDates[partner]=basePlan(partner)?.ds||'';delete moves[p.baseDs];if(partner)delete moves[partner];write(K.moves,moves);matchCache.signature='';state.selectedDs=p.baseDs;const restored=[basePlan(p.baseDs),partner?basePlan(partner):null].filter(Boolean);queueGarminMutation('plan:workout-moved',restored,{previousDates,reason:'restore'});addLog(`${p.title} er gjenopprettet til opprinnelig dag.`,'manual',before);trackEvent('plan_move_undone');renderAll()}
  function undoVisibleChange(p,kind){
    if(!p)return;
    if(kind==='choice'){
      const before=snapshot(K.dayModes),previousDate=p.ds,all=read(K.dayModes,{});delete all[p.baseDs];write(K.dayModes,all);matchCache.signature='';const restored=planFor(p.baseDs)||basePlan(p.baseDs);queueGarminMutation('plan:workout-adjusted',[restored],{previousDate,reason:'choice-undo'});addLog(`${p.title}: engangsvalget er fjernet.`,'manual',before);toast('Dagen følger planen igjen');renderAll();return;
    }
    if(kind==='adjustment'){
      const previousDate=p.ds,all=read(K.adjustments,{});delete all[p.sourceLabel];write(K.adjustments,all);matchCache.signature='';const restored=planFor(p.baseDs)||basePlan(p.baseDs);queueGarminMutation('plan:workout-adjusted',[restored],{previousDate,reason:'adjustment-undo'});addLog(`${p.title}: synlig planendring er angret.`,'manual');toast('Planendringen er angret');renderAll();return;
    }
    const before=snapshot(K.moves),moves=read(K.moves,{}),partner=Object.keys(moves).find(key=>moves[key]===p.baseDs),previousDates={[p.baseDs]:p.ds};if(partner)previousDates[partner]=planFor(moves[partner])?.ds||moves[partner];delete moves[p.baseDs];if(partner)delete moves[partner];write(K.moves,moves);matchCache.signature='';state.selectedDs=p.baseDs;queueGarminMutation('plan:workout-moved',[basePlan(p.baseDs),partner?basePlan(partner):null].filter(Boolean),{previousDates,reason:'visible-move-undo'});addLog(`${p.title} er flyttet tilbake til opprinnelig dag.`,'manual',before);toast('Flyttingen er angret');renderAll();
  }
  ...
  function acceptReadinessRecommendation(){
    ...
    const before=snapshot(K.adjustments),all=read(K.adjustments,{}),x=coach.proposed;all[p.sourceLabel]={created:new Date().toISOString(),source:'runnerbear-v10.23',reason:'readiness-v1022',type:x.type,title:x.title,desc:x.desc,detail:x.detail,km:Number(x.km||0),shoe:x.shoe||'',fuel:x.fuel||''};write(K.adjustments,all);const changed=basePlan(p.baseDs);queueGarminMutation('plan:workout-adjusted',[changed],{previousDate:p.ds,reason:'daily-readiness'});saveReadinessState({choice:'accepted'});addLog(`${coach.planned.title} er tilpasset etter Form i dag: ${x.title}. Ingen treningsgjeld.`,'manual',before);trackEvent('daily_readiness_recommendation',{choice:'accepted',severity:coach.severity});toast('Coachens anbefaling er brukt');renderAll();
  }
  function keepPlannedWorkout(){
    const p=planFor(today()),coach=readinessDecision(p);if(!p)return;const before=snapshot(K.adjustments),cleared=clearReadinessAdjustment(p);if(cleared)queueGarminMutation('plan:workout-adjusted',[basePlan(p.baseDs)],{previousDate:p.ds,reason:'keep-planned'});saveReadinessState({choice:'keep'});addLog(`${coach.planned?.title||p.title} beholdes etter brukerens valg, med forsiktighetsråd.`,'manual',before);trackEvent('daily_readiness_recommendation',{choice:'keep',severity:coach.severity});toast('Planlagt økt beholdes med margin');renderAll();
  }
  ...
  function bind(scope=document){
    ...
    qsa('[data-rb1023-garmin-retry]').forEach(b=>b.onclick=async()=>{b.disabled=true;garminSync()?.retry?.(b.dataset.rb1023GarminRetry||'');try{await garminSync()?.flush?.();toast(garminSync()?.available?.()?'Garmin-synk er startet':'Garmin kalenderpublisering venter fortsatt på Training API-tilgang.')}finally{renderAll()}});
  ...
  function init(){
    ...
    if(!engine()||!clarityModel()||!planIntegrity()){
      console.error(JSON.stringify({event:'runnerbear_fatal_init',build:'10.23',reason:'canonical core unavailable'}));
    ...
    migrateDocumentedThreshold();migrateTrainingPreferences();garminSync()?.init?.();runAutopilot();decorateNav();decorateBrand();renderToday();
