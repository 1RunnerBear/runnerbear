/* RunnerBear v10.25 · calm intelligence and evidence within locked Design Direction 1.0
...
(function(){
  ...
  function trackEvent(name,detail={}){
    const payload={event:name,runnerbearVersion:'10.25',...detail};
  ...
  function coachBasis(){
    ...
    return{...status,headline,message,positives:[...new Set(positives)].slice(0,4),watch:[...new Set(watch)].slice(0,3),sources:[...sources]};
  }
  ...
  function todayCoachHtml(base,d,suggest){
    ...
    return `<article class="rb119b-card rb119b-coach-card rb1022-coach-card rb1025-coach-intervention"><header><h2>Coachens vurdering</h2><span class="rb119b-quote">“</span></header><div><span class="rb119b-coach-avatar" aria-hidden="true">RB</span><p><b>${esc(coach.headline)}</b>${esc(coach.message)}</p></div>${recommendation}<footer class="rb1025-provenance"><span>${esc(source)}</span><small>${esc(sync.label)}</small></footer><button type="button" data-rb107-open-why>Hvorfor? ${icon('chevronRight')}</button></article>`;
  }
  ...
  function todayHtml(){
    ...
    return `<div id="rb107Today" class="rb107-surface rb119b rb119b-today"><div class="rb107-shell">${appBarHtml()}${raceMetaHtml()}${viewTitleHtml(terminal?'Dagens resultat':'Dagens økt',terminal?planStatus.label:'')}
      ${workoutHeroHtml(base)}${changeNoticeHtml(base)}${terminal?result:`${pending?pendingResultHtml(base):''}${attention?todayCoachHtml(base,d,suggest):''}`}
  ...
  function replaceRetiredShoeInFuturePlan(retired){
    ...
    for(const p of affected){
      ...
      if(resolved?.replaced)all[p.sourceLabel]={...(all[p.sourceLabel]||{}),created:new Date().toISOString(),source:'runnerbear-v10.25',reason:'retired-shoe',shoe:resolved.shoe,retiredShoe:retired.name};
    }
  ...
  function init(){
    ...
    if(!engine()||!clarityModel()||!planIntegrity()){
      console.error(JSON.stringify({event:'runnerbear_fatal_init',build:'10.25',reason:'canonical core unavailable'}));
