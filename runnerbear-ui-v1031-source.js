/* RunnerBear v10.31.1 · Coach Loop within locked Design Direction 1.0
...
(function(){
  ...
  function trackEvent(name,detail={}){
    const payload={event:name,runnerbearVersion:'10.31.1',...detail};
  ...
  function runAutopilot(){
    ...
    window.RunnerBearCloudV1031?.applySafeAuto?.().catch(error=>console.warn(JSON.stringify({event:'coach_loop_auto_failed',build:'10.31.1',message:error?.message||String(error)})));
  }
  ...
  async function init(){
    ...
    if(!engine()||!clarityModel()||!planIntegrity()||!adaptivePlan()){
      console.error(JSON.stringify({event:'runnerbear_fatal_init',build:'10.31.1',reason:'canonical core unavailable'}));
