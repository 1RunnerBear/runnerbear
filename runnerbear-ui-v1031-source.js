/* RunnerBear v10.31.0 · Coach Loop within locked Design Direction 1.0
...
(function(){
  ...
  function trackEvent(name,detail={}){
    const payload={event:name,runnerbearVersion:'10.31.0',...detail};
  ...
  const coachLoopUi=()=>window.RunnerBearCloudV1031?.snapshot?.()?.flags?.coach_loop_ui===true;
  ...
  function changeNoticeHtml(p,context='today'){
    const canonicalPlan=window.RunnerBearCloudV1031?.snapshot?.()?.activePlan;if(coachLoopUi()&&canonicalPlan?.reasonCode==='safe-autopilot'&&/redusert dose/i.test(p?.title||''))return`<article class="rb107-card rb117-change ${context==='plan'?'compact':''}"><header><span class="rb117-change-mark">${icon('swap')}</span><div><span class="rb107-overline">Coachen har justert</span><h3>${esc(p.title)}</h3></div><strong>Endret</strong></header><details><summary>Hvorfor?</summary><p>En liten, reverserbar doseendring ble gjort innenfor volumtak, låser og Coach Loop-policyen.</p></details><button class="rb117-undo" data-rb1026-undo-plan>Angre endringen</button></article>`;
  ...
  function garminWorkout(p){
    const x=prescription(p);if(!x||!['easy','quality','race'].includes(x.type)||Number(x.km||0)<=0)return null;const target=targetFor(p),canonicalId=String(p.workoutId||'').replace(/[^a-z0-9._-]+/gi,'-').toLowerCase(),stable=canonicalId?`rb-workout-${canonicalId}`:(planIntegrity()?.stableExternalId?.(p)||`rb-workout-${p.baseDs}`),planRevisionId=String(p.planRevisionId||window.RunnerBearCloudV1031?.snapshot?.()?.planRevisionId||'');
  ...
  async function persistCanonicalChange(before,{reason='plan-change',trigger=reason}={}){
    if(window.RunnerBearCloudV1031?.snapshot?.()?.flags?.coach_loop_write!==true)return null;
    try{return await window.RunnerBearCloudV1031.reconfigure({reason,trigger,confirm:false,force:true})}
    catch(error){restoreSnapshot(before);await window.RunnerBearCloudV1031.refresh('full').catch(()=>{});throw error}
  }
  ...
  function queueTredictMutation(type,plans=[],detail={}){
    ...
    if(window.RunnerBearCloudV1031?.snapshot?.()?.flags?.coach_loop_write===true)return mutation;
  ...
  function runAutopilot(){
    ...
    window.RunnerBearCloudV1031?.applySafeAuto?.().catch(error=>console.warn(JSON.stringify({event:'coach_loop_auto_failed',build:'10.31.0',message:error?.message||String(error)})));
  }
  ...
  function saveReadinessState(patch,ds=today()){
    const all=read(K.readiness,{}),current=readinessState(ds);all[ds]={...current,...patch,updatedAt:new Date().toISOString()};write(K.readiness,all);if(coachLoopUi()){const value=all[ds],previous=effectiveSchedule().filter(p=>p.ds<ds&&isDone(p)).sort((a,b)=>a.ds.localeCompare(b.ds)).at(-1),isNextMorning=previous&&dayDiff(ds,previous.ds)===1,payload={sourceId:`morning-response:${ds}:${previous?.workoutId||'general'}`,localDate:isNextMorning?previous.ds:ds,responseDate:ds,responsePhase:isNextMorning?'next_morning':'post_workout',workoutId:previous?.workoutId||'',planRevisionId:window.RunnerBearCloudV1031?.snapshot?.()?.planRevisionId||'',illness:value.reasons.includes('illness'),stress:value.reasons.includes('stress'),poor_sleep:value.reasons.includes('poor_sleep'),pain:value.reasons.includes('achilles')?3:0};window.RunnerBearCloudV1031?.submitFeedback?.(payload).catch(error=>console.warn(JSON.stringify({event:'coach_loop_feedback_failed',build:'10.31.0',message:error?.message||String(error)})))}return all[ds];
  }
  ...
  function readinessDecision(base=planFor(today())){
    const canonical=window.RunnerBearCloudV1031?.toLegacyDecision?.(base);if(canonical)return canonical;
  ...
  function proactiveCoachBrief(){
    const snapshot=window.RunnerBearCloudV1031?.snapshot?.(),brief=snapshot?.coachBrief;
  ...
  function weeklyReview(){const snapshot=window.RunnerBearCloudV1031?.snapshot?.(),review=snapshot?.weeklyReview;return review?.version==='weekly-review-1'&&review.planRevisionId===snapshot?.planRevisionId?review:null}
  ...
  function weeklyReviewCardHtml(){
    ...
    return `<section class="rb1031-week-review ${tone}" aria-labelledby="rb1031WeekReview"><header><div><span class="rb107-overline">Forrige uke</span><h2 id="rb1031WeekReview">${esc(review.headline)}</h2></div><span>${esc(review.dataQuality==='verified'?'Verifisert':'Foreløpig')}</span></header><div><span><small>Løping</small><b>${esc(reviewKm(t.completedDistanceM))}</b><em>av ${esc(reviewKm(t.plannedDistanceM))}</em></span><span><small>Nøkkeløkter</small><b>${Number(t.completedKeySessions||0)}/${Number(t.plannedKeySessions||0)}</b><em>gjennomført/erstattet</em></span><span><small>Tilpasset</small><b>${Number(t.replacedSessions||0)}</b><em>alternativ trening</em></span></div><p>${esc(review.nextDirection)}</p><button type="button" data-rb1031-week-review>Se ukereview ${icon('chevronRight')}</button></section>`;
  }
  function weeklyReviewModalHtml(){
    ...
    return `<div class="rb1020-modal rb1031-review-modal" data-rb1020-modal-backdrop="weekly-review"><section class="rb1020-sheet rb1031-review-sheet" role="dialog" aria-modal="true" aria-labelledby="rb1031ReviewTitle"><header><div><span>Coachens ukereview</span><h2 id="rb1031ReviewTitle">${esc(review.headline)}</h2></div><button type="button" data-rb1020-modal-close="weekly-review" aria-label="Lukk ukereview">${icon('close')}</button></header><div class="rb1020-sheet-body"><section><small>Gjennomført</small><b>${esc(reviewKm(t.completedDistanceM))} løping · ${Number(t.completedQualitySessions||0)} kvalitetsøkter</b><ul>${rows(done)}</ul></section><section><small>Tilpasset</small><b>${Number(t.replacedSessions||0)} erstattet · ${Number(t.missedSessions||0)} tapt</b><ul>${rows([...adapted,...missed])}</ul></section><section><small>Læring</small><p>${esc(review.learning)}</p></section><section><small>Neste retning</small><p>${esc(review.nextDirection)}</p></section></div></section></div>`;
  }
  function syncRepair(){const snapshot=window.RunnerBearCloudV1031?.snapshot?.(),repair=snapshot?.syncRepair;return repair?.version==='sync-repair-1'&&repair.planRevisionId===snapshot?.planRevisionId?repair:null}
  function syncRepairModalHtml(){
    const repair=syncRepair(),primary=repair?.primary;if(!state.syncRepairOpen||!primary)return'';
    return `<div class="rb1020-modal rb1031-sync-modal" data-rb1020-modal-backdrop="sync-repair"><section class="rb1020-sheet rb1031-sync-sheet" role="dialog" aria-modal="true" aria-labelledby="rb1031SyncTitle"><header><div><span>Tredict → Garmin</span><h2 id="rb1031SyncTitle">${esc(primary.title)}</h2></div><button type="button" data-rb1020-modal-close="sync-repair" aria-label="Lukk synkreparasjon">${icon('close')}</button></header><div class="rb1020-sheet-body"><p>${esc(primary.message)}</p><ol><li>RunnerBear-planen er allerede lagret og endres ikke av kontrollen.</li><li>${esc(primary.kind==='structural_review'?'Rett eller fjern den berørte økten i Tredict-kalenderen.':'Publiser eller aktiver den aktive RunnerBear-planen i Tredict.')}</li><li>Trykk «${esc(primary.verifyLabel)}» for å verifisere kalenderen.</li></ol><div class="rb1031-sync-actions"><button class="rb107-button secondary" data-rb108-publish-plan>${icon('sync')} ${esc(primary.actionLabel)}</button><button class="rb107-button" data-rb1031-sync-verify="${esc(primary.operationId)}">${esc(primary.verifyLabel)}</button></div><small>Garmin følger den aktive Tredict-kalenderen. RunnerBear skriver ikke direkte til klokken.</small></div></section></div>`;
  }
  ...
  function todayCoachHtml(base,d,suggest){
    const coach=coachBasis(),reported=coach.reported||readinessState(),stateLabel={fresh:'Frisk',tired:'Litt redusert',heavy:'Klart redusert',unknown:'Ikke meldt'}[reported.state]||'Ikke meldt',reasonLabels={poor_sleep:'dårlig søvn',fatigue:'generell tretthet',heavy_legs:'tunge bein',stress:'stress',illness:'sykdomsfølelse',achilles:'akilles / hælfeste'},reportedReasons=(reported.reasons||[]).map(x=>reasonLabels[x]).filter(Boolean),planned=coach.planned||rawPlanFor(base),proposed=coach.proposed||planned,choice=reported.choice||'pending',canonicalStatus=window.RunnerBearCloudV1031?.decision?.()?.status,accepted=coach.canonical&&['accepted','auto_applied'].includes(canonicalStatus);
  ...
  function todayHtml(){
    ...
    return `<div id="rb107Today" class="rb107-surface rb119b rb119b-today"><div class="rb107-shell">${appBarHtml()}${raceMetaHtml()}${viewTitleHtml(terminal?'Dagens resultat':'Dagens økt',terminal?planStatus.label:'')}
      ${workoutHeroHtml(base)}${todayHealthCardHtml()}${todayCoachHtml(base,d,suggest)}${weeklyPriorityHtml()}${weeklyReviewCardHtml()}${changeNoticeHtml(base)}${terminal?result:`${pending?pendingResultHtml(base):''}`}
      <div class="rb119b-sync ${sync.stale?'stale':''}"><i></i><span>${esc(sync.label)}</span></div>
    </div>${qs('.view.active')?.id==='today'&&state.workoutDetailOpen?workoutDetailModalHtml(planFor(state.workoutDetailDs)||base):''}${qs('.view.active')?.id==='today'&&state.workoutBankOpen?workoutBankModalHtml(planFor(state.workoutDetailDs)||base):''}${qs('.view.active')?.id==='today'&&state.coachReasonOpen?coachReasonModalHtml():''}${weeklyReviewModalHtml()}${syncRepairModalHtml()}</div>`;
  }
  ...
  function canonicalSyncOperations(){return(window.RunnerBearCloudV1031?.snapshot?.()?.sync||[]).filter(row=>row.status!=='superseded')}
  ...
  function finishRender(id,startMark){
    ...
    document.body.classList.toggle('rb109-modal-open',state.goalManagerOpen||state.workoutDetailOpen||state.workoutBankOpen||!!state.movePreview||state.coachReasonOpen||state.intensityExplanationOpen||state.weeklyReviewOpen||state.syncRepairOpen);
    bind($(id));
  ...
  function renderView(id=activeView()){
    if(!engine()||!clarityModel()||!initialRenderAuthorized&&!window.RunnerBearCloudV1031?.snapshot?.()?.flags?.coach_loop_read)return;
  ...
  function acceptReadinessRecommendation(){
    ...
    if(coach.canonical){window.RunnerBearCloudV1031?.resolveDecision?.('accept').then(()=>{toast('Coachens anbefaling er lagret i planen');renderAll()}).catch(error=>toast(error?.message||'Anbefalingen kunne ikke lagres'));return}
  ...
  function bind(scope=document){
    ...
    qsa('[data-rb1028-sync-retry]').forEach(b=>b.onclick=async()=>{b.disabled=true;b.textContent='Kontrollerer…';try{await window.RunnerBearCloudV1031?.retrySync?.();toast('Tredict-status er kontrollert')}catch(error){toast(error?.message||'Tredict-kontrollen feilet')}finally{renderAll()}});
    ...
    qsa('[data-rb1031-sync-verify]').forEach(b=>b.onclick=async()=>{b.disabled=true;b.textContent='Kontrollerer…';try{await window.RunnerBearCloudV1031?.verifySyncRepair?.(b.dataset.rb1031SyncVerify);const repair=syncRepair();if(repair?.attention==='action')toast('Tredict krever fortsatt kontroll');else{state.syncRepairOpen=false;toast('Kalenderen er bekreftet i Tredict')}renderAll()}catch(error){b.disabled=false;toast(error?.message||'Kalenderkontrollen feilet')}});
    ...
    qsa('[data-rb1020-modal-close]').forEach(b=>b.onclick=()=>{const kind=b.dataset.rb1020ModalClose;if(kind==='workout')state.workoutDetailOpen=false;if(kind==='bank')closeWorkoutBank();if(kind==='move')state.movePreview=null;if(kind==='coach')state.coachReasonOpen=false;if(kind==='intensity')state.intensityExplanationOpen=false;if(kind==='goal-insight')state.goalInsightOpen='';if(kind==='weekly-review')state.weeklyReviewOpen=false;if(kind==='sync-repair')state.syncRepairOpen=false;renderAll()});
    qsa('[data-rb1020-modal-backdrop]').forEach(el=>el.onclick=e=>{if(e.target!==el)return;const kind=el.dataset.rb1020ModalBackdrop;if(kind==='workout')state.workoutDetailOpen=false;if(kind==='bank')closeWorkoutBank();if(kind==='move')state.movePreview=null;if(kind==='coach')state.coachReasonOpen=false;if(kind==='intensity')state.intensityExplanationOpen=false;if(kind==='goal-insight')state.goalInsightOpen='';if(kind==='weekly-review')state.weeklyReviewOpen=false;if(kind==='sync-repair')state.syncRepairOpen=false;renderAll()});
    ...
    qsa('[data-rb1026-undo-plan]').forEach(b=>b.onclick=async()=>{b.disabled=true;try{await window.RunnerBearCloudV1031?.undoPlan?.();toast('Coachendringen er angret');renderAll()}catch(error){b.disabled=false;toast(error?.message||'Endringen kunne ikke angres')}});
  ...
  function waitForCanonicalRuntime(){if(window.RunnerBearCloudV1031)return Promise.resolve(window.RunnerBearCloudV1031);return new Promise((resolve,reject)=>{const timer=setTimeout(()=>{window.removeEventListener('runnerbear:canonical-runtime-ready',ready);reject(new Error('Den kanoniske planmotoren ble ikke lastet.'))},8000),ready=()=>{clearTimeout(timer);resolve(window.RunnerBearCloudV1031)};window.addEventListener('runnerbear:canonical-runtime-ready',ready,{once:true})})}
  async function init(){
    mark('runnerbear:init:start');
    if(!engine()||!clarityModel()||!planIntegrity()||!adaptivePlan()){
      console.error(JSON.stringify({event:'runnerbear_fatal_init',build:'10.31.0',reason:'canonical core unavailable'}));
    ...
    document.addEventListener('keydown',e=>{if(e.key!=='Escape')return;if(state.workoutBankOpen){closeWorkoutBank();renderAll();return}if(state.goalManagerOpen||state.goalInsightOpen||state.workoutDetailOpen||state.movePreview||state.coachReasonOpen||state.intensityExplanationOpen||state.weeklyReviewOpen||state.syncRepairOpen){state.goalManagerOpen=false;state.goalEditor='';state.goalInsightOpen='';state.workoutDetailOpen=false;state.movePreview=null;state.coachReasonOpen=false;state.intensityExplanationOpen=false;state.weeklyReviewOpen=false;state.syncRepairOpen=false;renderAll()}});
    let storageFrame=0;
