/* RunnerBear v10.27.0 · Coach Loop within locked Design Direction 1.0
...
(function(){
  ...
  function trackEvent(name,detail={}){
    const payload={event:name,runnerbearVersion:'10.27.0',...detail};
  ...
  const coachLoopUi=()=>window.RunnerBearCloudV1027?.snapshot?.()?.flags?.coach_loop_ui===true;
  ...
  function effectiveSchedule(){
    ...
    const schedule=applyGoalRaces([...rows,...activityOnly]).sort((a,b)=>a.ds.localeCompare(b.ds)||String(a.workoutId||'').localeCompare(String(b.workoutId||'')));
  ...
  function changeNoticeHtml(p,context='today'){
    const canonicalPlan=window.RunnerBearCloudV1027?.snapshot?.()?.activePlan;if(coachLoopUi()&&canonicalPlan?.reasonCode==='safe-autopilot'&&/redusert dose/i.test(p?.title||''))return`<article class="rb107-card rb117-change ${context==='plan'?'compact':''}"><header><span class="rb117-change-mark">${icon('swap')}</span><div><span class="rb107-overline">Coachen har justert</span><h3>${esc(p.title)}</h3></div><strong>Endret</strong></header><details><summary>Hvorfor?</summary><p>En liten, reverserbar doseendring ble gjort innenfor volumtak, låser og Coach Loop-policyen.</p></details><button class="rb117-undo" data-rb1026-undo-plan>Angre endringen</button></article>`;
  ...
  function garminWorkout(p){
    const x=prescription(p);if(!x||!['easy','quality','race'].includes(x.type)||Number(x.km||0)<=0)return null;const target=targetFor(p),canonicalId=String(p.workoutId||'').replace(/[^a-z0-9._-]+/gi,'-').toLowerCase(),stable=canonicalId?`rb-workout-${canonicalId}`:(planIntegrity()?.stableExternalId?.(p)||`rb-workout-${p.baseDs}`),planRevisionId=String(p.planRevisionId||window.RunnerBearCloudV1027?.snapshot?.()?.planRevisionId||'');
  ...
  function queueTredictMutation(type,plans=[],detail={}){
    ...
    if(window.RunnerBearCloudV1027?.snapshot?.()?.flags?.coach_loop_write===true)return mutation;
  ...
  function runAutopilot(){
    ...
    window.RunnerBearCloudV1027?.applySafeAuto?.().catch(error=>console.warn(JSON.stringify({event:'coach_loop_auto_failed',build:'10.27.0',message:error?.message||String(error)})));
  }
  ...
  function saveReadinessState(patch,ds=today()){
    const all=read(K.readiness,{}),current=readinessState(ds);all[ds]={...current,...patch,updatedAt:new Date().toISOString()};write(K.readiness,all);if(coachLoopUi()){const value=all[ds],previous=effectiveSchedule().filter(p=>p.ds<ds&&isDone(p)).sort((a,b)=>a.ds.localeCompare(b.ds)).at(-1),isNextMorning=previous&&dayDiff(ds,previous.ds)===1,payload={sourceId:`morning-response:${ds}:${previous?.workoutId||'general'}`,localDate:isNextMorning?previous.ds:ds,responseDate:ds,responsePhase:isNextMorning?'next_morning':'post_workout',workoutId:previous?.workoutId||'',planRevisionId:window.RunnerBearCloudV1027?.snapshot?.()?.planRevisionId||'',illness:value.reasons.includes('illness'),stress:value.reasons.includes('stress'),poor_sleep:value.reasons.includes('poor_sleep'),pain:value.reasons.includes('achilles')?3:0};window.RunnerBearCloudV1027?.submitFeedback?.(payload).catch(error=>console.warn(JSON.stringify({event:'coach_loop_feedback_failed',build:'10.27.0',message:error?.message||String(error)})))}return all[ds];
  }
  ...
  function readinessDecision(base=planFor(today())){
    const canonical=window.RunnerBearCloudV1027?.toLegacyDecision?.(base);if(canonical)return canonical;
  ...
  function todayCoachHtml(base,d,suggest){
    const coach=coachBasis(),reported=coach.reported||readinessState(),stateLabel={fresh:'Frisk',tired:'Litt redusert',heavy:'Klart redusert',unknown:'Ikke meldt'}[reported.state]||'Ikke meldt',reasonLabels={poor_sleep:'dårlig søvn',fatigue:'generell tretthet',heavy_legs:'tunge bein',stress:'stress',illness:'sykdomsfølelse',achilles:'akilles / hælfeste'},reportedReasons=(reported.reasons||[]).map(x=>reasonLabels[x]).filter(Boolean),planned=coach.planned||rawPlanFor(base),proposed=coach.proposed||planned,choice=reported.choice||'pending',canonicalStatus=window.RunnerBearCloudV1027?.decision?.()?.status,accepted=coach.canonical&&['accepted','auto_applied'].includes(canonicalStatus);
  ...
  async function saveTrainingPreferences(form){
    ...
    if(runDays.length<3)return toast('Velg minst tre løpedager');if(!qualityDays.length||qualityDays.some(x=>!runDays.includes(x)))return toast('Kvalitetsdager må være valgte løpedager');if(!runDays.includes(longRunDay))return toast('Langturdagen må være en valgt løpedag');if(alternativeDays.some(x=>runDays.includes(x)))return toast('Alternative dager kan ikke samtidig være vanlige løpedager');if(!(normalLow<=baseKm&&baseKm<=normalHigh&&normalHigh<=maxKm))return toast('Volum må følge: coach fra ≤ normal ≤ coach til ≤ øvre grense');
    const button=form.querySelector('button[type="submit"]'),previousRows=planRowsSnapshot(),previousProfile=localStorage.getItem(K.profile),previousLegacy=localStorage.getItem(K.legacyProfile),before=snapshot(K.profile),stored=read(K.profile,{}),nextPreferences={baseKm,normalLow,normalHigh,maxKm,targetWeeklyVolume:baseKm,minRunDays:runDays.length,flexibleSessions:Math.min(2,alternativeDays.length),runDays,qualityDays,longRunDay,alternativeDays,preferencesConfigured:true,preferencesVersion:3};
  ...
  function acceptReadinessRecommendation(){
    ...
    if(coach.canonical){window.RunnerBearCloudV1027?.resolveDecision?.('accept').then(()=>{toast('Coachens anbefaling er lagret i planen');renderAll()}).catch(error=>toast(error?.message||'Anbefalingen kunne ikke lagres'));return}
  ...
  function bind(scope=document){
    ...
    qsa('[data-rb1026-undo-plan]').forEach(b=>b.onclick=async()=>{b.disabled=true;try{await window.RunnerBearCloudV1027?.undoPlan?.();toast('Coachendringen er angret');renderAll()}catch(error){b.disabled=false;toast(error?.message||'Endringen kunne ikke angres')}});
  ...
  function init(){
    ...
    if(!engine()||!clarityModel()||!planIntegrity()||!adaptivePlan()){
      console.error(JSON.stringify({event:'runnerbear_fatal_init',build:'10.27.0',reason:'canonical core unavailable'}));
