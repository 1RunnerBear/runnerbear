/* RunnerBear v10.30.0 · Coach Loop within locked Design Direction 1.0
...
(function(){
  ...
  function trackEvent(name,detail={}){
    const payload={event:name,runnerbearVersion:'10.30.0',...detail};
  ...
  function runAutopilot(){
    ...
    window.RunnerBearCloudV1027?.applySafeAuto?.().catch(error=>console.warn(JSON.stringify({event:'coach_loop_auto_failed',build:'10.30.0',message:error?.message||String(error)})));
  }
  ...
  function proactiveCoachBrief(){
    const snapshot=window.RunnerBearCloudV1027?.snapshot?.(),brief=snapshot?.coachBrief;
  ...
  function todayHtml(){
    ...
    return `<div id="rb107Today" class="rb107-surface rb119b rb119b-today"><div class="rb107-shell">${appBarHtml()}${raceMetaHtml()}${viewTitleHtml(terminal?'Dagens resultat':'Dagens økt',terminal?planStatus.label:'')}
      ${workoutHeroHtml(base)}${todayHealthCardHtml()}${todayCoachHtml(base,d,suggest)}${weeklyPriorityHtml()}${changeNoticeHtml(base)}${terminal?result:`${pending?pendingResultHtml(base):''}`}
  ...
  function bind(scope=document){
    ...
    qsa('[data-rb1020-modal-close]').forEach(b=>b.onclick=()=>{const kind=b.dataset.rb1020ModalClose;if(kind==='workout')state.workoutDetailOpen=false;if(kind==='bank')closeWorkoutBank();if(kind==='move')state.movePreview=null;if(kind==='coach')state.coachReasonOpen=false;if(kind==='intensity')state.intensityExplanationOpen=false;if(kind==='goal-insight')state.goalInsightOpen='';renderAll()});
    qsa('[data-rb1020-modal-backdrop]').forEach(el=>el.onclick=e=>{if(e.target!==el)return;const kind=el.dataset.rb1020ModalBackdrop;if(kind==='workout')state.workoutDetailOpen=false;if(kind==='bank')closeWorkoutBank();if(kind==='move')state.movePreview=null;if(kind==='coach')state.coachReasonOpen=false;if(kind==='intensity')state.intensityExplanationOpen=false;if(kind==='goal-insight')state.goalInsightOpen='';renderAll()});
  ...
  async function init(){
    ...
    if(!engine()||!clarityModel()||!planIntegrity()||!adaptivePlan()){
      console.error(JSON.stringify({event:'runnerbear_fatal_init',build:'10.30.0',reason:'canonical core unavailable'}));
    ...
    document.addEventListener('keydown',e=>{if(e.key!=='Escape')return;if(state.workoutBankOpen){closeWorkoutBank();renderAll();return}if(state.goalManagerOpen||state.goalInsightOpen||state.workoutDetailOpen||state.movePreview||state.coachReasonOpen||state.intensityExplanationOpen){state.goalManagerOpen=false;state.goalEditor='';state.goalInsightOpen='';state.workoutDetailOpen=false;state.movePreview=null;state.coachReasonOpen=false;state.intensityExplanationOpen=false;renderAll()}});
    let storageFrame=0;
