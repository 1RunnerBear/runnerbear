/* RunnerBear v10.28.0 · Coach Loop within locked Design Direction 1.0
...
(function(){
  ...
  function trackEvent(name,detail={}){
    const payload={event:name,runnerbearVersion:'10.28.0',...detail};
  ...
  async function persistCanonicalChange(before,{reason='plan-change',trigger=reason}={}){
    if(window.RunnerBearCloudV1027?.snapshot?.()?.flags?.coach_loop_write!==true)return null;
    try{return await window.RunnerBearCloudV1027.reconfigure({reason,trigger,confirm:false,force:true})}
    catch(error){restoreSnapshot(before);await window.RunnerBearCloudV1027.refresh('full').catch(()=>{});throw error}
  }
  ...
  function runAutopilot(){
    ...
    window.RunnerBearCloudV1027?.applySafeAuto?.().catch(error=>console.warn(JSON.stringify({event:'coach_loop_auto_failed',build:'10.28.0',message:error?.message||String(error)})));
  }
  ...
  const state={selectedDs:sessionStorage.getItem(K.selected)||'',planView:savedPlanView==='done'?'done':'plan',planLens:['week','focus','long'].includes(savedPlanLens)?savedPlanLens:'week',openWeek:null,monthKey:'',planDayViewOpen:false,planDetailOpen:false,moveOpen:false,movePreview:null,moveSaving:false,adaptOpen:false,completedId:'',doneScroll:0,goalManagerOpen:false,goalEditor:'',workoutDetailOpen:false,workoutDetailDs:'',workoutBankOpen:false,workoutBankReturnToDetail:false,coachReasonOpen:false,intensityExplanationOpen:false};
  ...
  function canonicalSyncOperations(){return(window.RunnerBearCloudV1027?.snapshot?.()?.sync||[]).filter(row=>row.status!=='superseded')}
  ...
  function canonicalSyncBannerHtml(){if(!coachLoopUi())return'';const sync=canonicalSyncSummary();return`<section class="rb1028-sync-banner ${esc(sync.tone)}" aria-live="polite"><i aria-hidden="true"></i><span><b>${esc(sync.label)}</b><small>${esc(sync.copy)}</small></span>${sync.action?'<button type="button" data-rb1028-sync-retry>Prøv igjen</button>':''}</section>`}
  ...
  function renderView(id=activeView()){
    if(!engine()||!clarityModel()||!initialRenderAuthorized&&!window.RunnerBearCloudV1027?.snapshot?.()?.flags?.coach_loop_read)return;
  ...
  function bind(scope=document){
    ...
    qsa('[data-rb1028-sync-retry]').forEach(b=>b.onclick=async()=>{b.disabled=true;b.textContent='Kontrollerer…';try{await window.RunnerBearCloudV1027?.retrySync?.();toast('Tredict-status er kontrollert')}catch(error){toast(error?.message||'Tredict-kontrollen feilet')}finally{renderAll()}});
    ...
    qsa('[data-rb1020-modal-close]').forEach(b=>b.onclick=()=>{const kind=b.dataset.rb1020ModalClose;if(kind==='workout')state.workoutDetailOpen=false;if(kind==='bank')closeWorkoutBank();if(kind==='move')state.movePreview=null;if(kind==='coach')state.coachReasonOpen=false;if(kind==='intensity')state.intensityExplanationOpen=false;renderAll()});
    qsa('[data-rb1020-modal-backdrop]').forEach(el=>el.onclick=e=>{if(e.target!==el)return;const kind=el.dataset.rb1020ModalBackdrop;if(kind==='workout')state.workoutDetailOpen=false;if(kind==='bank')closeWorkoutBank();if(kind==='move')state.movePreview=null;if(kind==='coach')state.coachReasonOpen=false;if(kind==='intensity')state.intensityExplanationOpen=false;renderAll()});
  ...
  function waitForCanonicalRuntime(){if(window.RunnerBearCloudV1027)return Promise.resolve(window.RunnerBearCloudV1027);return new Promise((resolve,reject)=>{const timer=setTimeout(()=>{window.removeEventListener('runnerbear:canonical-runtime-ready',ready);reject(new Error('Den kanoniske planmotoren ble ikke lastet.'))},8000),ready=()=>{clearTimeout(timer);resolve(window.RunnerBearCloudV1027)};window.addEventListener('runnerbear:canonical-runtime-ready',ready,{once:true})})}
  async function init(){
    mark('runnerbear:init:start');
    if(!engine()||!clarityModel()||!planIntegrity()||!adaptivePlan()){
      console.error(JSON.stringify({event:'runnerbear_fatal_init',build:'10.28.0',reason:'canonical core unavailable'}));
    ...
    migrateDocumentedThreshold();migrateTrainingPreferences();tredictSync()?.init?.();decorateNav();decorateBrand();
    try{const canonical=await waitForCanonicalRuntime(),boot=await canonical.start();if(boot?.flags?.coach_loop_read===true&&!canonical.snapshot?.())throw new Error('Gjeldende plan kunne ikke verifiseres.');initialRenderAuthorized=true}catch(error){console.error(JSON.stringify({event:'runnerbear_canonical_first_paint_failed',build:'10.28.0',message:error?.message||String(error)}));const boot=qs('#rb108Boot div');if(boot)boot.innerHTML='<b>Planen kunne ikke verifiseres</b><span>Last siden på nytt. RunnerBear viser ikke en eldre plan som om den var gjeldende.</span>';return}
    ...
    document.addEventListener('keydown',e=>{if(e.key!=='Escape')return;if(state.workoutBankOpen){closeWorkoutBank();renderAll();return}if(state.goalManagerOpen||state.workoutDetailOpen||state.movePreview||state.coachReasonOpen||state.intensityExplanationOpen){state.goalManagerOpen=false;state.goalEditor='';state.workoutDetailOpen=false;state.movePreview=null;state.coachReasonOpen=false;state.intensityExplanationOpen=false;renderAll()}});
    let storageFrame=0;
