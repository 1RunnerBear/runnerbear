/* RunnerBear v10.31.1 · Tredict Transport Bridge · Cloudflare Worker
...
const toolFor=(tools,kind)=>{const patterns={create:[/planned.*(?:create|add)/,/create.*planned/,/calendar.*create/,/training.*create/],update:[/planned.*(?:update|edit)/,/(?:update|edit).*planned/,/calendar.*update/],metadataUpdate:[/^activity-update$/],delete:[/planned.*(?:delete|remove)/,/(?:delete|remove).*planned/,/calendar.*delete/],planApply:[/plan.*apply/,/apply.*plan/]}[kind]||[];return(tools||[]).find(tool=>normalizedToolName(tool?.name)!=='plan-creation'&&patterns.some(pattern=>pattern.test(normalizedToolName(tool?.name))))||null};
async function mcpTools(env){
  if(capabilityCache&&capabilityCache.expiresAt>Date.now())return capabilityCache;
  const initialized=await mcpPost(env,{jsonrpc:'2.0',id:'rb-init',method:'initialize',params:{protocolVersion:'2025-06-18',capabilities:{},clientInfo:{name:'RunnerBear',version:'12.0.0'}}}),sessionId=initialized.sessionId;await mcpPost(env,{jsonrpc:'2.0',method:'notifications/initialized'},sessionId);
  const listed=await mcpPost(env,{jsonrpc:'2.0',id:'rb-tools',method:'tools/list',params:{}},sessionId),tools=listed.envelope?.result?.tools||[],selected={create:toolFor(tools,'create'),update:toolFor(tools,'update'),metadataUpdate:toolFor(tools,'metadataUpdate'),delete:toolFor(tools,'delete'),planApply:toolFor(tools,'planApply')};capabilityCache={sessionId,tools,selected,expiresAt:Date.now()+5*60000};return capabilityCache;
}
function toolArguments(tool,operation={},row={}){const properties=tool?.inputSchema?.properties||{},id=remoteId(row)||String(operation.remoteWorkoutId||''),date=`${isoDate(operation.date)}T15:00:00.000Z`,structuredWorkout=operation.structuredWorkout||{title:operation.title,notes:operation.notes},plannedTraining={id,trainingId:id,date,title:operation.title,notes:structuredWorkout.notes,structuredWorkout,sportType:'running'},args={};for(const key of Object.keys(properties)){const name=key.toLowerCase();if(/^(id|activityid|trainingid|plannedtrainingid|workoutid)$/.test(name))args[key]=id;else if(/date|starttime|scheduled/.test(name))args[key]=date;else if(name==='structuredworkout')args[key]=structuredWorkout;else if(/plannedtraining|training|workout/.test(name))args[key]=plannedTraining;else if(name==='title'||name==='name')args[key]=operation.title;else if(/note|description/.test(name))args[key]=structuredWorkout.notes;else if(/sport/.test(name))args[key]='running';else if(name==='externalid')args[key]=operation.externalId}return args}
async function callCalendarTool(env,tool,operation,row={}){if(!tool)throw new Error('TREDICT_CAPABILITY_UNAVAILABLE');const discovered=await mcpTools(env),called=await mcpPost(env,{jsonrpc:'2.0',id:`rb-${Date.now()}`,method:'tools/call',params:{name:tool.name,arguments:toolArguments(tool,operation,row)}},discovered.sessionId),envelope=called.envelope;if(envelope?.error||envelope?.result?.isError)throw new Error(`Tredict MCP ${tool.name} failed · ${String(envelope?.error?.message||envelope?.result?.content?.[0]?.text||'unknown error').slice(0,300)}`);const result=envelope?.result||{},text=result?.content?.find?.(part=>part?.type==='text')?.text||'';let parsed={};try{parsed=JSON.parse(text)}catch{}return result?.structuredContent||parsed||result}
class TredictCalendarProvider{
  constructor(env){this.env=env}
  async discoverCapabilities(){const found=await mcpTools(this.env),capabilities={supportsMove:true,supportsCreate:!!found.selected.create,supportsUpdate:!!found.selected.update,supportsDelete:!!found.selected.delete,supportsReplace:!!found.selected.update||!!found.selected.create&&!!found.selected.delete,supportsOwnedRelink:!!found.selected.metadataUpdate,supportsPlanApply:!!found.selected.planApply,discoveredAt:new Date().toISOString(),tools:Object.values(found.selected).filter(Boolean).map(tool=>tool.name)};console.log(JSON.stringify({event:'tredict.capabilities',provider:'tredict',result:capabilities}));return capabilities}
...
async function createPlanViaMcp(env,payload){
  const initialized=await mcpPost(env,{jsonrpc:'2.0',id:'rb-init',method:'initialize',params:{protocolVersion:'2025-06-18',capabilities:{},clientInfo:{name:'RunnerBear',version:'12.0.0'}}});
...
async function buildSnapshot(env,requestedDays=365){
  ...
  return{
    ok:true,version:'10.31.1',syncedAt:new Date().toISOString(),windowDays:days,
