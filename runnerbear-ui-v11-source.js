/* RunnerBear v12.1 · One Truth · One Decision within locked Concept 1
...
(function(){
  'use strict';
  const RELEASE=window.RunnerBearRelease||{build:'12.1.0',label:'One Truth · One Decision'},BUILD=RELEASE.build;
  ...
  function todayHtml(){
    ...
    const support=calm()?.chooseSupport({transport:canonicalSyncBannerHtml(),change:changeSupportHtml(base),review:weeklyReviewCardHtml(),priority:weeklyPriorityHtml()})||'';
    ...
    </div>${qs('.view.active')?.id==='today'&&state.workoutDetailOpen?workoutDetailModalHtml(planFor(state.workoutDetailDs)||base):''}${qs('.view.active')?.id==='today'&&state.workoutBankOpen?workoutBankModalHtml(planFor(state.workoutDetailDs)||base):''}${qs('.view.active')?.id==='today'&&state.coachReasonOpen?coachReasonModalHtml():''}${bodyResponseDetailModalHtml()}${oneDecisionProposalModalHtml()}${weeklyReviewModalHtml()}${syncRepairModalHtml()}</div>`;
  }
  ...
  function finishRender(id,startMark){
    ...
    document.body.classList.toggle('rb109-modal-open',state.goalManagerOpen||state.workoutDetailOpen||state.workoutBankOpen||!!state.movePreview||state.coachReasonOpen||state.bodyResponseOpen||state.oneDecisionProposalOpen||state.intensityExplanationOpen||state.weeklyReviewOpen||state.syncRepairOpen);
    bind($(id));
