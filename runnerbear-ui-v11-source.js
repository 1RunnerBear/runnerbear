/* RunnerBear v11.3.0 · One Decision Coach Experience within locked Design Direction 1.0
...
(function(){
  ...
  function trackEvent(name,detail={}){
    const payload={event:name,runnerbearVersion:'11.3.0',...detail};
  ...
  function runAutopilot(){
    ...
    window.RunnerBearCloudV11?.applySafeAuto?.().catch(error=>console.warn(JSON.stringify({event:'coach_loop_auto_failed',build:'11.3.0',message:error?.message||String(error)})));
  }
  ...
  function oneDecision(){const snapshot=window.RunnerBearCloudV11?.snapshot?.(),value=snapshot?.oneDecision;return value?.version==='one-decision-1'&&value.planRevisionId===snapshot?.planRevisionId&&value.safety?.planWritesByAi===false?value:null}
  ...
  async function runOneDecisionAction(kind){
    const current=oneDecision(),base=planFor(today())||effectiveSchedule().find(p=>p.ds>=today());if(!current)return;
    ...
    if(kind==='open_workout'||kind==='view_result'){if(!base)return;state.workoutDetailDs=base.ds;state.workoutDetailOpen=true;renderAll();requestAnimationFrame(()=>qs('.rb1020-workout-sheet [data-rb1020-modal-close]')?.focus());return}
  ...
  async function init(){
    ...
    if(!engine()||!clarityModel()||!planIntegrity()||!adaptivePlan()){
      console.error(JSON.stringify({event:'runnerbear_fatal_init',build:'11.3.0',reason:'canonical core unavailable'}));
