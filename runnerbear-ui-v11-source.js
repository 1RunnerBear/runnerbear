/* RunnerBear v12.0 · One Truth · One Decision within locked Concept 1
...
(function(){
  'use strict';
  const RELEASE=window.RunnerBearRelease||{build:'12.0.0',label:'One Truth · One Decision'},BUILD=RELEASE.build;
  ...
  const state={selectedDs:sessionStorage.getItem(K.selected)||'',planView:savedPlanView==='done'?'done':'plan',planLens:['week','focus','long'].includes(savedPlanLens)?savedPlanLens:'week',openWeek:null,monthKey:'',monthOpen:false,planDayViewOpen:false,planDetailOpen:false,moveOpen:false,movePreview:null,moveSaving:false,adaptOpen:false,completedId:'',doneScroll:0,goalManagerOpen:false,goalEditor:'',goalInsightOpen:'',workoutDetailOpen:false,workoutDetailDs:'',workoutBankOpen:false,workoutBankReturnToDetail:false,coachReasonOpen:false,bodyResponseOpen:false,oneDecisionProposalOpen:false,oneDecisionResolving:false,intensityExplanationOpen:false,weeklyReviewOpen:false,syncRepairOpen:false};
  ...
  function oneDecisionHeroHtml(base){
    ...
    return `<section class="rb113-one-decision rb116-contextual ${esc(current.tone||'calm')} ${esc(current.state)}" aria-labelledby="rb113DecisionTitle" aria-live="polite"><header><span class="rb113-decision-status"><i aria-hidden="true"></i>${esc(status)}</span><small><b>${esc(confidence?.label||'Kontrollert beslutning')}</b>${esc(freshness)}</small></header><div class="rb113-decision-copy"><span class="rb107-overline">Coachens råd</span><h2 id="rb113DecisionTitle">${esc(headline)}</h2><p>${esc(summary)}</p></div>${base?`<button type="button" class="rb113-workout-context" data-rb1020-workout-open="${esc(base.ds)}"><span>${icon(workoutIconName(base))}</span><span><small>${esc(formatDate(base.ds,{weekday:'long',day:'numeric',month:'long'}))}</small><b>${esc(workout.title||base.title)}</b><em>${esc(target.main||oneDecisionDose(current.workout||{}))}</em></span>${icon('chevronRight')}</button>`:''}${evidence.length?`<ul class="rb113-evidence" aria-label="Beslutningsgrunnlag">${evidence.map(row=>`<li class="${esc(row.tone||'neutral')}"><i aria-hidden="true"></i><span><b>${esc(row.label)}</b><small>${esc(row.source)}</small></span></li>`).join('')}</ul>`:''}${memoryHtml}<div class="rb113-decision-actions"><button type="button" class="rb113-primary" data-rb113-decision-action="${esc(primaryAction?.kind||'view_plan')}" ${state.oneDecisionResolving?'disabled':''}>${state.oneDecisionResolving?'Lagrer …':esc(primaryAction?.label||'Se planen')}${state.oneDecisionResolving?'':icon('arrowRight')}</button>${hasBasis?`<button type="button" class="rb116-secondary-link" data-rb111-body-open>${icon('info')} Se helsegrunnlaget</button>`:''}</div></section>`;
  }
  ...
  function weeklyPriorityHtml(){
    const brief=proactiveCoachBrief(),week=brief.week||{},next=week.nextKeyWorkout,nextCopy=next?`${formatDate(next.localDate,{weekday:'long'})}: ${next.title}`:nextReviewCopy(brief),presentation=window.RunnerBearV12Decision?.presentation?.({decision:oneDecision(),body:bodyResponse(),brief})||{},tone=presentation.weekTone||(['normal','watch'].includes(brief.attention)?brief.attention:'normal'),status=presentation.weekStatus||(tone==='watch'?'Følg med':'Planen står');
    return `<section class="rb1030-week-priority ${tone}" aria-labelledby="rb1030WeekPriority"><header><div><span class="rb107-overline">Denne ukens prioritet</span><h2 id="rb1030WeekPriority">${esc(week.priority||'Bevar kontinuiteten')}</h2></div><span class="rb1030-status">${esc(status)}</span></header><p>${esc(week.reason||'Kontrollert arbeid over tid er hovedprioriteten.')}</p><div><span><small>Neste nøkkelpunkt</small><b>${esc(nextCopy)}</b></span><span><small>Hva kan endres</small><b>${esc(week.nextChange||'Resten av uken står.')}</b></span></div><button type="button" data-rb107-open-why>Se hvorfor og neste vurdering ${icon('chevronRight')}</button></section>`;
  }
  ...
  function todayHtml(){
    const base=planFor(today())||effectiveSchedule().find(p=>p.ds>=today())||effectiveSchedule().at(-1),d=decision(),sync=syncState(),planStatus=sessionState(base),terminal=planStatus.terminal,coach=coachBasis(),suggest=coach.requiresChoice&&coach.reported?.choice==='pending',pending=!terminal&&manualCandidates(base).length>0,result=planStatus.code==='completed'?resultCardHtml(base,'today'):planStatus.code==='replaced'?replacementCardHtml(base,'today'):terminalStatusCardHtml(base),decisionSurface=oneDecision()?`${oneDecisionHeroHtml(base)}${contextualHealthStripHtml()}`:`${workoutHeroHtml(base)}${todayHealthCardHtml()}${todayCoachHtml(base,d,suggest)}`;
  ...
  function canonicalSyncBannerHtml(){if(!coachLoopUi())return'';const sync=canonicalSyncSummary();return`<section class="rb1028-sync-banner ${esc(sync.tone)}" aria-live="polite"><i aria-hidden="true"></i><span><b>${esc(sync.label)}</b><small>${esc(sync.copy)}</small></span></section>`}
  ...
  function goalPremiumSummaryHtml(goal,f,pred){
    const gates=goalGates(),next=gates.next,five=Number(pred?.five?.seconds||0),direction=f.progress.code==='green'?'God':f.progress.code==='amber'?'Krever fremgang':'Bygges',target=goal?.targetSeconds||f.current,context=contextualCoach()?.surfaces?.goal;
    const items=[
  ...
  function moreHtml(){
    ...
    const shoes=shoesState(),activeShoes=shoes.filter(x=>x.active!==false).length,canonicalTransport=canonicalSyncSummary(),transportLabel=canonicalTransport.label,transportCopy=canonicalTransport.copy;
