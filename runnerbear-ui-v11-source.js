...
(function(){
  'use strict';
  const RELEASE=window.RunnerBearRelease||{build:'12.2.0',label:'One Truth · One Decision'},BUILD=RELEASE.build;
  ...
  function canonicalSyncBannerHtml(){if(!coachLoopUi())return'';const sync=canonicalSyncSummary();if(!calm()?.exceptionalSync(sync.tone))return'';return`<section class="rb1028-sync-banner ${esc(sync.tone)}" aria-live="polite"><i aria-hidden="true"></i><span><b>${esc(sync.label)}</b><small>${esc(sync.copy)}</small></span></section>`}
