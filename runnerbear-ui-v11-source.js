/* RunnerBear v12.2 · Race Focus within locked Concept 1
...
(function(){
  'use strict';
  const RELEASE=window.RunnerBearRelease||{build:'12.6.0',label:'Concept 1 A · Personlig oppfølging'},BUILD=RELEASE.build;
  ...
  const K={
    ...
    log:'runnerbear_v107_coach_log',seen:'runnerbear_v107_seen_actions',planView:'runnerbear_v107_plan_view',planLens:'runnerbear_v1019b_plan_lens',selected:'runnerbear_v108_selected_day',exclusions:'runnerbear_v108_match_exclusions',goals:'runnerbear_v109_goals',
  ...
  const displayCopy=s=>String(s||'').replace(/\bGATE\s*(\d+)/gi,'Sjekkpunkt $1').replace(/RACE WEEK/g,'Løpsuke').replace(/Taper/g,'Nedtrapping').replace(/VJ på[^.]*Premium på asfalt\./g,'').trim();
  ...
  function trackEvent(name,detail={}){
    const payload={event:name,runnerbearVersion:BUILD,...detail};
  ...
  const control=()=>localStorage.getItem(K.control)||'suggest';
  const calm=()=>window.RunnerBearCalmFlow;
  const raceFocus=()=>window.RunnerBearRaceFocus;
  ...
  const HERO_BANK={
    tempo:{src:'runnerbear-hero-tempo-v1019c.webp',position:'65% 50%'},
    intervals:{src:'runnerbear-hero-intervals-v1019c.webp',position:'80% 50%'},
    race:{src:'runnerbear-hero-race-v1019c.webp',position:'68% 48%'},
    urban:{src:'runnerbear-hero-urban-v1019c.webp',position:'65% 50%'},
    recovery:{src:'runnerbear-hero-recovery-v1019c.webp',position:'62% 54%'},
    strength:{src:'runnerbear-hero-strength-v1019c.webp',position:'72% 48%'}
  };
  ...
  const heroStyle=name=>`style="--hero-position:${(HERO_BANK[name]||HERO_BANK.urban).position}" data-hero="${esc(name)}"`;
  const heroImage=(name,primary=false)=>`<img class="rb125-hero-image" src="${(HERO_BANK[name]||HERO_BANK.urban).src}" width="1536" height="1024" alt="" loading="${primary?'eager':'lazy'}" fetchpriority="${primary?'high':'auto'}" decoding="async">`;
  ...
  function appBarHtml(){return `<header class="rb119b-appbar rb1025-appbar"><div class="rb119b-wordmark">RunnerBear</div><button type="button" class="rb126-profile" data-rb126-more aria-label="Åpne profil og innstillinger">${icon('profile')}</button></header>`}
  ...
  function canonicalRows(){
    ...
    return snapshot.activePlan.items.map(item=>{
      ...
      return{...raw,...legacy,...item,...adjustment,baseDs,sourceLabel,ds,date:dateFrom(ds),type:adjustment.type||item.workoutType||raw.type||'easy',workoutType:adjustment.type||item.workoutType,km:adjustment.km??Number(item.plannedDistanceM||0)/1000,title:adjustment.title||item.title||raw.title||'Planlagt økt',desc:adjustment.desc||legacy.desc||'',detail:adjustment.detail||legacy.detail||'',week:raw.week??Math.floor(dayDiff(ds,first)/7)+1,label:formatDate(ds,{weekday:'short',day:'numeric',month:'short'}).replace('.',''),workoutId:item.workoutId,planRevisionId:snapshot.planRevisionId,locked:item.lockLevel==='user',terminal:['completed','cancelled','replaced','skipped'].includes(item.status)};
    });
  }
  ...
  function prescription(p){
    const mode=p?.source==='runnerbear-v12.6'?null:oneOffChoiceFor(p);let result=p;
    if(mode){
      const base={...p,mode};
  ...
  function workoutStructure(p){
    ...
    return{
      warmup:x.prescription?.warmup?.seconds?`${x.prescription.warmup.seconds/60} min rolig oppvarming`:model?.warmup||'Åpen oppvarming · 10–15 min rolig. Fortsett ved behov.',
      ...
      cooldown:x.prescription?.cooldown?.seconds?`${x.prescription.cooldown.seconds/60} min rolig nedjogg`:model?.cooldown||'Åpen nedjogg · 10–15 min svært rolig.',
      estimate:workoutDuration(p).label,
      garmin:x.prescription?.warmup?.seconds?'Oppvarming → hovedserie → nedjogg':'Åpen oppvarming → hovedserie → åpen nedjogg'
    };
  }
  ...
  function normalizeActivity(a){
    ...
    return{source:a?.source,sourceId:a?.sourceId,id:String(a?.id||a?._id||''),date:a?.date||'',ds:a?.ds||localIso(a?.date),sportType:String(a?.sportType||'').toLowerCase(),subSportType:String(a?.subSportType||'').toLowerCase(),title:a?.title||s.title||'',duration:Number(a?.duration??s.duration??s.durationTotal)||0,distance:Number(a?.distance??s.distance)||0,pace:Number(a?.pace??s.pace)||0,heartrate:Number(a?.heartrate??s.heartrate)||0,heartrateMax:Number(a?.heartrateMax??s.heartrateMax)||0,power:Number(a?.power??s.power)||0,cadence:Number(a?.cadence??s.cadence)||0,ascent:Number(a?.ascent??s.ascent??s?.altitude?.ascent)||0,temperature:Number(a?.temperature??s.temperature)||0,detail:a?.detail||null,raw:a};
  }
  function activityData(){return window.RunnerBearDataTrust?.history(window.RunnerBearCloudV11?.snapshot?.())||{rows:[],loaded:false,current:false,label:'Aktivitetshistorikken avklares'}}
  function historyCovers(date){return window.RunnerBearDataTrust?.covers(activityData(),date)===true}
  function historyNoticeHtml(){const data=activityData();return data.current?'':`<section class="rb1028-sync-banner retry" role="status"><span><b>${esc(data.label)}</b><small>Registrerte økter vises. Manglende data regnes ikke som uteblitt trening. Oppdateres automatisk.</small></span></section>`}
  function activities(){
    if(renderCache?.activities)return renderCache.activities;
    const rows=read(K.cache,{}).activities,legacy=(Array.isArray(rows)&&rows.length?rows:(engine()?.activities?.()||[])),canonical=activityData().rows,cloud=canonical.map(row=>({...row.payload,source:row.source,sourceId:row.source_id,id:row.payload?.id||row.source_id,date:row.date,ds:row.date,sportType:row.sport_type||row.payload?.sportType,subSportType:row.sub_sport_type||row.payload?.subSportType,title:row.title||row.payload?.title,duration:row.duration_seconds,distance:row.distance_m,pace:row.pace_seconds_per_km??row.payload?.pace,heartrate:row.avg_hr,heartrateMax:row.max_hr??row.payload?.heartrateMax,power:row.power??row.payload?.power,cadence:row.cadence??row.payload?.cadence,updatedAt:row.updated_at})),seen=new Set();
  ...
  function sessionState(p){
    ...
    if(p.ds<today()&&!cancelledFor(p)&&!['rest','cross'].includes(prescription(p).type)&&(!historyCovers(p.ds)||activities().some(a=>a.ds===p.ds)))return{code:'pending',label:activities().some(a=>a.ds===p.ds)?'Kobling avklares':'Historikk avklares',terminal:false};
  ...
  function resultMetrics(a,p){
    if(!a)return[];a=analysisActivity(a);const work=a.detail?.analysis||{},blocks=Array.isArray(work.workBlocks)?work.workBlocks:[],kind=sportKind(a,p),assessment=p?sessionAssessment(p,a):null;
    if(p?.type==='quality'&&blocks.length&&assessment?.confidence?.code!=='limited'){
      ...
      return[
        ...
        [a.paceCorrection?'Arbeidsfart · oppgitt av deg':'Arbeidsfart',work.workPace?`${fmtPace(work.workPace)}/km`:'–'],
        ['Arbeidspuls',work.workHr?`${Math.round(work.workHr)} bpm`:'–'],
        [/45\/15|400/i.test(String(p.title||''))?'Fartsutvikling':'Pulsdrift',/45\/15|400/i.test(String(p.title||''))?a.paceCorrection?'Ikke vurdert':`${fade>0?'+':''}${fade} sek/km`:`${drift>0?'+':''}${drift} bpm`]
      ];
    }
    if(kind==='row')return[
  ...
  function analysisFor(p,a){
    ...
    if(comparable.length&&p.type==='quality'&&Number(work.workPace)>0&&Number(work.workHr)>0){
      const rows=comparable.filter(x=>!!x.activity?.paceCorrection===!!a.paceCorrection&&x.activity?.paceCorrection?.mode!=='unreliable').map(x=>x.activity?.detail?.analysis||{}).filter(x=>Number(x.workPace)>0&&Number(x.workHr)>0),reference=rows.at(-1);
    ...
    }else if(comparable.length&&a.heartrate&&!a.paceCorrection){
    ...
    if(a.paceCorrection)comparison=a.paceCorrection.mode==='unreliable'?'Arbeidsfarten er merket upålitelig av deg og brukes ikke i fartssammenligning.':`Arbeidsfart ${fmtPace(a.paceCorrection.paceSeconds)}/km er oppgitt av deg. ${comparison}`;
    const basis=`${a.paceCorrection?'Korrigert arbeidsgrunnlag · ':''}Analysekvalitet: ${assessment.confidence.label.toLowerCase()}. Basert på Garmin/Tredict, ${match?.confidence==='high'?'sikker':'sannsynlig'} planmatch${assessment.confidence.confirmed?`, ${assessment.confidence.confirmed} planlagte arbeidsdrag bekreftet`:''} og siste 30 dagers belastning.`;
  ...
  function comparisonHtml(p,a){
    a=analysisActivity(a);
    const t=targetFor(p),x=sessionAssessment(p,a),w=x.work||{},actualKm=Number(a.distance||0)/1000,actualPace=a.paceCorrection?.mode==='unreliable'?'Upålitelig arbeidsfart':w.workPace?`${fmtPace(w.workPace)}/km`:a.duration&&actualKm?`${fmtPace(a.duration/actualKm)}/km`:'–',actualHr=w.workHr?`${Math.round(w.workHr)} bpm`:a.heartrate?`${Math.round(a.heartrate)} bpm`:'–',expected=x.model?.expectedIntervals||0;
    ...
    const rows=[[t.label||'Hoveddel',t.main||t.work,actualMain],...(p.type==='quality'?[['Total',t.total,actualKm?`${actualKm.toFixed(1).replace('.',',')} km`:'–']]:[]),[a.paceCorrection?.mode==='set'?'Arbeidsfart · oppgitt av deg':'Fart / styring',t.pace,actualPace],['Puls',t.hr,actualHr]];
  ...
  function plannedDetailsHtml(p){
    const x=prescription(p);return `<div class="rb109-plan-details"><div><span>Hensikt</span><p>${esc(purposeFor(x))}</p></div><div><span>Gjennomføring</span><p>${esc(x.detail||x.desc||'Følg kontrollert belastning.')}</p></div>${x.fuel?`<div><span>Energi</span><p>${esc(x.fuel)}</p></div>`:''}${workoutStructureHtml(p)}</div>`;
  }
  ...
  async function persistDayChoice(before,reason){try{if(window.RunnerBearCloudV11?.snapshot?.()?.flags?.coach_loop_write===true)await persistCanonicalChange(before,{reason});else{const upload=window.RunnerBearCloud?.uploadLocal;if(typeof upload!=='function')throw new Error('Lagring er ikke tilgjengelig.');const saved=await upload(true);if(saved?.ok!==true)throw new Error('Lagringen er ikke bekreftet. Prøv igjen.')}}catch(error){restoreSnapshot(before);renderAll();throw error}}
  async function adapt(p,reason,proposed=null){
    if(!proposed&&reason==='time'){openAdaptation(p?.ds);state.adaptReason='time';renderAll();return false;}
    if(!p||isLocked(p)||isTerminal(p)||p.ds<today()){toast('Økten er låst eller avsluttet.');return false;}
    ...
    if(proposed)all[p.sourceLabel]={...base,...proposed,reason,source:'runnerbear-v12.6'};
    else if(reason==='skip')all[p.sourceLabel]={...base,reason,type:'rest',title:'Hvile · økten utgår',desc:'Ingen treningsgjeld.',detail:'Økten flyttes ikke til en annen dag, og kilometer tas ikke igjen.',km:0,shoe:'',fuel:''};
    ...
    write(K.adjustments,all);try{await persistDayChoice(before,'day-'+reason)}catch(error){toast(error.message||'Valget kunne ikke lagres.');return false}const changed=basePlan(p.baseDs);queueTredictMutation(reason==='skip'?'plan:workout-cancelled':'plan:workout-adjusted',[changed],{previousRows,previousDate:p.ds,reason});addLog(`${p.title} er tilpasset: ${reason==='time'?'kortere tid':reason==='tired'?'sliten kropp':reason==='achilles'?'akillesrespons':reason==='activity'?'annen aktivitet':'økten utgår'}.`,'manual',before);state.adaptOpen=false;state.workoutDetailOpen=false;toast('Valget er lagret i RunnerBear');renderAll();return true;
  }
  ...
  async function setChoice(p,mode){
    if(!canOneOff(p)&&!flexible(p))return;const previousRows=planRowsSnapshot(),before=snapshot(K.dayModes),choices=read(K.dayModes,{});choices[p.baseDs]=mode;write(K.dayModes,choices);try{await persistDayChoice(before,'day-choice-'+mode)}catch(error){toast(error.message||'Valget kunne ikke lagres.');return false}balanceFlexChoice(p);
    const changed=basePlan(p.baseDs);queueTredictMutation('plan:workout-adjusted',[changed],{previousRows,previousDate:p.ds,reason:`day-choice-${mode}`});addLog(`${formatDate(p.ds,{weekday:'long'})}: ${mode==='run'?'rolig jogg':mode==='row'?'Concept2':mode==='bike'?'Zwift':'hvile'} er valgt.`,'manual',before);state.adaptOpen=false;state.workoutDetailOpen=false;toast('Valget er lagret i RunnerBear');renderAll();
  }
  function runAutopilot(){
    if(control()!=='autopilot'||window.RunnerBearCloudV11?.snapshot?.()?.flags?.coach_loop_safe_auto!==true)return;
    window.RunnerBearCloudV11?.applySafeAuto?.().catch(error=>console.warn(JSON.stringify({event:'coach_loop_auto_failed',build:BUILD,message:error?.message||String(error)})));
  }
  ...
  const state={selectedDs:sessionStorage.getItem(K.selected)||'',planView:savedPlanView==='done'?'done':'plan',planLens:['week','long'].includes(savedPlanLens)?savedPlanLens:'week',openWeek:null,monthKey:'',monthOpen:false,planDayViewOpen:false,planDetailOpen:false,moveOpen:false,movePreview:null,moveSaving:false,adaptOpen:false,adaptReason:'',adaptMinutes:45,adaptMode:'bike',adaptPreview:null,adaptError:'',adaptBusy:false,healthRange:7,completedId:'',doneScroll:0,goalManagerOpen:false,goalEditor:'',goalInsightOpen:'',workoutDetailOpen:false,workoutDetailDs:'',workoutBankOpen:false,workoutBankReturnToDetail:false,coachReasonOpen:false,bodyResponseOpen:false,oneDecisionProposalOpen:false,oneDecisionResolving:false,intensityExplanationOpen:false,weeklyReviewOpen:false,syncRepairOpen:false};
  ...
  function syncState(){
    const c=read(K.cache,{}),snapshot=window.RunnerBearCloudV11?.snapshot?.(),canonicalAt=snapshot?.syncSource?.last_synced_at||snapshot?.syncSource?.updated_at||'',at=Date.parse(canonicalAt||c.syncedAt||localStorage.getItem('runnerbear_tredict_last_sync')||0),age=at?Date.now()-at:Infinity;
    const stale=age>6*3600000,time=at?new Intl.DateTimeFormat('nb-NO',{hour:'2-digit',minute:'2-digit'}).format(new Date(at)):'';
    return{at,age,stale,time,label:stale?'Garmin-data oppdateres i bakgrunnen':`Garmin oppdatert automatisk ${time}`};
  }
  ...
  function contextualCoach(){const snapshot=window.RunnerBearCloudV11?.snapshot?.(),value=snapshot?.contextualCoach;return value?.version==='contextual-coach-1'&&value.planRevisionId===snapshot?.planRevisionId&&value.safety?.planWritesByAi===false?value:null}
  ...
  function dailyPresentation(base){return runningDay()?.presentation({decision:oneDecision(),body:bodyResponse(),workout:prescription(base),result:!!(activityFor(base)||replacementActivityFor(base)),reported:readinessState()})}
  function workoutDuration(p){return runningDay()?.duration(prescription(p),analysisEngine()?.structuredWorkout?.(prescription(p)))||{label:'Se øktstrukturen'}}
  function nextStepHtml(base){
    const next=effectiveSchedule().find(row=>row.ds>base?.ds&&!isTerminal(row));if(!next)return'';
    return `<section class="rb126-next"><small>Neste planlagte steg</small><button type="button" data-rb1020-workout-open="${esc(next.ds)}"><span><b>${esc(displayCopy(prescription(next).title))}</b><small>${esc(formatDate(next.ds,{weekday:'long',day:'numeric',month:'short'}))}</small></span>${icon('chevronRight')}</button></section>`;
  }
  function oneDecisionHeroHtml(base){
    const current=oneDecision();if(!current)return'';
    const view=dailyPresentation(base),normal=['workout','rest'].includes(view?.mode),workout=prescription(base),rest=view?.mode==='rest',fresh=runningDay()?.fresh(current),evidence=(current.evidence||[]).slice(0,2),hero=heroNameForWorkout(base),change=planChange(base),action=rest?'view_plan':view?.action||'view_plan';
    const headline=rest?'I dag er det hvile.':displayCopy(view?.headline||current.headline),summary=normal?purposeFor(workout):fresh?current.summary:'Dagens helsegrunnlag er ikke vurdert. Den lagrede planen kan fortsatt åpnes.';
    return `<section class="rb113-one-decision rb116-contextual rb126-recommendation ${normal?'calm':esc(current.tone||'watch')} ${esc(current.state)}" aria-labelledby="rb113DecisionTitle">${normal?`<div class="rb126-photo" ${heroStyle(hero)}>${heroImage(hero,true)}</div>`:''}<div class="rb126-recommendation-body"><header><span class="rb113-decision-status">${change?'Planen er tilpasset':normal?rest?'Planlagt restitusjon':'Dagens anbefaling':'Dagens avklaring'}</span></header><div class="rb113-decision-copy"><h2 id="rb113DecisionTitle">${esc(headline)}</h2><p>${esc(normal&&!rest?workoutDuration(base).label:summary)}</p></div>${normal?`<div class="rb126-focus"><b>Dagens fokus</b><p>${esc(keyAdvice(base))}</p></div>`:base?`<p class="rb123-planned">Fra planen: ${esc(displayCopy(workout.title))}</p>`:''}<div class="rb113-decision-actions"><button type="button" class="rb113-primary" data-rb113-decision-action="${esc(action)}" ${state.oneDecisionResolving?'disabled':''}>${state.oneDecisionResolving?'Lagrer …':rest?'Se ukeplanen':normal?'Se økten':esc(view?.label||'Se planen')}${icon('arrowRight')}</button></div></div></section>${normal&&base?.ds===today()&&!isTerminal(base)&&!rest?'<button type="button" class="rb126-adapt-link" data-rb123-adapt>Dårlig tid eller tunge bein? <b>Tilpass</b> ›</button>':''}${normal?`<section class="rb126-why"><h2>${rest?'Hvile med mening':'Derfor denne økten'}</h2><p>${esc(change?.why||summary)}</p><details class="rb121-workout-why"><summary>Se coachens grunnlag ${icon('chevronRight')}</summary>${evidence.length?`<ul>${evidence.map(row=>`<li>${esc(row.label)} · ${esc(row.source)}</li>`).join('')}</ul>`:''}<button type="button" class="rb107-button ghost" data-rb107-open-why>Åpne vurderingen</button></details></section>${rest?nextStepHtml(base):''}`:''}`;
  }
  ...
  function contextualHealthStripHtml(){
    const body=bodyResponse(),surface=contextualCoach()?.surfaces?.health,presentation=window.RunnerBearV12Decision?.presentation?.({decision:oneDecision(),body,brief:proactiveCoachBrief()});
    if(!body)return `<button type="button" class="rb116-health-strip empty" data-rb111-body-open><span>${icon('heartbeat')}</span><span><b>Nye helsedata mangler</b><small>Se datagrunnlaget ›</small></span></button>`;
    const freshness={fresh:'Ferske data',partial:'Delvis ferske data',stale:'Oppdateres automatisk'}[body.freshness?.status]||'Datastatus ukjent',headline=presentation?.healthHeadline||surface?.headline||body.stateLabel,summary=presentation?.healthSummary||surface?.summary||body.summary,attention=surface?.visible===true||body.state!=='as_planned'||body.freshness?.status!=='fresh'||presentation?.actionRequired===true;
    if(!attention)return `<button type="button" class="rb116-health-strip normal rb121-health-compact" data-rb111-body-open aria-label="Åpne helsebildet"><span>${icon('heart')}</span><b>${esc('Helsebildet · se målinger')}</b>${icon('chevronRight')}</button>`;
  ...
  function bodyResponseDetailModalHtml(){
    if(!state.bodyResponseOpen)return'';
    const body=bodyResponse(),range=state.healthRange,rows=[['HRV','Nattens pulsvariasjon',body?.metrics?.hrv],['Søvn','Registrert søvnlengde',body?.metrics?.sleep],['Hvilepuls','Registrert hvilepuls',body?.metrics?.rhr]],load=body?.domains?.find(row=>row.id==='load'),fresh=body?.freshness?.status==='fresh',complete=['hrv','sleep','rhr'].every(key=>body?.metrics?.[key]?.current!=null),updated=body?.freshness?.syncedAt?dataTime(body.freshness.syncedAt):'Ukjent';
    const metrics=rows.map(([label,copy,metric])=>`<article><header><small>${label}</small><b>${esc(metric?.current==null?'Mangler':bodyMetricTrend(metric,'current'))}</b></header><p>${esc(copy)}</p><dl><div><dt>Personlig referanse</dt><dd>${esc(metric?.baseline==null?'For lite grunnlag':bodyMetricTrend(metric,'baseline'))}</dd></div><div><dt>${range} dager</dt><dd>${esc(metric?.['trend'+range]==null?'For lite grunnlag':bodyMetricTrend(metric,'trend'+range))}</dd></div></dl></article>`).join('');
    const reported=readinessState();const check=body?.checkIn?.required?body.checkIn.options:[{value:'fresh',label:'Bra'},{value:'tired',label:'Litt sliten'},{value:'heavy',label:'Tung'}];
    return `<div class="rb1020-modal rb111-body-modal" data-rb1020-modal-backdrop="body-response"><section class="rb1020-sheet rb111-body-sheet rb126-health" role="dialog" aria-modal="true" aria-labelledby="rb111BodyTitle"><header><div><span>Grunnlaget for planen</span><h2 id="rb111BodyTitle">Helsebildet</h2><p>Sist oppdatert: ${esc(updated)}</p></div><button type="button" data-rb1020-modal-close="body-response" aria-label="Lukk helsebildet">${icon('close')}</button></header><div class="rb1020-sheet-body"><section class="rb111-body-lead ${esc(body?.state||'wait_for_data')}"><div><small>${fresh&&complete?'Dagens vurdering':fresh?'Delvis datagrunnlag':'Begrenset datagrunnlag'}</small><h3>${esc(!body?'Nye helsedata mangler':!fresh?'Helsegrunnlaget er ikke helt oppdatert':!complete?'Noen helsemålinger mangler':body.stateLabel)}</h3><p>${esc(!body?'Dagens helsegrunnlag er ikke vurdert. Data hentes automatisk.':!fresh?'Siste tilgjengelige målinger vises nedenfor. De bekrefter ikke kroppens tilstand i dag.':!complete?'Tilgjengelige målinger er ferske. Det som mangler vises nedenfor og regnes ikke som en normal måling.':body.summary)}</p></div></section><div class="rb126-options" role="group" aria-label="Periode for helsetrend">${[7,28].map(n=>`<button data-rb126-health-range="${n}" aria-pressed="${range===n}">${n} dager</button>`).join('')}</div><section class="rb111-metric-grid">${metrics}${load?`<article><header><small>Belastning</small><b>${esc(load.label||'Belastning')}</b></header><p>${esc(load.summary)}</p></article>`:''}</section><section class="rb111-checkin"><h3>${esc(body?.checkIn?.prompt||'Hvordan kjenner du deg?')}</h3><fieldset class="rb126-body-reasons"><legend>Noe coachen bør vite?</legend>${[['achilles','Smerte eller stivhet i akilles'],['illness','Sykdomsfølelse']].map(([value,label])=>`<label><input type="checkbox" name="rb126-body-reason" value="${value}" ${reported.reasons.includes(value)?'checked':''}>${label}</label>`).join('')}</fieldset><div>${(check||[]).map(option=>`<button type="button" data-rb111-checkin="${esc(option.value)}">${esc(option.label)}</button>`).join('')}</div></section><details class="rb121-workout-why"><summary>Slik brukes målingene ${icon('chevronRight')}</summary><p>Trender og egenfølelse inngår i coachens vurdering. En enkelt måling endrer ikke planen alene. Personlig referanse bygger på tilgjengelig historikk, inntil 28 dager.</p>${(body?.domains||[]).filter(row=>row.id!=='load').map(row=>`<p><b>${esc(row.label)}</b> · ${esc(row.summary)}</p>`).join('')}<p>Manglende målinger vises som «Mangler», ikke som null eller normal tilstand.</p></details></div></section></div>`;
  }
  ...
  async function runOneDecisionAction(kind){
    ...
    trackEvent('one_decision_action',{state:current.state,action:kind});
    if(kind==='review_adjustment'&&!runningDay()?.fresh(current)){toast('Vurderingen må fornyes før du velger.');await window.RunnerBearCloudV11?.refresh?.();return}
    ...
    if(kind==='open_workout'||kind==='view_result'||kind==='complete_feedback'){if(!base)return;state.adaptOpen=false;responsePhase='post_workout';state.workoutDetailDs=base.ds;state.workoutDetailOpen=true;renderAll();requestAnimationFrame(()=>{if(kind==='complete_feedback'){const field=qs('.rb123-response input,.rb123-response button');field?.focus();field?.scrollIntoView({block:'center',behavior:'smooth'})}else qs('.rb1020-workout-sheet [data-rb1020-modal-close]')?.focus()});return}
  ...
  function weeklyPriorityHtml(){
    const week=proactiveCoachBrief().week||{};
    return `<section class="rb121-support rb119b-card"><span class="rb107-overline">Ukens hensikt</span><h2>${esc(week.priority||'Bevar kontinuiteten')}</h2><p>${esc(week.reason||'Kontrollert arbeid over tid er hovedprioriteten.')}</p></section>`;
  }
  ...
  function changeSupportHtml(base){
    const snapshot=window.RunnerBearCloudV11?.snapshot?.(),change=planChange(base),html=changeNoticeHtml(base);
    if(!html)return'';
    const content={date:base?.ds,title:base?.title,before:change?.before,after:change?.after,why:change?.why,reason:snapshot?.activePlan?.reasonCode};
    if(calm()?.hasSeen(localStorage,'plan-change',content))return'';
    return `<div class="rb121-change-support">${html}<button class="rb121-read-link" data-rb121-change-read="${esc(JSON.stringify(content))}">Forstått · detaljer finnes i Plan</button></div>`;
  }
  function weeklyReviewCardHtml(){
    const review=weeklyReview();if(!review||calm()?.hasSeen(localStorage,'weekly-review',calm().reviewContent(review)))return'';
    return `<section class="rb121-support rb119b-card"><span class="rb107-overline">Ny ukesoppsummering</span><h2>${esc(review.headline)}</h2><p>${esc(review.coachComment||review.nextDirection)}</p><button class="rb107-button secondary" type="button" data-rb1031-week-review>Les ukesoppsummering ${icon('chevronRight')}</button></section>`;
  }
  function weeklyReviewModalHtml(){
    const review=weeklyReview();if(!state.weeklyReviewOpen||!review)return'';const t=review.totals||{},sessions=review.sessions||[],done=sessions.filter(row=>row.state==='completed'),adapted=sessions.filter(row=>row.state==='replaced'),missed=sessions.filter(row=>row.state==='missed'),context=contextualCoach()?.surfaces?.weekly,priorities=(context?.priorities||[review.nextDirection]).filter(Boolean).slice(0,2);
    ...
    return `<div class="rb1020-modal rb1031-review-modal" data-rb1020-modal-backdrop="weekly-review"><section class="rb1020-sheet rb1031-review-sheet" role="dialog" aria-modal="true" aria-labelledby="rb1031ReviewTitle"><header><div><span>Coachens ukesoppsummering</span><h2 id="rb1031ReviewTitle">${esc(review.headline)}</h2></div><button type="button" data-rb1020-modal-close="weekly-review" aria-label="Lukk ukesoppsummeringen">${icon('close')}</button></header><div class="rb1020-sheet-body"><section><small>Uken i tall</small><b>${Number(t.completedQualitySessions||0)} av ${Number(t.plannedQualitySessions||0)} kvalitetsøkter · ${esc(reviewLongRun(review))} langtur</b><p>${esc(reviewKm(t.completedDistanceM))} løping · ${esc(reviewDuration(t.completedDurationSeconds))}</p></section><section><small>Gjennomført</small><ul>${rows(done)}</ul></section><section><small>Tilpasset</small><b>${Number(t.replacedSessions||0)} erstattet · ${Number(t.missedSessions||0)} tapt</b><ul>${rows([...adapted,...missed])}</ul></section><section><small>Det som gikk bra</small><p>${esc(context?.whatWentWell||review.coachComment||review.learning)}</p></section>${context?.healthTrend?`<section><small>Helsetrend og belastning</small><b>${esc(context.healthTrend)}</b><p>${esc(review.coachComment)}</p></section>`:''}<section class="rb116-review-priorities"><small>Neste uke</small><ol>${priorities.map(value=>`<li>${esc(value)}</li>`).join('')}</ol></section></div></section></div>`;
  }
  ...
  function forecast(){const pred=engine()?.predictions?.()||{},goal=activeGoal(),distance=goal?.distance||'half';return{...pred,goal,distance,current:Number(pred?.[distance]?.seconds)||null,foundation:predictionFoundation(distance,pred),form:formDirection()}}
  function thresholdEvidence(){
    const sessions=effectiveSchedule().filter(p=>p.type==='quality'&&p.baseDs<=today()).map(p=>{
      const activity=analysisActivity(activityFor(p)||replacementActivityFor(p)),feedback=feedbackFor(p),assessment=activity?sessionAssessment(p,activity):null;
  ...
  function responseContext(p,phase='post_workout'){
    const snapshot=window.RunnerBearCloudV11?.snapshot?.(),activity=activityFor(p)||replacementActivityFor(p),source=snapshot?.recentActivities?.find(a=>String(a.payload?.id||a.source_id)===activity?.id),candidates=(snapshot?.activePlan?.items||[]).filter(w=>w.localDate===p?.ds&&(w.workoutId===p.workoutId||!p.workoutId)),workout=candidates.length===1?candidates[0]:null,age=dayDiff(today(),p?.ds);
    if(!snapshot?.responseCapture||!source||!workout||age<0||age>7||phase==='next_morning'&&age!==1)return null;
    const receipt=runningDay()?.receipt(snapshot.responseEvents,workout.workoutId,source.source_id,phase),post=runningDay()?.receipt(snapshot.responseEvents,workout.workoutId,source.source_id),pain=Number(post?.payload?.pain)>0||readinessState().reasons.includes('achilles'),key=`${source.source}:${source.source_id}:${phase}`;
    return{snapshot,activity,source,workout,receipt,pain,key,phase};
  }
  function coachFeedbackHtml(p,assessment={},phase='post_workout'){
    const c=responseContext(p,phase);if(!c)return'';const fields=runningDay()?.questions({type:prescription(p).type,confidence:assessment.confidence?.code||assessment.confidence,pain:c.pain,phase})||[];if(!fields.length)return'';
    if(c.receipt&&!responseEdits.has(c.key)){const used=c.snapshot.coachDecision?.inputCursor?.includes(c.receipt.payload?.sourceId);return`<section class="rb123-response-saved" role="status"><b>Svaret er lagret</b><p>${used?'Tatt med i coachens siste vurdering. Se dagens råd for videre oppfølging.':'Tilgjengelig for coachens neste vurdering. En eventuell planendring vises i dagens råd.'}</p><button type="button" class="rb107-button ghost" data-rb123-response-edit="${esc(c.key)}">Endre svaret</button></section>`}
    const saved=c.receipt?.payload||{},busy=responseSaving.has(c.key),error=responseErrors.get(c.key),values=responseDrafts.get(c.key)?.values||saved;
    return `<form class="rb1026-feedback rb123-response" data-rb123-response="${esc(p.baseDs)}" data-response-phase="${phase}" data-response-key="${esc(c.key)}"><span class="rb107-overline">${phase==='next_morning'?'Dagen etter':'Kort respons etter økten'}</span><p>Svaret hjelper coachen å vurdere neste belastning.</p>${fields.map(q=>`<fieldset ${busy?'disabled':''}><legend>${esc(q.label)}</legend>${q.options? q.options.map(([value,label])=>`<label><input type="radio" name="${q.key}" value="${value}" ${String(values[q.key])===String(value)?'checked':''} required><span>${esc(label)}</span></label>`).join(''):`<select name="${q.key}" required aria-label="${esc(q.label)}"><option value="">Velg 0–10</option>${Array.from({length:11},(_,value)=>`<option value="${value}" ${Number(values[q.key])===value&&values[q.key]!=null?'selected':''}>${value}${value===0?' · ingen smerte':value===10?' · verst tenkelig':''}</option>`).join('')}</select>`}</fieldset>`).join('')}<p role="status" aria-live="polite">${esc(error||'')}</p><button type="submit" class="rb107-button" ${busy?'disabled':''}>${busy?'Lagrer …':'Lagre svar'}</button>${c.receipt?`<button type="button" class="rb107-button ghost" data-rb123-response-cancel="${esc(c.key)}">Avbryt endring</button>`:''}</form>`;
  }
  function recentResponseHtml(){
    const p=effectiveSchedule().filter(p=>p.ds===addDays(today(),-1)&&(activityFor(p)||replacementActivityFor(p))).at(-1);if(!p)return'';const c=responseContext(p);if(!c)return'';const phase=c.receipt&&c.pain?'next_morning':'post_workout',next=responseContext(p,phase),fields=runningDay()?.questions({type:prescription(p).type,confidence:sessionAssessment(p,c.activity).confidence?.code,pain:c.pain,phase})||[];if(!fields.length||next?.receipt)return'';
    return `<section class="rb119b-card rb121-support"><small>Etter gårsdagens økt</small><h2>${phase==='next_morning'?'Hvordan kjennes kroppen i dag?':'Hvordan opplevde du økten?'}</h2><p>${esc(prescription(p).title)} er registrert. Ett kort svar gir coachen mer å vurdere.</p><button type="button" class="rb107-button secondary" data-rb123-response-open="${esc(p.ds)}" data-response-phase="${phase}">${phase==='next_morning'?'Svar om morgenformen':'Gi kort respons'} ${icon('arrowRight')}</button></section>`;
  }
  ...
  function resultCardHtml(p,context='today'){
    const a=activityFor(p);if(!a)return'';const metrics=resultMetrics(a,p),x=analysisFor(p,a);
    return `<article class="rb107-card rb109-result-card ${context==='plan'?'compact':''}">${context==='today'?`<div class="rb126-photo" ${heroStyle(heroNameForWorkout(p))}>${heroImage(heroNameForWorkout(p))}</div>`:''}<header class="rb109-result-head"><div class="rb109-result-mark">${icon('check')}</div><div><span class="rb107-overline">Registrert gjennomføring</span><h2>${esc(prescription(p).title)}</h2><p>Økten er lagret i historikken din.</p></div></header><div class="rb109-result-metrics">${metrics.slice(0,3).map(m=>`<div><span>${esc(m[0])}</span><b>${esc(m[1])}</b></div>`).join('')}</div><section class="rb109-coach-verdict"><span>Fra økten</span><p>${esc(x.review)}</p></section>${coachFeedbackHtml(p,x)}<div class="rb109-result-details"><details><summary>Se øktanalysen ${icon('chevronRight')}</summary>${analysisDetailsHtml(p,a)}</details></div></article>`;
  }
  ...
  function workoutHeroHtml(base){
    ...
    const hero=heroNameForWorkout(base);
    return `<button type="button" class="rb119b-workout-hero rb1020-workout-hero" data-rb1020-workout-open="${esc(base.ds)}" aria-label="Åpne øktdetaljer for ${esc(displayCopy(p.title))}" ${heroStyle(hero)}>${heroImage(hero,true)}<span class="rb119b-workout-copy"><small>${esc(typeLabel(p.type))}</small><h2>${esc(displayCopy(p.title))}</h2></span></button><section class="rb125-workout-information" aria-label="Øktinformasjon"><p>${esc(main)}</p>${recovery?`<p class="rb125-recovery">${esc(recovery)}</p>`:''}<div class="rb119b-hero-metrics"><div>${icon('clock')}<span><small>Total tid</small><b>${esc(total)}</b></span></div><div>${icon('pin')}<span><small>Distanse</small><b>${esc(distance)}</b></span></div><div>${icon('goal')}<span><small>Fokus</small><b>${esc(focus)}</b></span></div></div><button class="rb107-button" data-rb1020-workout-open="${esc(base.ds)}">${esc(status)} ${icon('chevronRight')}</button></section>`;
  }
  ...
  function bakkenChoiceHtml(p){
    ...
    return `<details class="rb121-workout-why rb1020-detail-section"><summary>Hvorfor denne økten? ${icon('chevronRight')}</summary><p>${esc(meta.rationale||meta.purpose||'Økten er valgt ut fra respons og resten av treningsuken.')}</p></details>`;
  }
  function simpleStructureHtml(p){
    const x=prescription(p),s=workoutStructure(p);if(s)return workoutStructureHtml(p);
    if(x.type==='rest')return `<div class="rb1020-simple-structure"><div><b>Hvile</b><span>${esc(displayCopy(x.detail||'Planlagt restitusjon mellom belastningsdagene.'))}</span></div></div>`;
    return `<div class="rb1020-simple-structure"><div><b>Gjennomføring</b><span>${esc(x.desc||x.title)}</span></div><div><b>Styring</b><span>${esc(displayCopy(x.detail||'Jevn, kontrollert belastning.'))}</span></div></div>`;
  }
  function contextualWorkoutCoachHtml(p,planStatus,a){
    const replacement=planStatus.code==='replaced'?replacementFor(p)?.classification?.coach:null,assessment=a&&planStatus.code==='completed'?analysisFor(p,a):null,post=contextualCoach()?.surfaces?.postWorkout,after=!!a||planStatus.terminal;
    const headline=after?(replacement?.headline||assessment?.headline||(post?.visible?post.headline:'Økten er registrert')):'Nøkkelråd før økten',summary=after?(replacement?.message||assessment?.review||(post?.visible?post.summary:'Gjennomføringen er registrert og tas med i neste vurdering.')):keyAdvice(p),consequence=after?(replacement?.consequence||(post?.visible?post.consequence:null)||assessment?.consequence||'Resten av uken vurderes fra den registrerte responsen.'):'Dette er det eneste du trenger å prioritere i gjennomføringen.';
    return `<section class="rb1020-coach-key rb116-workout-coach ${after?'after':'before'}"><span>${after?'Etter økten':'Før økten'} · Coachens vurdering</span><h3>${esc(headline)}</h3><p>${esc(summary)}</p><small><b>${after?'Dette betyr for uka':'Prioritet'}</b>${esc(consequence)}</small></section>`;
  }
  function adaptationSignature(p){return JSON.stringify({revision:window.RunnerBearCloudV11?.snapshot?.()?.planRevisionId,local:currentPlanRevision(),body:bodyResponse()?.inputCursor,decision:oneDecision()?.inputCursor,reported:readinessState(p?.ds),workout:prescription(p),status:sessionState(p).code,locked:isLocked(p)})}
  function openAdaptation(ds=today()){
    state.adaptOpen=true;state.workoutDetailOpen=true;state.workoutDetailDs=ds;state.adaptReason='';state.adaptPreview=null;state.adaptError='';renderAll();
    requestAnimationFrame(()=>qs('.rb126-adapt-sheet [data-rb1020-modal-close]')?.focus());
  }
  async function previewAdaptation(p){
    if(state.adaptBusy||!p||isTerminal(p)||isLocked(p))return;
    state.adaptError='';const workout=prescription(p),model=analysisEngine()?.structuredWorkout?.(workout);
    let proposal;
    if(state.adaptReason==='time')proposal=runningDay().timeProposal({workout,model,minutes:state.adaptMinutes});
    else if(state.adaptReason==='activity'){
      const mode=state.adaptMode,title={bike:'Rolig sykling',row:'Rolig roing',rest:'Hvile'}[mode],minutes=mode==='rest'?0:state.adaptMinutes;
      proposal={before:workout.title,after:minutes?`${title} · ${minutes} min`:title,beforeDuration:workoutDuration(p).label,afterDuration:minutes?`${minutes} min`:'Hviledag',patch:{title:minutes?`${title} · ${minutes} min`:title,type:mode==='rest'?'rest':'cross',workoutType:mode==='rest'?'rest':'cross',sport:{bike:'cycling',row:'rowing',rest:'rest'}[mode],desc:minutes?'Rolig, jevn innsats. Ingen ekstra kvalitetsøkt.':'Planlagt restitusjon.',detail:minutes?'Alternativ aktivitet etter ditt valg. Belastningen vurderes fra faktisk gjennomføring.':'Økten tas ut uten treningsgjeld.',km:0,plannedDistanceM:0,plannedDurationSeconds:minutes*60}};
    }
    if(!proposal)return;if(proposal.error){state.adaptError=proposal.error;renderAll();return}
    const signature=adaptationSignature(p),key=`day-choice:${crypto.randomUUID()}`;state.adaptBusy=true;renderAll();
    try{
      let server=null;
      if(window.RunnerBearCloudV11?.snapshot?.()?.flags?.coach_loop_write===true){
        server=await window.RunnerBearCloudV11.previewUserWorkout(p.workoutId,proposal.patch,key);
        const row=server.rows?.find(row=>row.workoutId===p.workoutId);
        if(!row)throw new Error('Økten mangler i forslaget. Åpne den på nytt.');
        proposal={...proposal,after:row.title,afterDuration:row.plannedDurationSeconds?`ca. ${Math.round(row.plannedDurationSeconds/60)} min`:'Se foreslått struktur'};
      }
      if(!state.adaptOpen||signature!==adaptationSignature(basePlan(p.baseDs)))throw new Error('Planen har endret seg. Hent et nytt forslag.');
      state.adaptPreview={...proposal,signature,server,key,baseDs:p.baseDs,reason:state.adaptReason,at:Date.now()};
    }catch(error){state.adaptError=error.message||'Forslaget kunne ikke hentes.'}
    finally{state.adaptBusy=false;renderAll();requestAnimationFrame(()=>{qs('.rb126-adapt-sheet h2')?.focus({preventScroll:true});const body=qs('.rb126-adapt-sheet .rb1020-sheet-body');if(body)body.scrollTop=0})}
  }
  async function applyAdaptation(){
    const preview=state.adaptPreview,p=basePlan(preview?.baseDs);if(state.adaptBusy||!preview||!p)return;
    if(isTerminal(p)||isLocked(p)||p.ds<today()||preview.signature!==adaptationSignature(p)||Date.now()-preview.at>5*60000){state.adaptPreview=null;state.adaptError='Planen eller datagrunnlaget er endret. Hent et nytt forslag.';renderAll();return}
    state.adaptBusy=true;state.adaptError='';renderAll();
    try{
      if(preview.server){const saved=await window.RunnerBearCloudV11.commitUserWorkout(preview.server);state.adaptOpen=false;state.workoutDetailOpen=false;toast(saved.refreshPending?'Endringen er lagret. Visningen oppdateres når forbindelsen er tilbake.':'Planendringen er lagret. Kalenderstatus oppdateres automatisk.')}
      else if(!await adapt(p,preview.reason==='time'?'time':'activity',preview.patch))throw new Error('Endringen er ikke bekreftet lagret. Prøv igjen.');
      state.adaptPreview=null;
    }catch(error){state.adaptError=error.message||'Lagringen er ikke bekreftet.'}
    finally{state.adaptBusy=false;renderAll()}
  }
  function adaptationHtml(p){
    const preview=state.adaptPreview,busy=state.adaptBusy,disabled=busy?'disabled':'',choices=[['time','clock','Mindre tid'],['body','heart','Tunge bein eller ubehag'],['activity','swap','Annen aktivitet'],['move','calendar','Flytte økten']];
    const changed=preview?.server?.rows?.filter(row=>{const old=preview.server.before.find(x=>x.workoutId===row.workoutId);return !old||JSON.stringify([old.localDate,old.title,old.plannedDistanceM,old.plannedDurationSeconds,old.workoutType])!==JSON.stringify([row.localDate,row.title,row.plannedDistanceM,row.plannedDurationSeconds,row.workoutType])})||[];
    const impact=preview?.server?`<ul class="rb126-impact">${changed.map(row=>`<li><time>${esc(formatDate(row.localDate,{weekday:'short',day:'numeric',month:'short'}))}</time><b>${esc(displayCopy(row.title))}</b><span>${row.plannedDurationSeconds?`${Math.round(row.plannedDurationSeconds/60)} min`:esc(typeLabel(row.workoutType))}</span></li>`).join('')}</ul>`:'<p>Dette valget endrer bare denne dagen. Resten av den lagrede planen beholdes. Coachen vurderer belastningen videre.</p>';
    const content=preview?`<div class="rb126-compare"><div><small>Planlagt</small><h3>${esc(preview.before)}</h3><p>${esc(preview.beforeDuration)}</p></div><div><small>Forslag</small><h3>${esc(preview.after)}</h3><p>${esc(preview.afterDuration)}</p></div></div><section class="rb126-why"><h3>Hvorfor dette passer</h3><p>${preview.reason==='time'?'Økten forkortes innenfor tiden du har. Intensiteten økes ikke.':'Du velger en annen aktivitet. Den erstatter den planlagte økten.'}</p></section><section class="rb126-why"><h3>Dette betyr for uken</h3>${preview.workReductionSeconds?`<p>${Math.round(preview.workReductionSeconds/60)} min mindre terskelarbeid i dagens forslag.</p>`:''}${impact}</section><p class="rb126-note">Forslaget er ikke lagret ennå.</p><button class="rb107-button" data-rb126-apply ${disabled}>${busy?'Lagrer …':'Bruk forslaget'}</button><button class="rb107-button ghost" data-rb126-preview-back ${disabled}>Behold opprinnelig økt</button>`:`<p>Velg hva du trenger. Du får se endringen før noe lagres.</p><div class="rb126-reasons" role="group" aria-label="Behov i dag">${choices.map(([key,mark,label])=>`<button type="button" data-rb126-reason="${key}" aria-pressed="${state.adaptReason===key}" ${disabled}>${icon(mark)}<span>${label}</span>${state.adaptReason===key?icon('check'):icon('chevronRight')}</button>`).join('')}</div>${state.adaptReason==='time'||state.adaptReason==='activity'?`${state.adaptReason==='activity'?`<div class="rb126-options" role="group" aria-label="Aktivitet">${[['bike','Sykling'],['row','Roing'],['rest','Hvile']].map(([key,label])=>`<button data-rb126-mode="${key}" aria-pressed="${state.adaptMode===key}" ${disabled}>${label}</button>`).join('')}</div>`:''}${state.adaptReason!=='activity'||state.adaptMode!=='rest'?`<h3>Hvor mye tid har du?</h3><div class="rb126-options" role="group" aria-label="Tilgjengelig tid">${[30,45,60].map(n=>`<button data-rb126-minutes="${n}" aria-pressed="${state.adaptMinutes===n}" ${disabled}>${n} min</button>`).join('')}</div>`:''}<button class="rb107-button" data-rb126-preview="${esc(p.baseDs)}" ${disabled}>${busy?'Vurderer forslaget …':'Se forslag →'}</button>`:''}${state.adaptReason==='body'?`<section class="rb126-why"><h3>La kroppen få en vurdering</h3><p>Egenfølelse, smerte og sykdom vurderes sammen med helsedataene før treningsbelastningen endres.</p><button class="rb107-button" data-rb126-body>Åpne kroppssjekken →</button></section>`:''}${state.adaptReason==='move'?`<section class="rb126-why"><h3>Finn en ny dag</h3><p>Velg dato i planen. Du får se konsekvensene før flyttingen lagres.</p><button class="rb107-button" data-rb126-move="${esc(p.ds)}">Velg ny dag →</button></section>`:''}`;
    return `<div class="rb1020-modal" data-rb1020-modal-backdrop="workout"><section class="rb1020-sheet rb1020-workout-sheet rb126-adapt-sheet" role="dialog" aria-modal="true" aria-labelledby="rb1020WorkoutTitle" aria-busy="${busy}"><header><div><span>Tilpass økten</span><h2 id="rb1020WorkoutTitle" tabindex="-1">${preview?'Se endringen før du velger':'Hva trenger du i dag?'}</h2></div><button type="button" data-rb1020-modal-close="workout" aria-label="Lukk øktdetaljer" ${disabled}>${icon('close')}</button></header><div class="rb1020-sheet-body">${content}${state.adaptError?`<p class="rb126-error" role="alert">${esc(state.adaptError)}</p>`:''}</div></section></div>`;
  }

  function workoutDetailModalHtml(p){
    if(!p)return'';if(state.adaptOpen)return adaptationHtml(p);const x=prescription(p),planStatus=sessionState(p),a=activityFor(p)||replacementActivityFor(p),done=planStatus.terminal,status=planStatus.code==='planned'?(x.type==='rest'?'Hviledag':'Planlagt'):planStatus.label,metrics=a?resultMetrics(a,p):[],bodyCheck=!done&&p.ds===today()?todayActionsHtml(p):'',bankAction=!done&&x.type==='quality'?`<button class="rb107-button secondary rb10251-bank-open" data-rb10251-bank-open="${esc(p.baseDs)}">${icon('swap')} Bytt økt</button>`:'';
    return `<div class="rb1020-modal" data-rb1020-modal-backdrop="workout"><section class="rb1020-sheet rb1020-workout-sheet" role="dialog" aria-modal="true" aria-labelledby="rb1020WorkoutTitle"><header><div><span>${esc(status)} · ${esc(formatDate(p.ds,{weekday:'long',day:'numeric',month:'long'}))}</span><h2 id="rb1020WorkoutTitle">${state.adaptOpen?'Tilpass i dag':esc(displayCopy(x.title))}</h2><p>${esc(typeLabel(x.type))}</p></div><button type="button" data-rb1020-modal-close="workout" aria-label="Lukk øktdetaljer">${icon('close')}</button></header><div class="rb1020-sheet-body">${state.adaptOpen?`<section class="rb1020-detail-section"><h3>${esc(displayCopy(x.title))}</h3><p>Velg det som passer dagen. Valget gjelder denne økten.</p></section>${flexHtml(p)}${bodyCheck}`:a?`<section class="rb1020-detail-section"><span>${planStatus.code==='replaced'?'Faktisk aktivitet · erstattet':'Gjennomført'}</span><div class="rb1020-result-grid">${metrics.map(row=>`<div><small>${esc(row[0])}</small><b>${esc(row[1])}</b></div>`).join('')}</div></section>`:''}${a&&!state.adaptOpen?coachFeedbackHtml(p,sessionAssessment(p,a),responsePhase)+correctionHtml(p,a):''}${!state.adaptOpen?`${a?'<details class="rb121-workout-why"><summary>Se den planlagte økten</summary>':''}<section class="rb1020-detail-section"><span>${a?'Planlagt øktstruktur':'Øktstruktur'}</span>${simpleStructureHtml(p)}</section><details class="rb121-workout-why"><summary>Se intensitetsmål ${icon('chevronRight')}</summary>${workoutIntensityHtml(p)}</details>${bakkenChoiceHtml(p)}${contextualWorkoutCoachHtml(p,planStatus,a)}${x.fuel?`<section class="rb1020-detail-section rb1020-practical"><span>Praktisk</span>${x.fuel?`<p><b>Energi</b>${esc(x.fuel)}</p>`:''}</section>`:''}${!done?`<button type="button" class="rb107-button" data-rb126-adapt-open="${esc(p.ds)}">Tilpass økten</button>${bankAction}`:''}${!done?garminStatusHtml(p):''}${a?'</details>':''}`:''}</div></section></div>`;
  }
  function workoutBankModalHtml(p){
    if(!p)return'';const intended=adaptivePlan()?.stimulusForWorkout?.(prescription(p))||'threshold',nextQuality=effectiveSchedule().find(row=>row.ds>p.ds&&['quality','race'].includes(prescription(row).type)),long=effectiveSchedule().find(row=>row.ds>=p.ds&&/langtur/i.test(row.title||'')),weekState=adaptiveWeekState(),goal=activeGoal(),meta=bakkenMeta(p),ranked=adaptivePlan()?.rankWorkoutBank?.({plan:prescription(p),intendedStimulus:intended,weekMode:weekState.mode,healthTrend:healthSignal().tone==='red'?'negative':'normal',daysToNextQuality:nextQuality?dayDiff(nextQuality.ds,p.ds):99,daysToLongRun:long?dayDiff(long.ds,p.ds):99,phase:String(meta?.phase||(goal&&goalDays(goal)<35?'SPECIFIC':'BUILD')),goalDistance:goal?.distance||'half'})||[],groups=[['main_threshold','Hovedterskel'],['support_threshold','Støtteterskel'],['x','X-økter'],['race_specific','Løpsspesifikt'],['reduced','Redusert dose']],role=row=>row.group==='main_threshold'?'Hovedterskel':row.group==='support_threshold'?'Støtteterskel':row.group==='race_specific'?'Løpsspesifikk':row.group==='reduced'?'Redusert dose':row.dose==='moderate'?'Kontrollert X · moderat dose':row.dose==='full'?'Kontrollert X · full dose':'Kontrollert X',pause=row=>row.main?.recoverySeconds?`${row.main.recoverySeconds<60?`${row.main.recoverySeconds} sek`:`${String(row.main.recoverySeconds/60).replace('.',',')} min`} rolig pause`:'kontrollert pause',rowHtml=row=>`<button data-rb10251-bank-select="${esc(row.id)}" data-base-ds="${esc(p.baseDs)}"><span><small>${esc(role(row))}</small><b>${esc(row.title)}</b><em>${esc(row.stimulus==='x'&&row.dose?role(row):`${role(row)} · ${pause(row)}`)}</em></span><strong>${row.suitabilityScore>=88?'Passer svært godt':row.suitabilityScore>=70?'Passer godt':'Vurderes med margin'}</strong>${icon('chevronRight')}</button>`,sections=groups.map(([key,title])=>{const rows=ranked.filter(row=>row.group===key);return rows.length?`<section class="${key==='x'?'rb10251-x-bank':''}"><h3>${title}</h3>${rows.map(rowHtml).join('')}</section>`:''}).join('');
    return `<div class="rb1020-modal rb10251-bank-modal" data-rb1020-modal-backdrop="bank"><section class="rb1020-sheet rb10251-bank" role="dialog" aria-modal="true" aria-labelledby="rb10251BankTitle"><header><div><span>Øktbanken</span><h2 id="rb10251BankTitle">Bytt kvalitetsøkt</h2><p>Øktene rangeres rolig ut fra fase, respons og resten av treningsuken.</p></div><button type="button" data-rb1020-modal-close="bank" aria-label="Tilbake til øktdetaljer">${icon('arrowLeft')}</button></header><div class="rb1020-sheet-body">${sections}</div></section></div>`;
  }
  async function replaceQualityWorkout(p,workoutId){
    const candidate=adaptivePlan()?.QUALITY_BANK?.find(row=>row.id===workoutId);if(!p||!candidate||isTerminal(p)||isLocked(p))return toast('Denne økten kan ikke byttes nå.');const weekState=adaptiveWeekState();if(candidate.stimulus==='x'&&['HOLD','DELOAD','RECOVERY'].includes(weekState.mode))return toast('Coachen anbefaler ikke X-belastning med dagens trend.');
  ...
  function responseHistoryHtml(){const current=oneDecision(),memory=current?.memory||coachContinuity()?.memory||{},memoryRows=(memory.recent||[]).slice(0,2);return memoryRows.length?`<details class="rb114-coach-memory"><summary><span>${icon('log')}</span><span><b>Tidligere respons</b><small>${esc(memory.summary||'Tidligere råd og respons er koblet sammen.')}</small></span>${icon('chevronRight')}</summary><div>${memoryRows.map(row=>`<article><time>${esc(row.localDate?formatDate(row.localDate,{day:'numeric',month:'short'}):'Tidligere')}</time><span><b>${esc(row.recommendation)}</b><small>${esc(row.resolution)} · ${esc(row.response)}</small></span></article>`).join('')}</div></details>`:''}
  function coachReasonModalHtml(){
    ...
    return `<div class="rb1020-modal" data-rb1020-modal-backdrop="coach"><section class="rb1020-sheet rb1020-reason-sheet" role="dialog" aria-modal="true" aria-labelledby="rb1020ReasonTitle"><header><div><span>Hvorfor denne økten?</span><h2 id="rb1020ReasonTitle">Derfor valgte coachen denne økten</h2></div><button type="button" data-rb1020-modal-close="coach" aria-label="Lukk coachvurderingen">${icon('close')}</button></header><div class="rb1020-sheet-body"><p class="rb1020-reason-lead">${esc(brief.today?.summary||coach.message)}</p><section class="rb1030-plan-effect"><span>${icon(changed?'swap':'check')}</span><div><small>Planstatus</small><b>${changed?`${affected} ${affected===1?'økt er':'økter er'} berørt`:'Ingen planendring'}</b><p>${esc(brief.week?.nextChange||'Resten av uken står uendret.')}</p></div></section><section><h3>Det som taler for økten</h3><ul>${[...new Set(positives)].slice(0,5).map(x=>`<li>${esc(x)}</li>`).join('')}</ul></section><section><h3>Det vi følger med på</h3><ul>${[...new Set(watch)].slice(0,5).map(x=>`<li>${esc(x)}</li>`).join('')}</ul></section><p class="rb10251-purpose"><b>Dagens mål</b>${esc(purposeFor(base))}</p>${responseHistoryHtml()}<p class="rb1030-next-review"><b>Neste vurdering</b>${esc(nextReviewCopy(brief))}</p><footer><span>${esc(coach.confidence)}</span><b>${esc(coach.sources.length?coach.sources.join(' · '):'plan og tilgjengelig treningshistorikk')}</b></footer></div></section></div>`;
  }
  function todayHtml(){
    const base=planFor(today())||effectiveSchedule().find(p=>p.ds>=today())||effectiveSchedule().at(-1),d=decision(),planStatus=sessionState(base),terminal=planStatus.terminal,coach=coachBasis(),suggest=coach.requiresChoice&&coach.reported?.choice==='pending',pending=!terminal&&manualCandidates(base).length>0;
    const result=planStatus.code==='completed'?resultCardHtml(base,'today'):planStatus.code==='replaced'?replacementCardHtml(base,'today'):terminalStatusCardHtml(base);
    const attention=dailyPresentation(base)?.mode==='attention',primary=terminal?(attention?oneDecisionHeroHtml(base):'')+result:oneDecision()?oneDecisionHeroHtml(base):`${workoutHeroHtml(base)}${todayCoachHtml(base,d,suggest)}`;
    const support=calm()?.chooseSupport({transport:canonicalSyncBannerHtml()||historyNoticeHtml(),change:changeSupportHtml(base),response:recentResponseHtml(),race:raceFocusEntryHtml(true),review:weeklyReviewCardHtml(),priority:weeklyPriorityHtml()})||'';
    return `<div id="rb107Today" class="rb107-surface rb119b rb119b-today"><div class="rb107-shell">${appBarHtml()}<p class="rb126-date">${esc(formatDate(today()))}</p>${viewTitleHtml(terminal?'Godt gjennomført.':new Date().getHours()<12?'God morgen.':new Date().getHours()<18?'God dag.':'God kveld.','')}
      <div class="rb121-today-primary">${primary}${!terminal&&pending?pendingResultHtml(base):''}</div>${terminal?nextStepHtml(base):''}${contextualHealthStripHtml()}${support}
    </div>${qs('.view.active')?.id==='today'&&state.workoutDetailOpen?workoutDetailModalHtml(planFor(state.workoutDetailDs)||base):''}${qs('.view.active')?.id==='today'&&state.workoutBankOpen?workoutBankModalHtml(planFor(state.workoutDetailDs)||base):''}${qs('.view.active')?.id==='today'&&state.coachReasonOpen?coachReasonModalHtml():''}${bodyResponseDetailModalHtml()}${oneDecisionProposalModalHtml()}${weeklyReviewModalHtml()}${syncRepairModalHtml()}${raceFocusModalHtml('today')}</div>`;
  }
  ...
  const correctionDrafts=new Map(),correctionErrors=new Map(),correctionSaving=new Set();
  function correctionHtml(p,a){
    const snap=window.RunnerBearCloudV11?.snapshot?.(),c=progressModel()?.context(a,snap),work=a?.detail?.analysis;if(!p||p.type!=='quality'||!snap?.correctionCapture||!c||!work?.workBlocks?.length||!Number(work.workDuration))return'';
    const key=a.id,busy=correctionSaving.has(key),last=c.latest,draft=correctionDrafts.get(key)?.values,error=correctionErrors.get(key),mode=draft?.mode||(c.active?last.mode:''),pace=draft?.pace||(c.active&&last.mode==='set'?fmtPace(last.paceSeconds):'');
    return `<section class="rb124-correction">${last?`<p role="status"><b>${c.stale?'Kildedataene er oppdatert · kontroller korrigeringen':c.active?last.mode==='set'?`${fmtPace(last.paceSeconds)}/km · oppgitt av deg`:'Arbeidsfart merket upålitelig av deg':'Korrigeringen er angret · originalgrunnlaget brukes'}</b></p>`:''}<details id="rb124-correction-${esc(key)}" ${error?'open':''}><summary>Korriger grunnlaget ${icon('chevronRight')}</summary><form class="rb123-response" data-rb124-correction="${esc(p.baseDs)}"><p>Gjelder bare arbeidsdragene på mølle. Oppvarming, pauser, nedjogg og totaldistanse beholder originaldataene.</p><fieldset ${busy?'disabled':''}><legend>Hvordan skal arbeidsfarten vurderes?</legend><label><input type="radio" name="mode" value="set" ${mode==='set'?'checked':''} required>Samme faktiske fart på alle arbeidsdrag</label><label><input type="radio" name="mode" value="unreliable" ${mode==='unreliable'?'checked':''} required>Farten er upålitelig – utelat fartssammenligning</label><label>Faktisk arbeidsfart (min/km)<input name="pace" type="text" inputmode="text" placeholder="4:00" value="${esc(pace)}" maxlength="5" aria-label="Faktisk arbeidsfart i minutter og sekunder per kilometer"></label></fieldset><p>Lagres i RunnerBear med kilde «Oppgitt av deg». Ingen endring sendes til Garmin eller Tredict.</p><p role="status">${esc(error||'')}</p><button type="submit" class="rb107-button" ${busy?'disabled':''}>${busy?'Lagrer …':'Lagre korrigering'}</button></form>${last&&last.mode!=='clear'?`<button class="rb107-button ghost" data-rb124-undo="${esc(p.baseDs)}" ${busy?'disabled':''}>Angre korrigeringen</button>`:''}</details>${c.history.length?`<details><summary>Korrigeringshistorikk</summary><ul>${c.history.map(r=>`<li>${esc(formatDate(r.savedAt.slice(0,10),{day:'numeric',month:'short'}))} · ${r.mode==='clear'?'Angret':r.mode==='unreliable'?'Arbeidsfart upålitelig':`${fmtPace(r.paceSeconds)}/km · oppgitt av deg`}</li>`).join('')}</ul><small>Viser de fem siste hendelsene. Originaldata og hele historikken er bevart.</small></details>`:''}</section>`;
  }
  async function saveCorrection(p,values){
    const a=activityFor(p)||replacementActivityFor(p),snap=window.RunnerBearCloudV11?.snapshot?.(),c=progressModel()?.context(a,snap);if(!c||correctionSaving.has(a.id))return;
    const input={activityId:c.source.source_id,activitySource:c.source.source,activityVersion:c.source.updated_at,expectedEventId:c.latest?.eventId||'',scope:'work',mode:values.mode};if(values.mode==='set'){input.paceSeconds=progressModel().parsePace(values.pace);if(!input.paceSeconds){correctionDrafts.set(a.id,{values});correctionErrors.set(a.id,'Bruk min:sek, for eksempel 4:00.');renderAll();return}}
    let draft=correctionDrafts.get(a.id);if(!draft?.key||JSON.stringify(draft.input)!==JSON.stringify(input)){draft={values,input,key:`pace:${crypto.randomUUID()}`};correctionDrafts.set(a.id,draft)}correctionSaving.add(a.id);correctionErrors.delete(a.id);renderAll();
    try{await window.RunnerBearCloudV11.submitPaceCorrection(input,draft.key);correctionDrafts.delete(a.id);toast(values.mode==='clear'?'Korrigeringen er angret':'Korrigeringen er lagret')}catch(error){correctionErrors.set(a.id,error?.message||'Lagring er ikke bekreftet. Prøv igjen.')}finally{correctionSaving.delete(a.id);renderAll()}
  }
  ...
  function canonicalSyncSummary(){
    const snapshot=window.RunnerBearCloudV11?.snapshot?.(),rows=canonicalSyncOperations(),ids=new Set(effectiveSchedule().filter(p=>p.ds>=today()&&p.ds<=addDays(today(),13)&&['easy','quality','race'].includes(prescription(p).type)).map(p=>String(p.workoutId||p.baseDs))),latest=[],seen=new Set();for(const row of rows){const id=String(row.workout_id||row.workoutId||'');if(!ids.has(id)||seen.has(id))continue;seen.add(id);latest.push(row)}
    if(!snapshot?.flags?.coach_loop_sync){const mirror=snapshot?.calendarMirror,age=Date.now()-Date.parse(mirror?.last_completed_at),confirmed=age>=-60000&&mirror?.active_plan_revision_id===snapshot?.planRevisionId&&mirror?.last_result==='confirmed'&&age<=6*3600000;return{tone:confirmed?'confirmed':'idle',label:confirmed?'Kalenderen er kontrollert i Tredict':'Kalenderstatus avklares',copy:confirmed?`Planvinduet ${mirror.window_start}–${mirror.window_end} ble kontrollert ${dataTime(mirror.last_completed_at)}. Mottak på selve Garmin-klokken er ikke bekreftet her.`:'Planen er lagret i RunnerBear. Siste kalenderkontroll er ikke bekreftet for gjeldende plan.',action:false,count:0}};
    ...
    if(action)return{tone:'action',label:'Kalenderen oppdateres automatisk',copy:'Vi får ikke oppdatert treningskalenderen akkurat nå. Planen din er trygg i RunnerBear.',action:false,count:latest.length};
    ...
    if(busy&&latest.some(row=>['queued','processing'].includes(row.status)&&calm()?.syncDelayed(row.created_at||row.createdAt||row.updated_at||row.updatedAt)))return{tone:'retry',label:'Kalenderen bruker lengre tid',copy:'Planen er trygg i RunnerBear. Automatisk oppdatering tar lengre tid enn vanlig.',action:false,count:latest.length};
  ...
  function canonicalSyncBannerHtml(){const limited=window.RunnerBearCloudV11?.snapshot?.()?.storage?.writeAvailable===false,sync=limited?{tone:'attention',label:'Lagring midlertidig utilgjengelig',copy:'Du kan se planen. Nye registreringer og endringer kan ikke lagres akkurat nå.'}:canonicalSyncSummary();if(!limited&&!calm()?.exceptionalSync(sync.tone))return'';return`<section class="rb1028-sync-banner ${esc(sync.tone)}" aria-live="polite"><i aria-hidden="true"></i><span><b>${esc(sync.label)}</b><small>${esc(sync.copy)}</small></span></section>`}
  ...
  function garminStatusHtml(p){
    const stateValue=tredictStatusFor(p),labels={confirmed:'Bekreftet i kalenderen',queued:'Kalenderen oppdateres automatisk',processing:'Kalenderen oppdateres automatisk',failed_retryable:'Planen er trygg · vi prøver igjen',failed_terminal:'Planen er trygg i RunnerBear',synced:'Synkronisert',pending:'Kalenderen oppdateres automatisk',syncing:'Kalenderen oppdateres automatisk',awaiting_activation:'Kalenderen oppdateres automatisk',review_required:'Planen er trygg i RunnerBear',error:'Planen er trygg i RunnerBear',not_synced:'Kalenderen oppdateres automatisk'},label=labels[stateValue.status]||labels.not_synced,action=!stateValue.canonical&&stateValue.status==='error';
    if(!calm()?.exceptionalSync(stateValue.status))return'';
  ...
  function planRowHtml(p){
    ...
    return `<button class="rb119b-plan-row ${tone} ${draggable?'rb1023-draggable':''} ${locked?'rb1023-locked':''}" data-rb107-day="${p.ds}" data-rb1023-drag-base="${esc(p.baseDs)}" ${draggable?'draggable="true"':''} aria-label="${esc(`${formatDate(p.ds)} · ${displayCopy(x.title)} · ${status}${draggable?' · kan flyttes':''}`)}"><time><span>${esc(formatDate(p.ds,{weekday:'short'}).replace('.',''))}</span><b>${dateFrom(p.ds).getDate()}</b></time><span class="rb119b-plan-type">${icon(workoutIconName(p))}</span><span class="rb119b-plan-copy"><b>${esc(displayCopy(x.title))}</b><small>${esc([workoutDuration(p).seconds?workoutDuration(p).label:'',x.desc||targetFor(p).main||typeLabel(x.type)].filter(Boolean).join(' · '))}</small></span><span class="rb119b-plan-status"><small>${esc(status)}</small>${planStatus.code==='completed'?icon('check'):locked?icon('lock'):`<i aria-hidden="true"></i>`}</span>${draggable?'<span class="rb1023-drag-handle" aria-hidden="true"></span>':''}</button>`;
  }
  ...
  function contextualPlanNoticeHtml(){
    const surface=contextualCoach()?.surfaces?.plan;if(!surface?.visible)return'';const affected=Number(surface.affectedWorkoutIds?.length||0);
    return `<section class="rb116-plan-notice" aria-live="polite"><span>${icon('swap')}</span><div><small>Coachens planforklaring</small><b>${esc(surface.headline)}</b><p>${esc(surface.summary)}</p>${surface.why?`<em><strong>Hvorfor</strong>${esc(surface.why)}</em>`:''}</div>${affected?`<i>${affected} ${affected===1?'økt':'økter'}</i>`:''}</section>`;
  }
  function planOverviewHtml(week){
    const rows=weekRows(week),s=weekStats(week),plannedRuns=rows.filter(p=>!p.activityOnly&&['easy','quality','race'].includes(prescription(p).type)&&kmFor(p)>0),completedRuns=plannedRuns.filter(p=>isDone(p)&&sportKind(activityFor(p),p)==='run'),completedKm=roundHalf(activities().filter(a=>sportKind(a)==='run'&&a.ds>=rows[0]?.ds&&a.ds<=rows.at(-1)?.ds).reduce((sum,a)=>sum+Number(a.distance||0)/1000,0)),remainingKm=roundHalf(plannedRuns.filter(p=>p.ds>=today()&&!isTerminal(p)).reduce((sum,p)=>sum+kmFor(p),0)),plannedQuality=plannedRuns.filter(p=>['quality','race'].includes(prescription(p).type)),completedQuality=plannedQuality.filter(isDone),plannedEasy=plannedRuns.filter(p=>prescription(p).type==='easy'),completedEasy=plannedEasy.filter(p=>isDone(p)&&sportKind(activityFor(p),p)==='run').length;
    ...
    return `<div class="rb119b-plan-overview rb1025-plan-overview"><article class="rb119b-card rb1025-week-so-far"><header><div><h2>Uken så langt</h2><small>Planlagt, gjennomført og gjenstående holdes adskilt</small></div><strong>${s.km} <span>km planlagt</span></strong></header><div><p><b>${completedKm} km</b> ${historyCovers(rows[0]?.ds)?'gjennomført':'registrert foreløpig'}</p><p><b>${remainingKm} km</b> gjenstår</p><p><b>${completedQuality.length}</b> av ${plannedQuality.length} kvalitetsøkter</p><p><b>${completedEasy}</b> av ${plannedEasy.length} rolige økter</p></div>${s.volumeReason?`<footer><span>Kontrollert avvik</span><b>${esc(s.volumeReason)}</b></footer>`:''}</article></div>`;
  }
  ...
  function dayViewHtml(p){
    ...
    const planned=`<div class="rb1020-day-plan"><span>Planlagt</span><b>${esc(displayCopy(x.title))}</b><small>${esc(plannedAmount)} · ${esc(x.desc||typeLabel(x.type))}</small></div>`;
    ...
    return `<section class="rb119b-card rb1020-day-view ${esc(day.code)}"><header><div><span>${esc(formatDate(p.ds,{weekday:'long',day:'numeric',month:'long'}))}</span><h2>${esc(displayCopy(x.title))}</h2></div><strong>${esc(day.label)}</strong></header>${body}</section>`;
  }
  function planHtml(){
    ...
    const normalPlan=`${canonicalSyncBannerHtml()||historyNoticeHtml()}${contextualPlanNoticeHtml()}<div class="rb119b-segments" role="group" aria-label="Planvisning"><button class="${state.planLens==='week'?'active':''}" data-rb119b-plan-lens="week" aria-pressed="${state.planLens==='week'}">Uke</button><button class="${state.planLens==='long'?'active':''}" data-rb119b-plan-lens="long" aria-pressed="${state.planLens==='long'}">Fremover</button></div>${state.planLens==='week'&&week===current?weeklyPriorityHtml():''}${planListHtml(week,state.planLens)}${state.planLens==='week'?planOverviewHtml(week):''}${weeklyReview()?`<button class="rb119b-completed-link" data-rb1031-week-review>Forrige ukes oppsummering ${icon('chevronRight')}</button>`:''}<button class="rb119b-completed-link" data-rb107-plan-view="done">Gjennomførte økter og coachanalyser ${icon('chevronRight')}</button>`;
    ...
    return `<div id="rb107Plan" class="rb107-surface rb119b rb119b-plan"><div class="rb107-shell">${appBarHtml()}${viewTitleHtml(state.planView==='done'?'Gjennomførte økter':state.planDayViewOpen?'Valgt dag':'Din uke.',state.planView==='done'?'Resultater og coachanalyser':state.planDayViewOpen?formatDate(selected.ds,{weekday:'long',day:'numeric',month:'long'}):subtitle)}
      ${state.planView==='done'?`<button class="rb119b-back" data-rb107-plan-view="plan">${icon('arrowLeft')} Tilbake til plan</button>${completedHtml()}`:`<div class="rb119b-week-controls"><div><button data-rb113-week-step="-1" aria-label="Forrige uke">${icon('chevronLeft')}</button><b>${weekLabel} · ${esc(range)}</b><button data-rb113-week-step="1" aria-label="Neste uke">${icon('chevronRight')}</button></div><button data-rb119c-month-focus aria-label="${state.monthOpen?'Lukk månedsoversikt':'Åpne månedsoversikt'}" aria-expanded="${state.monthOpen}" aria-controls="rb119cMonth">${icon('calendar')}<span>${state.monthOpen?'Lukk månedsoversikt':'Månedsoversikt'}</span></button></div>${state.monthOpen?monthCalendarHtml():''}${state.planDayViewOpen?dayPlan:normalPlan}`}
    </div>${qs('.view.active')?.id==='plan'&&state.workoutDetailOpen?workoutDetailModalHtml(planFor(state.workoutDetailDs)||selected):''}${qs('.view.active')?.id==='plan'&&state.workoutBankOpen?workoutBankModalHtml(planFor(state.workoutDetailDs)||selected):''}${qs('.view.active')?.id==='plan'&&state.movePreview?movePreviewModalHtml():''}${weeklyReviewModalHtml()}${syncRepairModalHtml()}</div>`;
  }
  ...
  function predictionDisclosureHtml(f,rows){return `<details class="rb107-card rb116-disclosure"><summary>Kapasitetsestimater fra originalgrunnlaget</summary><p>Disse estimatene endres ikke av en oppgitt møllefart. Måltiden er ditt mål, ikke en prognose.</p><div class="rb116-prediction-grid">${rows.filter(r=>r.value>0).map(r=>`<div><span>${esc(r.label)}</span><b>${fmtTime(r.value)}</b><small>${esc(r.foundation.level)} grunnlag</small></div>`).join('')||'<p>Ingen bekreftede estimater tilgjengelig.</p>'}</div></details>`}
  ...
  function goalHeroFidelityHtml(goal,g,pred){
    const hero=heroNameForGoal(goal);
    return `<section class="rb126-goal"><div class="rb126-photo" ${heroStyle(hero)}>${heroImage(hero)}</div><small>${esc(goal?distanceMeta(goal.distance).label:'Formbygging')}</small><h2>${esc(goal?.name||'Bygg formen i din rytme')}</h2>${goal?.targetSeconds?`<div class="rb126-goal-target"><b>${esc(fmtTime(goal.targetSeconds))}</b><span>Ditt tidsmål</span></div>`:''}<p>${goal?.date?`${esc(formatDate(goal.date,{day:'numeric',month:'long'}))} · ${Math.max(0,goalDays(goal))} dager igjen`:'Uten løpsdato · jevn trening over tid'}</p></section>`;
  }
  ...
  function goalProgressModel(){const snapshot=window.RunnerBearCloudV11?.snapshot?.(),next=effectiveSchedule().find(p=>p.ds>=today()&&p.type==='quality'&&!isTerminal(p));return progressModel()?.progress({history:activityData(),activities:snapshot?.recentActivities||[],evidence:thresholdEvidence(),today:today(),next})}
  function goalPremiumSummaryHtml(goal,f,pred){
    const m=goalProgressModel();if(!m)return'';return `<section class="rb119b-card rb121-goal-direction rb124-progress"><span class="rb107-overline">På vei mot målet</span><h2>Dette har utviklet seg</h2>${m.observations.map(o=>`<article><h3>${esc(o.title)}</h3><p>${esc(o.copy)}</p><small>${esc(formatDate(o.from,{day:'numeric',month:'short'}))}–${esc(formatDate(o.through,{day:'numeric',month:'short'}))} · ${esc(o.source)}</small></article>`).join('')}${m.reason?`<p>${esc(m.reason)}</p>`:''}${m.next?`<div class="rb124-next"><small>Neste planlagte sjekkpunkt</small><b>${esc(displayCopy(m.next.title))}</b><p>Fra gjeldende plan. Dagens helseavklaring har alltid prioritet.</p><button class="rb107-button ghost" data-rb116-open-gate="${esc(m.next.ds)}">Se økten ${esc(formatDate(m.next.ds,{day:'numeric',month:'short'}))} ${icon('arrowRight')}</button></div>`:'<small>Neste sjekkpunkt vises når en relevant økt finnes i planen.</small>'}</section>`;
  }

  function raceFocusEntryHtml(onToday=false){
    if(!activeGoal())return '';
    const model=raceFocusModel();if(!model?.available||onToday&&!model.todayVisible)return '';
    const checked=raceFocus().checked(localStorage,model.key),count=checked.size,total=raceFocus().checklist.length;
    return `<section class="rb119b-card rb121-support rb122-entry"><small>${model.days===0?'Løpsdag':model.days===1?'1 dag til start':`${model.days} dager til start`}</small><h2>${esc(model.headline)}</h2><p>${model.practicalOnly?'Samle starttid, utstyr og andre praktiske forberedelser før løpet.':onToday?'Øktene frem mot start og det praktiske, samlet på ett sted.':'Praktisk sjekkliste, konkurranseuken og mellomtider fra måltiden.'}</p><button type="button" class="rb107-button secondary" data-rb122-race-open aria-haspopup="dialog">${model.phase==='race'?'Åpne løpsdagen':'Se løpsforberedelsene'} ${icon('arrowRight')}</button>${count?`<small class="rb122-progress">${count} av ${total} praktiske punkter klare · på denne enheten</small>`:''}</section>`;
  }
  function raceFocusModalHtml(view){
    if(!raceFocusOpen||activeView()!==view)return '';
    const model=raceFocusModel(),available=model?.available,checks=available?raceFocus().checked(localStorage,model.key):new Set();
    const target=available?model.target:null;
    const splitLabel=row=>row.finish?'Mål':`${String(row.meters/1000).replace('.',',')} km`;
    const reference=target?`<details class="rb122-reference" id="rb122TargetReference" ${model.phase==='race'?'open':''}><summary><span><b>Mellomtider fra måltiden</b><small>${esc(fmtTime(target.seconds))} · ${esc(fmtPace(target.paceSeconds))}/km i snitt</small></span>${icon('chevronRight')}</summary><div><p>Jevn fart, regnet fra måltiden. Ikke en prognose eller en ny øktinstruks.</p><table><caption>Mellomtider ved jevn målfart</caption><thead><tr><th scope="col">Passering</th><th scope="col">Tid fra start</th></tr></thead><tbody>${target.splits.map(row=>`<tr><th scope="row">${esc(splitLabel(row))}</th><td>${esc(fmtTime(row.seconds))}</td></tr>`).join('')}</tbody></table><p>Bruk arrangørens distansemarkeringer som referanse. GPS-distansen på klokken kan avvike.</p></div></details>`:'';
    return `<div class="rb1020-modal rb122-modal" data-rb1020-modal-backdrop="race-focus"><section class="rb1020-sheet rb122-sheet" role="dialog" aria-modal="true" aria-labelledby="rb122RaceTitle"><header><div><span>Race Focus · ${available?esc(model.name):'Løpsforberedelser'}</span><h2 id="rb122RaceTitle">${available?esc(model.headline):'Grunnlaget oppdateres'}</h2></div><button type="button" data-rb1020-modal-close="race-focus" aria-label="Lukk løpsforberedelsene">${icon('close')}</button></header><div class="rb1020-sheet-body">${available?`
      <section class="rb122-lead"><small>${esc(formatDate(model.raceDate,{weekday:'long',day:'numeric',month:'long'}))}</small><h3>${model.days===0?'I dag':model.days===1?'I morgen':`${model.days} dager igjen`}</h3><p>${esc(model.summary)}</p></section>
      ${model.blocked&&!model.practicalOnly?`<section class="rb122-safety" role="status"><b>Dagens avklaring kommer først</b><p>${esc(model.safetyCopy)}</p><button type="button" class="rb107-button secondary" data-rb122-today>Se dagens coachråd</button></section>`:''}
      ${model.phase==='race'?reference+'<details id="rb122RaceChecklist"><summary><span><b>Det praktiske</b><small>Sjekkliste før start</small></span>'+icon('chevronRight')+'</summary>':''}
      <section class="rb122-checklist">${model.phase==='race'?'':'<h3>Det praktiske</h3>'}<p id="rb122ChecklistHelp">Huskes bare på denne enheten. Endrer ingen økter.</p><div role="group" aria-label="Praktisk sjekkliste" aria-describedby="rb122ChecklistHelp">${raceFocus().checklist.map(item=>`<label><input type="checkbox" data-rb122-check="${item.id}" ${checks.has(item.id)?'checked':''}><span>${esc(item.label)}</span></label>`).join('')}</div><p class="rb122-progress" data-rb122-progress role="status" aria-live="polite">${checks.size} av ${raceFocus().checklist.length} punkter klare</p></section>${model.phase==='race'?'</details>':reference}
      ${!model.blocked&&!target?'<p>Legg inn en måltid under Administrer mål for å se beregnede mellomtider.</p>':''}
      ${model.practicalOnly?'':`<details class="rb122-week" id="rb122RaceWeek"><summary><span><b>Planlagt frem mot start</b><small>Gjenværende økter i siste uke</small></span>${icon('chevronRight')}</summary><div>${model.items.length?`<ul>${model.items.map(item=>`<li><time datetime="${esc(item.date)}">${esc(formatDate(item.date,{weekday:'short',day:'numeric',month:'short'}))}</time><b>${esc(item.title)}</b></li>`).join('')}</ul>`:'<p>Ingen gjenstående økter er lastet inn for konkurranseuken ennå.</p>'}<p>Viser bare det gjeldende planvinduet. Ingen økter legges til eller endres her.</p><button type="button" class="rb107-button secondary" data-rb122-plan="${esc(model.raceDate)}">Se hele planen ${icon('arrowRight')}</button></div></details>`}
      <p class="rb122-footnote">Dagens coachråd og helseavklaring har alltid prioritet. Sjekk starttid og praktiske detaljer hos arrangøren.</p>
    `:`<p>${esc(model?.reason||'Gjeldende mål og plan kunne ikke bekreftes.')}</p>`}</div></section></div>`;
  }
  function goalsHtml(){
    ...
    return `<div id='rb107Goals' class='rb107-surface rb119b rb119b-goals'><div class='rb107-shell'>${appBarHtml()}${viewTitleHtml('Mot startstreken.',subtitle,action)}${goalHeroFidelityHtml(goal,g,pred)}${goalPremiumSummaryHtml(goal,f,pred)}${raceFocusEntryHtml()}<details class="rb119b-card rb10251-goal-foundation"><summary><span>${icon('info')}</span><span><b>Se grunnlaget</b><small>Terskeltrend, volum, spesifisitet og coachens sikkerhet</small></span>${icon('chevronRight')}</summary><div>${goalEvidenceHtml(f,pred)}${thresholdDevelopmentHtml(sessionEvidence)}${predictionDisclosureHtml(f,predictionRows)}</div></details><div class="rb119b-supporting-insight">${secondaryGoalsHtml(g.secondary)}${goalHistoryHtml(g.history)}</div></div>${goalManagerHtml(g)}${raceFocusModalHtml(activeView())}</div>`;
  }

  function logHtml(){
    const rows=read(K.log,[]).filter(row=>row.undo||!/^Form i dag:|^Coachnivå|pensjonert|er aktiv igjen|sko/i.test(row.message||''));if(!rows.length)return'<div class="rb107-empty"><b>Ingen inngrep å vise</b><p>Coachen er stille når planen står.</p></div>';
  ...
  function dataTime(value){if(!value)return'ikke bekreftet';const date=new Date(value);return Number.isFinite(date.getTime())?new Intl.DateTimeFormat('nb-NO',{day:'numeric',month:'short',hour:'2-digit',minute:'2-digit',timeZone:window.RunnerBearCloudV11?.snapshot?.()?.config?.timezone||'Europe/Oslo'}).format(date):'ikke bekreftet'}
  function dataSourcesHtml(sync){const history=activityData(),body=bodyResponse();return `<div class="rb107-data-row"><div><b>Aktiviteter · Garmin og Concept2</b><span>${esc(history.label)}. Sist hentet ${esc(dataTime(history.syncedAt))}.</span><span>${history.latestDate?`Siste registrerte aktivitet: ${esc(formatDate(history.latestDate,{day:'numeric',month:'long'}))}.`:'Siste aktivitet er ikke bekreftet.'}</span></div></div><div class="rb107-data-row"><div><b>Helsedata</b><span>${esc({fresh:'Oppdatert',partial:'Delvis oppdatert',stale:'Venter på nyere målinger'}[body?.freshness?.status]||'Grunnlaget avklares')} · sist hentet ${esc(dataTime(body?.freshness?.syncedAt))}.</span></div></div><div class="rb1023-garmin-summary"><b>${esc(sync.label)}</b><p>${esc(sync.copy)}</p></div>`}
  function moreHtml(){
    const prefs=trainingPreferences(),mode=control(),safeAuto=window.RunnerBearCloudV11?.snapshot?.()?.flags?.coach_loop_safe_auto===true,sync=canonicalSyncSummary();
    ...
    const insight=(id,title,copy,render)=>`<details class="rb119b-setting rb121-insight" data-rb121-insight="${id}" ${insightOpen.has(id)?'open':''}><summary><span>${icon('trend')}</span><div><b>${title}</b><small>${copy}</small></div>${icon('chevronRight')}</summary><div class="rb119b-setting-body">${insightOpen.has(id)?render():''}</div></details>`;
    const authority=mode==='observer'?'Du har valgt at coachen kun observerer. Planen endres ikke.':mode==='autopilot'&&safeAuto?'Trygg autopilot er aktiv for små, reversible reduksjoner.':'Coachen foreslår. Du godkjenner før planen endres.';
    const coachControl=`<p>${esc(authority)}</p>${mode==='observer'?'<button class="rb107-button secondary" data-rb107-control="suggest">La coachen foreslå endringer</button>':''}${safeAuto?`<label class="rb121-auto-toggle"><span><b>Trygg autopilot</b><small>Maks 20 % lavere dose. Ingen økning eller ekstra økter.</small></span><input type="checkbox" role="switch" data-rb121-safe-auto ${mode==='autopilot'?'checked':''} aria-label="Trygg autopilot"></label>`:'<small>Trygg autopilot er ikke tilgjengelig. Ingen automatikk blir aktivert her.</small>'}`;
    return `<div id="rb107More" class="rb107-surface rb119b rb119b-more"><div class="rb107-shell">${appBarHtml()}${viewTitleHtml('Mer','Innsikt og innstillinger.')}<section class="rb119b-tools"><h2>Din trening</h2><button class="rb126-setting-link" data-rb126-history>${icon('log')}<span>Aktivitetshistorikk</span>${icon('chevronRight')}</button>${setting('log','Coachens endringer','Planvalg og endringer',logHtml())}${insight('threshold','Terskelutvikling','Utviklingen i sammenlignbare økter',thresholdHistoryCardHtml)}${insight('intensity','Intensitet og soner','Din styring av fart, puls og følelse',intensityCardHtml)}${insight('distribution','Fordeling av trening','Rolig arbeid og kontrollert kvalitet',intensityDistributionCardHtml)}</section><section class="rb119b-tools"><h2>Ditt oppsett</h2>${setting('profile','Treningspreferanser',`${prefs.runDays.length} løpedager · volum automatisk`,trainingPreferencesHtml())}${setting('link','Tilkoblinger','Status og automatisk oppdatering',dataSourcesHtml(sync))}${setting('plan','Coachens handlingsrom',mode==='autopilot'&&safeAuto?'Trygg autopilot aktiv':mode==='observer'?'Kun observasjon':'Forslag med din godkjenning',coachControl)}</section><section class="rb119b-tools"><h2>Appen</h2>${setting('info','Hjelp','Slik bruker du RunnerBear','<p>Start i I dag for anbefalingen. Åpne økten for detaljer, eller velg Tilpass før du endrer planen. Plan viser kommende og gjennomførte økter. Helsebildet forklarer datagrunnlaget.</p>')}${setting('info','Om RunnerBear',`Versjon ${BUILD} · ${RELEASE.label}`,`<b>RunnerBear ${BUILD}</b><p>Garmin registrerer. RunnerBear tolker og planlegger. Tredict frakter godkjente endringer videre.</p>`)}</section></div>${qs('.view.active')?.id==='more'&&state.intensityExplanationOpen?intensityExplanationModalHtml():''}${syncRepairModalHtml()}</div>`;
  }
  ...
  function mount(id,html){
    ...
    if(!old){section.insertAdjacentHTML('beforeend',html);return}
    const scroll=window.scrollY,modalScroll=qs('.rb1020-sheet-body',old)?.scrollTop||0;
    const details=new Map(qsa('details',old).map(el=>[el.id||qs('summary',el)?.textContent.trim(),el.open]));
    const formKey=form=>form.id||[...form.attributes].filter(a=>a.name.startsWith('data-')&&a.name!=='data-rb121-dirty').map(a=>a.name+'='+a.value).join('|');
    const drafts=new Map(qsa('form[data-rb121-dirty="true"]',old).map(form=>[formKey(form),qsa('input,select,textarea',form).filter(el=>el.type!=='password'&&el.type!=='file').map(el=>({name:el.name,type:el.type,value:el.value,checked:el.checked,start:el.selectionStart,end:el.selectionEnd}))]));
    const active=document.activeElement,focusId=old.contains(active)?active.id:'',focusName=old.contains(active)?active.name:'',focusSelector=old.contains(active)?[...active.attributes].filter(a=>a.name.startsWith('data-')||a.name==='aria-label').map(a=>'['+a.name+'="'+CSS.escape(a.value)+'"]').join(''):'',focusSummary=old.contains(active)&&active.tagName==='SUMMARY'?active.textContent.trim():'';
    old.outerHTML=html;
    const next=qs(`#rb107${surface}`,section);
    qsa('details',next).forEach(el=>{const key=el.id||qs('summary',el)?.textContent.trim();if(details.has(key))el.open=details.get(key)});
    qsa('form',next).forEach(form=>{const draft=drafts.get(formKey(form));if(!draft)return;form.dataset.rb121Dirty='true';qsa('input,select,textarea',form).forEach(el=>{const row=draft.find(r=>r.name===el.name&&r.type===el.type&&(!['checkbox','radio'].includes(r.type)||r.value===el.value));if(!row)return;el.value=row.value;el.checked=row.checked;try{el.setSelectionRange(row.start,row.end)}catch{}})});
    const focus=focusSelector?qs(focusSelector,next):focusSummary?qsa('summary',next).find(el=>el.textContent.trim()===focusSummary):focusId?qs('#'+CSS.escape(focusId),next):focusName?qs('[name="'+CSS.escape(focusName)+'"]',next):null;
  ...
  function moreShellHtml(){return `<div id="rb107More" class="rb107-surface rb119b rb119b-more"><div class="rb107-shell">${appBarHtml()}${viewTitleHtml('Mer','Innsikt og innstillinger.')}<section class="rb1020-insight-group"><h2>Oppdaterer innsikten</h2><article class="rb119b-card"><p>Innsikten gjøres klar.</p></article></section></div></div>`}
  function renderMore(){
  ...
  function switchView(tab,{scroll=true,root=false}={}){
    const target=tab==='goals'&&$('goals')?'goals':tab;
    viewScroll.set(activeView(),window.scrollY);
    if(root&&target===activeView()){state.workoutDetailOpen=false;state.workoutBankOpen=false;state.adaptOpen=false;state.coachReasonOpen=false;state.bodyResponseOpen=false;state.oneDecisionProposalOpen=false;state.weeklyReviewOpen=false;raceFocusOpen=false;if(target==='plan'){state.planView='plan';state.completedId='';state.planDayViewOpen=false;state.monthOpen=false;sessionStorage.setItem(K.planView,'plan')}viewScroll.set(target,0)}
    if(target==='plan'&&!state.selectedDs)openPlanOnToday(false);
    qsa('.view').forEach(v=>v.classList.toggle('active',v.id===target));
    qsa('.navbtn').forEach(n=>{const selected=n.dataset.tab===tab;n.classList.toggle('active',selected);if(selected)n.setAttribute('aria-current','page');else n.removeAttribute('aria-current')});
    renderView(target);
    try{window.dispatchEvent(new CustomEvent('runnerbear:view',{detail:{view:target}}))}catch{}
    if(scroll)requestAnimationFrame(()=>window.scrollTo({top:viewScroll.get(target)||0,behavior:'instant'}));
  }
  ...
  function bind(scope=document){
    ...
    qsa('[data-rb119c-month-focus]').forEach(b=>b.onclick=()=>{state.monthOpen=!state.monthOpen;renderAll();if(state.monthOpen)requestAnimationFrame(()=>$('rb119cMonth')?.focus({preventScroll:false}))});
    qsa('[data-rb113-open-next]').forEach(b=>b.onclick=()=>{const ds=b.dataset.rb113OpenNext;if(!planFor(ds))return;state.selectedDs=ds;sessionStorage.setItem(K.selected,ds);state.openWeek=planFor(ds).week;state.planDayViewOpen=true;state.planView='plan';sessionStorage.setItem(K.planView,'plan');renderAll();switchView('plan')});
    qsa('[data-rb116-open-gate]').forEach(b=>b.onclick=()=>{const ds=b.dataset.rb116OpenGate;if(!planFor(ds))return;state.selectedDs=ds;sessionStorage.setItem(K.selected,ds);state.openWeek=planFor(ds).week;state.planDayViewOpen=true;state.planDetailOpen=false;state.monthOpen=false;state.planView='plan';sessionStorage.setItem(K.planView,'plan');renderAll();switchView('plan')});
    ...
    qsa('[data-rb111-checkin]').forEach(b=>b.onclick=async()=>{const value=b.dataset.rb111Checkin;if(!['fresh','tired','heavy'].includes(value))return;const before=JSON.parse(JSON.stringify(read(K.readiness,{}))),label=b.textContent;b.disabled=true;b.textContent='Lagrer …';try{const reasons=qsa('input[name="rb126-body-reason"]:checked',b.closest('.rb111-checkin')).map(input=>input.value);saveReadinessState({state:reasons.length&&value==='fresh'?'tired':value,reasons});await lastReadinessSubmit;if(!coachLoopUi()){const saved=await window.RunnerBearCloud?.uploadLocal?.(true);if(saved?.ok!==true)throw new Error('Kroppssjekken er ikke bekreftet lagret. Prøv igjen.')}state.bodyResponseOpen=false;trackEvent('body_response_checkin',{state:value});toast('Kroppssjekken er lagret');renderAll()}catch(error){write(K.readiness,before);b.disabled=false;b.textContent=label;toast(error?.message||'Kroppssjekken kunne ikke lagres')}});
    qsa('[data-rb1031-week-review]').forEach(b=>b.onclick=()=>{const review=weeklyReview();if(review)calm()?.markSeen(localStorage,'weekly-review',calm().reviewContent(review),window.RunnerBearCloudV11?.snapshot?.()?.planRevisionId||'');state.weeklyReviewOpen=true;trackEvent('coach_weekly_review_opened');renderAll();requestAnimationFrame(()=>qs('.rb1031-review-sheet [data-rb1020-modal-close]')?.focus())});
    ...
    qsa('[data-rb1020-modal-close]').forEach(b=>b.onclick=()=>{const kind=b.dataset.rb1020ModalClose;if(kind==='workout'&&state.adaptBusy)return;if(kind==='race-focus'){closeRaceFocus();return}if(kind==='workout'){state.adaptPreview=null;state.adaptError='';state.workoutDetailOpen=false;state.adaptOpen=false;responsePhase='post_workout'};if(kind==='bank')closeWorkoutBank();if(kind==='move')state.movePreview=null;if(kind==='coach')state.coachReasonOpen=false;if(kind==='body-response')state.bodyResponseOpen=false;if(kind==='one-decision')state.oneDecisionProposalOpen=false;if(kind==='intensity')state.intensityExplanationOpen=false;if(kind==='goal-insight')state.goalInsightOpen='';if(kind==='weekly-review')state.weeklyReviewOpen=false;if(kind==='sync-repair')state.syncRepairOpen=false;renderAll()});
    qsa('[data-rb1020-modal-backdrop]').forEach(el=>el.onclick=e=>{if(e.target!==el)return;const kind=el.dataset.rb1020ModalBackdrop;if(kind==='workout'&&state.adaptBusy)return;if(kind==='race-focus'){closeRaceFocus();return}if(kind==='workout'){state.adaptPreview=null;state.adaptError='';state.workoutDetailOpen=false;state.adaptOpen=false;responsePhase='post_workout'};if(kind==='bank')closeWorkoutBank();if(kind==='move')state.movePreview=null;if(kind==='coach')state.coachReasonOpen=false;if(kind==='body-response')state.bodyResponseOpen=false;if(kind==='one-decision')state.oneDecisionProposalOpen=false;if(kind==='intensity')state.intensityExplanationOpen=false;if(kind==='goal-insight')state.goalInsightOpen='';if(kind==='weekly-review')state.weeklyReviewOpen=false;if(kind==='sync-repair')state.syncRepairOpen=false;renderAll()});
    ...
    qsa('[data-rb121-change-read]').forEach(button=>button.onclick=()=>{calm()?.markSeen(localStorage,'plan-change',JSON.parse(button.dataset.rb121ChangeRead),window.RunnerBearCloudV11?.snapshot?.()?.planRevisionId||'');renderAll()});
    qsa('[data-rb122-race-open]').forEach(button=>button.onclick=()=>{raceFocusReturnView=activeView();raceFocusOpen=true;renderAll();requestAnimationFrame(()=>qs(`#${CSS.escape(activeView())} .rb122-sheet [data-rb1020-modal-close]`)?.focus())});
    qsa('[data-rb122-check]').forEach(input=>input.onchange=()=>{const model=raceFocusModel();if(!model?.available||!raceFocus().setChecked(localStorage,model.key,input.dataset.rb122Check,input.checked)){input.checked=!input.checked;toast('Kunne ikke huske punktet på denne enheten');return}const progress=qs('[data-rb122-progress]',input.closest('.rb122-sheet'));if(progress)progress.textContent=`${raceFocus().checked(localStorage,model.key).size} av ${raceFocus().checklist.length} punkter klare`});
    qsa('[data-rb122-today]').forEach(button=>button.onclick=()=>{raceFocusOpen=false;switchView('today');requestAnimationFrame(()=>qs('#today [data-rb113-decision-action]')?.focus())});
    qsa('[data-rb122-plan]').forEach(button=>button.onclick=()=>{raceFocusOpen=false;const model=raceFocusModel(),date=model?.items[0]?.date||today();state.selectedDs=date;sessionStorage.setItem(K.selected,date);state.planView='plan';state.planLens='week';switchView('plan')});
    qsa('[data-rb121-insight]').forEach(el=>el.ontoggle=()=>{const id=el.dataset.rb121Insight;if(el.open){if(insightOpen.has(id))return;insightOpen.add(id);const render={threshold:thresholdHistoryCardHtml,intensity:intensityCardHtml,distribution:intensityDistributionCardHtml}[id];if(render){qs('.rb119b-setting-body',el).innerHTML=render();bind(el)}}else insightOpen.delete(id)});
    qsa('[data-rb121-safe-auto]').forEach(input=>input.onchange=()=>{if(window.RunnerBearCloudV11?.snapshot?.()?.flags?.coach_loop_safe_auto!==true){input.checked=false;return}localStorage.setItem(K.control,input.checked?'autopilot':'suggest');moreRenderDirty=true;toast(input.checked?'Trygg autopilot er aktiv':'Coachen venter på din godkjenning');runAutopilot();renderAll()});
    qsa('form').forEach(form=>form.oninput=()=>{form.dataset.rb121Dirty='true'});
    ...
    qsa('[data-rb124-correction]').forEach(form=>form.onsubmit=async e=>{e.preventDefault();await saveCorrection(basePlan(form.dataset.rb124Correction),Object.fromEntries(new FormData(form).entries()))});
    qsa('[data-rb124-undo]').forEach(b=>b.onclick=()=>saveCorrection(basePlan(b.dataset.rb124Undo),{mode:'clear'}));
    qsa('[data-rb126-more]').forEach(b=>b.onclick=()=>switchView('more'));
    qsa('[data-rb126-history]').forEach(b=>b.onclick=()=>{state.planView='done';switchView('plan')});
    qsa('[data-rb126-adapt-open]').forEach(b=>b.onclick=()=>openAdaptation(b.dataset.rb126AdaptOpen));
    qsa('[data-rb126-reason]').forEach(b=>b.onclick=()=>{if(state.adaptBusy)return;state.adaptReason=b.dataset.rb126Reason;state.adaptError='';renderAll()});
    qsa('[data-rb126-mode]').forEach(b=>b.onclick=()=>{state.adaptMode=b.dataset.rb126Mode;state.adaptError='';renderAll()});
    qsa('[data-rb126-minutes]').forEach(b=>b.onclick=()=>{state.adaptMinutes=Number(b.dataset.rb126Minutes);state.adaptError='';renderAll()});
    qsa('[data-rb126-preview]').forEach(b=>b.onclick=()=>previewAdaptation(basePlan(b.dataset.rb126Preview)));
    qsa('[data-rb126-apply]').forEach(b=>b.onclick=applyAdaptation);
    qsa('[data-rb126-preview-back]').forEach(b=>b.onclick=()=>{if(state.adaptBusy)return;state.adaptPreview=null;state.adaptError='';renderAll()});
    qsa('[data-rb126-body]').forEach(b=>b.onclick=()=>{state.adaptOpen=false;state.workoutDetailOpen=false;state.bodyResponseOpen=true;renderAll()});
    qsa('[data-rb126-move]').forEach(b=>b.onclick=()=>{state.adaptOpen=false;state.workoutDetailOpen=false;state.selectedDs=b.dataset.rb126Move;state.planDayViewOpen=true;state.planView='plan';state.moveOpen=true;switchView('plan')});
    qsa('[data-rb126-health-range]').forEach(b=>b.onclick=()=>{state.healthRange=Number(b.dataset.rb126HealthRange);renderAll()});
    qsa('[data-rb123-adapt]').forEach(b=>b.onclick=()=>openAdaptation());
    qsa('[data-rb123-response-open]').forEach(b=>b.onclick=()=>{state.adaptOpen=false;responsePhase=b.dataset.responsePhase;state.workoutDetailDs=b.dataset.rb123ResponseOpen;state.workoutDetailOpen=true;renderAll();requestAnimationFrame(()=>{const input=qs('.rb123-response input,.rb123-response select');input?.focus();input?.scrollIntoView({block:'center',behavior:'smooth'})})});
    qsa('[data-rb123-response-edit]').forEach(b=>b.onclick=()=>{responseEdits.add(b.dataset.rb123ResponseEdit);renderAll()});
    qsa('[data-rb123-response-cancel]').forEach(b=>b.onclick=()=>{const key=b.dataset.rb123ResponseCancel;responseEdits.delete(key);responseDrafts.delete(key);responseErrors.delete(key);renderAll()});
    qsa('[data-rb123-response]').forEach(form=>form.onsubmit=async e=>{
      e.preventDefault();const p=basePlan(form.dataset.rb123Response),c=responseContext(p,form.dataset.responsePhase);if(!c||responseSaving.has(c.key))return;
      const values=Object.fromEntries(new FormData(form).entries());for(const key of ['rpe','pain'])if(values[key]!=null)values[key]=Number(values[key]);
      let draft=responseDrafts.get(c.key);if(!draft||JSON.stringify(draft.values)!==JSON.stringify(values)){draft={values,sourceId:`response:${crypto.randomUUID()}`};responseDrafts.set(c.key,draft)}
      responseSaving.add(c.key);responseErrors.delete(c.key);renderAll();
      try{const submit=window.RunnerBearCloudV11?.submitWorkoutResponse;if(typeof submit!=='function')throw new Error('Lagring er ikke tilgjengelig akkurat nå.');await submit({...values,activityId:c.source.source_id,activitySource:c.source.source,workoutId:c.workout.workoutId,planRevisionId:c.snapshot.planRevisionId,localDate:p.ds,responseDate:today(),responsePhase:c.phase},draft.sourceId);responseEdits.delete(c.key);responseDrafts.delete(c.key);toast('Svaret er lagret')}
      catch(error){responseErrors.set(c.key,error?.message||'Svaret er ikke bekreftet lagret. Prøv igjen.')}finally{responseSaving.delete(c.key);renderAll()}
    });
  ...
  function showBootFailure(){bootFailed=true;qs('.rb108-boot-mark')?.remove();qs('.rb125-skeleton')?.remove();const boot=qs('#rb108Boot .rb108-boot-heading>div');if(!boot)return;boot.innerHTML='<b>Planen kunne ikke lastes</b><span>Forbindelsen til RunnerBear feilet. Prøv igjen om litt.</span><button type="button" class="rb107-button" id="rb1221Retry">Prøv igjen</button>';qs('#rb1221Retry',boot).onclick=()=>location.reload()}
  async function init(){
    mark('runnerbear:init:start');
    if(!engine()||!clarityModel()||!planIntegrity()||!adaptivePlan()){
      console.error(JSON.stringify({event:'runnerbear_fatal_init',build:BUILD,reason:'canonical core unavailable'}));
      showBootFailure();
      return;
    }
  ...
  setTimeout(()=>{
    if(bootFailed||document.documentElement.classList.contains('rb107-ready'))return;
    const boot=qs('#rb108Boot .rb108-boot-heading>div');
