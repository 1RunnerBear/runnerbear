...
(function(){
  ...
  function workoutHeroHtml(base){
    ...
    return `<button type="button" class="rb119b-workout-hero rb1020-workout-hero" data-rb1020-workout-open="${esc(base.ds)}" aria-label="Åpne øktdetaljer for ${esc(displayCopy(p.title))}" ${heroStyle(heroNameForWorkout(base))}><span class="rb1020-hero-status">${esc(status)} ${icon('chevronRight')}</span><span class="rb119b-hero-icon">${icon(workoutIconName(base))}</span><div class="rb119b-workout-copy"><h2>${esc(displayCopy(p.title))}</h2><p>${esc(main)}</p>${recovery?`<small>${esc(recovery)}</small>`:''}</div><div class="rb119b-hero-metrics"><div>${icon('clock')}<span><small>Total tid</small><b>${esc(total)}</b></span></div><div>${icon('pin')}<span><small>Distanse</small><b>${esc(distance)}</b></span></div><div>${icon('goal')}<span><small>Fokus</small><b>${esc(focus)}</b></span></div></div></button>`;
  }
  ...
  function bind(scope=document){
    ...
    qsa('[data-rb116-open-gate]').forEach(b=>b.onclick=()=>{const ds=b.dataset.rb116OpenGate;if(!planFor(ds))return;state.selectedDs=ds;sessionStorage.setItem(K.selected,ds);state.openWeek=planFor(ds).week;state.planView='plan';sessionStorage.setItem(K.planView,'plan');renderAll();switchView('plan')});
  ...
  function showBootFailure(){bootFailed=true;qs('.rb108-boot-mark')?.remove();const boot=qs('#rb108Boot div');if(!boot)return;boot.innerHTML='<b>Planen kunne ikke lastes</b><span>Forbindelsen til RunnerBear feilet. Prøv igjen om litt.</span><button type="button" class="rb107-button" id="rb1221Retry">Prøv igjen</button>';qs('#rb1221Retry',boot).onclick=()=>location.reload()}
  async function init(){
