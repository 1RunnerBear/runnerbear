...
(function(){
  'use strict';
  const RELEASE=window.RunnerBearRelease||{build:'12.3.0',label:'One Truth · One Decision'},BUILD=RELEASE.build;
  ...
  function goalPremiumSummaryHtml(goal,f,pred){
    const context=contextualCoach()?.surfaces?.goal,next=goalGates().next,pending=!activityData().current;
    return `<section class="rb119b-card rb121-goal-direction">${goal?.targetSeconds?`<p class="rb121-goal-target"><span>Måltid</span><b>${esc(fmtTime(goal.targetSeconds))}</b></p>`:''}<span class="rb107-overline">Retning mot målet</span><h2>${esc(pending?'Historikken avklares':context?.visible?context.headline:f.progress.label)}</h2><p>${esc(pending?'Målet står. Retningen vurderes når aktivitetshistorikken er bekreftet.':context?.visible?context.summary:f.progress.copy)}</p>${next?`<button class="rb107-button secondary" data-rb116-open-gate="${esc(next.plan.ds)}">Neste sjekkpunkt · ${esc(formatDate(next.plan.ds,{day:'numeric',month:'short'}))} ${icon('chevronRight')}</button>`:'<small>Neste sjekkpunkt kommer når planen gir et relevant grunnlag.</small>'}</section>`;
  }
