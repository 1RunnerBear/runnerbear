...
(function(){
  'use strict';
  const RELEASE=window.RunnerBearRelease||{build:'12.2.2',label:'One Truth · One Decision'},BUILD=RELEASE.build;
  ...
  function oneDecisionHeroHtml(base){
    ...
    const contextual=contextualCoach()?.surfaces?.today,workout=prescription(base),target=targetFor(base),labels={follow:'Dagens råd',adjust:'Trygg justering',clarify:'Trenger ett svar',refresh:'Grunnlaget oppdateres',reflect:'Etter økten',completed:'Dagens status',rest:'Planlagt restitusjon'},status=labels[current.state]||'Dagens råd',evidence=(current.evidence||[]).slice(0,2),continuity=coachContinuity(),memory=current.memory||continuity?.memory||{},memoryRows=['adjust','clarify','reflect'].includes(current.state)?(memory.recent||[]).slice(0,2):[],confidence=current.confidence||continuity?.confidence,hasBasis=!!bodyResponse(),freshness=current.freshness==='current'?'Gjeldende plan':'Grunnlaget fornyes',headline=contextual?.visible?contextual.headline:current.headline,summary=contextual?.visible?contextual.summary:current.summary,primaryAction=current.primaryAction?.kind==='refresh_data'?(base?{kind:'open_workout',label:'Åpne dagens økt'}:{kind:'view_plan',label:'Se planen'}):current.primaryAction,memoryHtml=memoryRows.length?`<details class="rb114-coach-memory"><summary><span>${icon('log')}</span><span><b>Tidligere respons</b><small>${esc(memory.summary||'Tidligere råd og respons er koblet sammen.')}</small></span>${icon('chevronRight')}</summary><div>${memoryRows.map(row=>`<article><time>${esc(row.localDate?formatDate(row.localDate,{day:'numeric',month:'short'}):'Tidligere')}</time><span><b>${esc(row.recommendation)}</b><small>${esc(row.resolution)} · ${esc(row.response)}</small></span></article>`).join('')}</div></details>`:'';
    return `<section class="rb113-one-decision rb116-contextual ${esc(current.tone||'calm')} ${esc(current.state)}" aria-labelledby="rb113DecisionTitle" aria-live="polite"><header><span class="rb113-decision-status"><i aria-hidden="true"></i>${esc(status)}</span>${current.freshness!=='current'?`<small>${esc(freshness)}</small>`:''}</header><div class="rb113-decision-copy"><h2 id="rb113DecisionTitle">${esc(headline)}</h2><p>${esc(summary)}</p></div>${base?`<div class="rb113-workout-context"><span>${icon(workoutIconName(base))}</span><span><small>${esc(formatDate(base.ds,{weekday:'long',day:'numeric',month:'long'}))}</small><b>${esc(workout.title||base.title)}</b>${target.main&&target.main!==workout.title?`<em>${esc(target.main)}</em>`:''}</span></div>`:''}${evidence.length?`<ul class="rb113-evidence" aria-label="Beslutningsgrunnlag">${evidence.map(row=>`<li class="${esc(row.tone||'neutral')}"><i aria-hidden="true"></i><span><b>${esc(row.label)}</b><small>${esc(row.source)}</small></span></li>`).join('')}</ul>`:''}${memoryHtml}<div class="rb113-decision-actions"><button type="button" class="rb113-primary" data-rb113-decision-action="${esc(primaryAction?.kind||'view_plan')}" ${state.oneDecisionResolving?'disabled':''}>${state.oneDecisionResolving?'Lagrer …':esc(primaryAction?.label||'Se planen')}${state.oneDecisionResolving?'':icon('arrowRight')}</button></div></section>`;
  }
  ...
  function contextualHealthStripHtml(){
    ...
    if(!attention)return `<button type="button" class="rb116-health-strip normal rb121-health-compact" data-rb111-body-open aria-label="Åpne helsebildet"><span>${icon('heart')}</span><b>${esc(headline)}</b>${icon('chevronRight')}</button>`;
  ...
  function bodyResponseDetailModalHtml(){
    ...
    return `<div class="rb1020-modal rb111-body-modal" data-rb1020-modal-backdrop="body-response"><section class="rb1020-sheet rb111-body-sheet" role="dialog" aria-modal="true" aria-labelledby="rb111BodyTitle"><header><div><span>Kroppens respons</span><h2 id="rb111BodyTitle">Hvor mye tåler kroppen i dag?</h2><p>Bakken-prinsippene styrer hvordan du trener. Responsbildet setter dagens dose.</p></div><button type="button" data-rb1020-modal-close="body-response" aria-label="Lukk helsebildet">${icon('close')}</button></header><div class="rb1020-sheet-body"><section class="rb111-body-lead ${esc(body.state)}"><span>${icon(body.state==='as_planned'?'check':body.state==='wait_for_data'?'sync':'heartbeat')}</span><div><small>${esc(body.stateLabel)} · ${esc(body.confidence==='high'?'høy trygghet':body.confidence==='medium'?'middels trygghet':'begrenset grunnlag')}</small><h3>${esc(body.recommendedAction?.label||'Planen står')}</h3><p>${esc(body.summary)}</p></div></section><section class="rb111-metric-grid">${metricRows.map(([label,copy,metric])=>`<article><header><small>${esc(label)}</small><b>${esc(bodyMetricTrend(metric,'current'))}</b></header><p>${esc(copy)}</p><dl><div><dt>Personlig normal</dt><dd>${esc(bodyMetricTrend(metric,'baseline'))}</dd></div><div><dt>7 dager</dt><dd>${esc(bodyMetricTrend(metric,'trend7'))}</dd></div><div><dt>28 dager</dt><dd>${esc(bodyMetricTrend(metric,'trend28'))}</dd></div></dl></article>`).join('')}</section><section class="rb111-domains"><header><div><small>Beslutningsgrunnlag</small><h3>Fem domener · maks én stemme hver</h3></div><span>Ingen dobbelttelling</span></header>${(body.domains||[]).map(row=>`<div><span>${icon(domainIcons[row.id]||'info')}</span><div><b>${esc(row.label)}</b><small>${esc(row.summary)}</small></div><em class="${esc(row.status)}"><i></i>${esc(statusLabels[row.status]||row.status)}</em></div>`).join('')}</section>${body.checkIn?.required?`<section class="rb111-checkin"><small>Kort kroppssjekk</small><h3>${esc(body.checkIn.prompt)}</h3><div>${(body.checkIn.options||[]).map(option=>`<button type="button" data-rb111-checkin="${esc(option.value)}">${esc(option.label)}</button>`).join('')}</div></section>`:''}<section class="rb111-data-note"><span>${icon('info')}</span><p><b>Personlig trend, ikke diagnose.</b> Median og robust variasjon fra inntil 28 dager brukes som din normal. Ett lavt HRV-signal endrer ikke planen alene. Sist synkronisert ${esc(updated)}.</p></section></div></section></div>`;
  }
  ...
  function weeklyReviewCardHtml(){
    ...
    return `<section class="rb121-support rb119b-card"><span class="rb107-overline">Ny ukereview</span><h2>${esc(review.headline)}</h2><p>${esc(review.coachComment||review.nextDirection)}</p><button class="rb107-button secondary" type="button" data-rb1031-week-review>Les ukereview ${icon('chevronRight')}</button></section>`;
  }
  ...
  function plannedWorkoutHtml(base,d,suggest){
    ...
    return `<article class="rb107-card rb107-workout"><div class="rb107-workout-top"><div><span class="rb107-overline rb117-state">Dagens plan · før økten</span><span class="rb107-type ${p.type}">${typeLabel(p.type)}</span><h2>${esc(p.title)}</h2><p class="rb107-workout-lead">${esc(p.desc||'')}</p></div></div>
      ...
      <div class="rb107-workout-body"><div class="rb107-purpose"><span>Hensikt</span><p>${esc(purposeFor(p))}</p></div>${flexHtml(base)}${!canOneOff(base)&&!flexible(base)?matchPickerHtml(base):''}
        ...
        <details class="rb107-details" id="rb107TodayDetails"><summary>Planlagt økt og gjennomføring</summary><div class="rb107-detail-copy">${workoutStructureHtml(base)}<div class="rb107-detail-row"><b>Gjennomføring</b><span>${esc(p.detail||p.desc||'Følg kontrollert belastning.')}</span></div>${p.fuel?`<div class="rb107-detail-row"><b>Energi</b><span>${esc(p.fuel)}</span></div>`:''}<div class="rb107-detail-row"><b>Hvorfor nå</b><span>${esc(suggest?'RunnerBear anbefaler en tryggere dag.':d.message)} Konservativ tolkning vinner når puls, pust og følelse spriker.</span></div></div></details>
      </div></article>`;
  }
  ...
  function todayHtml(){
    ...
    const primary=oneDecision()?oneDecisionHeroHtml(base):`${workoutHeroHtml(base)}${todayCoachHtml(base,d,suggest)}`;
    const support=calm()?.chooseSupport({transport:canonicalSyncBannerHtml()||historyNoticeHtml(),change:changeSupportHtml(base),race:raceFocusEntryHtml(true),review:weeklyReviewCardHtml(),priority:weeklyPriorityHtml()})||'';
    return `<div id="rb107Today" class="rb107-surface rb119b rb119b-today"><div class="rb107-shell">${appBarHtml()}${raceMetaHtml()}${viewTitleHtml(terminal?'Dagens resultat':'Dagens økt',terminal?planStatus.label:'')}
      <div class="rb121-today-primary">${primary}${terminal?result:pending?pendingResultHtml(base):''}</div>${contextualHealthStripHtml()}${support}
  ...
  function planHtml(){
    ...
    const normalPlan=`${canonicalSyncBannerHtml()||historyNoticeHtml()}${contextualPlanNoticeHtml()}<div class="rb119b-segments" role="group" aria-label="Planvisning"><button class="${state.planLens==='week'?'active':''}" data-rb119b-plan-lens="week" aria-pressed="${state.planLens==='week'}">Uke</button><button class="${state.planLens==='long'?'active':''}" data-rb119b-plan-lens="long" aria-pressed="${state.planLens==='long'}">Langsiktig</button></div>${state.planLens==='week'&&week===current?weeklyPriorityHtml():''}${planListHtml(week,state.planLens)}${state.planLens==='week'?planOverviewHtml(week):''}${weeklyReview()?`<button class="rb119b-completed-link" data-rb1031-week-review>Forrige ukes review ${icon('chevronRight')}</button>`:''}<button class="rb119b-completed-link" data-rb107-plan-view="done">Gjennomførte økter og coachanalyser ${icon('chevronRight')}</button>`;
