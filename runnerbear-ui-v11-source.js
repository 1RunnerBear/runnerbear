/* RunnerBear v11.5.0 · Premium UX Lock within locked Design Direction 1.0
...
(function(){
  ...
  function trackEvent(name,detail={}){
    const payload={event:name,runnerbearVersion:'11.5.0',...detail};
  ...
  function coachLiveDecisionActionHtml(){const current=oneDecision(),action=current?.primaryAction;if(state.coachLiveSurface!=='today'||!action||!['review_adjustment','complete_checkin','complete_feedback','refresh_data'].includes(action.kind))return'';return `<aside class="rb113-coach-action rb115-coach-context"><span>${icon(action.kind==='review_adjustment'?'swap':action.kind==='complete_checkin'?'heartbeat':action.kind==='complete_feedback'?'message':'sync')}</span><div><small>One Decision · akkurat nå</small><b>${esc(current.headline)}</b><p>${esc(current.summary)}</p></div><button type="button" data-rb113-decision-action="${esc(action.kind)}">${esc(action.label)}</button></aside>`}
  ...
  function coachLiveAnswerHtml(content){
    const lines=String(content||'').replace(/\r/g,'').split('\n'),sections=[];let current={title:'',lines:[]};
    const flush=()=>{if(current.lines.some(line=>line.trim()))sections.push(current);current={title:'',lines:[]}};
    for(const line of lines){const match=line.trim().match(/^(?:#{1,3}\s*)?(?:\*\*)?(Mitt råd|Hvorfor|Planen)(?:\*\*)?\s*:?\s*(.*)$/i);if(match){flush();current={title:match[1].toLowerCase()==='mitt råd'?'Mitt råd':match[1][0].toUpperCase()+match[1].slice(1).toLowerCase(),lines:match[2]?[match[2]]:[]}}else current.lines.push(line)}flush();
    const renderLines=rows=>{let html='',list=[];const closeList=()=>{if(list.length){html+=`<ul>${list.map(item=>`<li>${esc(item)}</li>`).join('')}</ul>`;list=[]}};for(const row of rows){const value=row.trim();if(!value){closeList();continue}const bullet=value.match(/^[-*•]\s+(.+)/);if(bullet){list.push(bullet[1]);continue}closeList();html+=`<p>${esc(value)}</p>`}closeList();return html};
    return sections.length?sections.map((section,index)=>`<section class="rb115-coach-section${index===0?' primary':''}">${section.title?`<h3>${esc(section.title)}</h3>`:''}${renderLines(section.lines)}</section>`).join(''):'';
  }
  function coachLiveMessageHtml(message){const pending=message.pending||message.status==='running'?' pending':'',failed=message.status==='failed',retry=failed&&message.retryable&&message.in_reply_to,assistant=message.role==='assistant';return `<article class="rb112-message ${esc(message.role)}${pending}${failed?' failed':''}" data-rb112-message="${esc(message.message_id)}"><header>${assistant?`<span aria-hidden="true">${icon(failed?'info':'message')}</span>`:''}<small>${message.role==='user'?'Du':failed?'Svar ikke levert':'Coach Live'}</small></header><div class="rb112-message-content">${assistant?coachLiveAnswerHtml(message.content||''):esc(message.content||'')}</div>${pending?'<div class="rb115-coach-thinking" role="status"><span>Vurderer planen</span><i aria-hidden="true"><b></b><b></b><b></b></i></div>':''}${retry?`<button type="button" class="rb1141-coach-retry" data-rb1141-coach-retry="${esc(message.message_id)}">Prøv igjen</button>`:''}</article>`}
  function coachLiveModalHtml(){
    if(!state.coachLiveOpen)return'';const messages=state.coachLiveMessages,empty=!state.coachLiveLoading&&!messages.length,meta={today:['Dagens beslutning','Hva trenger du hjelp med?'],workout:['Denne økten','Hvordan vil du løse økten?'],body_response:['Helsebildet','Hva vil du forstå bedre?'],plan:['Gjeldende plan','Hva vil du forstå i planen?'],goals:['Målet ditt','Hva vil du vite frem mot målet?'],more:['RunnerBear','Hva vil du ha råd om?']}[state.coachLiveSurface]||['Gjeldende plan','Hva trenger du hjelp med?'],connected=!!window.RunnerBearCloudV11?.snapshot?.()?.planRevisionId;
    return `<div class="rb1020-modal rb112-coach-modal rb115-modal" data-rb1020-modal-backdrop="coach-live"><section class="rb1020-sheet rb112-coach-sheet rb115-coach-sheet" role="dialog" aria-modal="true" aria-labelledby="rb112CoachTitle" aria-describedby="rb115CoachTrust"><header><div class="rb115-coach-heading"><span>${icon('message')} Coach Live</span><h2 id="rb112CoachTitle">${esc(meta[1])}</h2><p>${esc(meta[0])} · ${connected?'koblet til gjeldende plan':'planen kontrolleres ved sending'}</p></div><div class="rb115-coach-tools"><button type="button" data-rb112-coach-new aria-label="Start ny samtale">${icon('message')}<span>Ny</span></button><button type="button" data-rb1020-modal-close="coach-live" aria-label="Lukk Coach Live">${icon('close')}</button></div></header>${coachLiveDecisionActionHtml()}<div class="rb112-conversation" data-rb112-conversation role="log" aria-live="polite" aria-relevant="additions text" aria-busy="${state.coachLiveSending?'true':'false'}">${state.coachLiveLoading?'<div class="rb112-loading" role="status"><i></i><span>Henter samtalen …</span></div>':''}${empty?`<section class="rb112-welcome"><span>${icon('message')}</span><small>Personlig råd · planen er kontekst</small><h3>Start med det viktigste</h3><p>Velg et forslag eller skriv med egne ord. Coach Live forklarer hva du bør gjøre, hvorfor og hva det betyr for planen.</p><div>${coachLiveStarters().map((text,index)=>`<button type="button" class="${index===0?'recommended':''}" data-rb112-coach-starter="${esc(text)}">${index===0?'<span>Anbefalt</span>':''}${esc(text)}${icon('arrowRight')}</button>`).join('')}</div></section>`:''}${messages.map(coachLiveMessageHtml).join('')}${state.coachLiveError?`<div class="rb112-error" role="alert"><b>Coach Live er ikke tilgjengelig akkurat nå</b><span>${esc(state.coachLiveError)}</span></div>`:''}</div><form class="rb112-composer" data-rb112-coach-form><label for="rb112CoachInput">Spør Coach Live</label><div><textarea id="rb112CoachInput" name="message" rows="1" maxlength="1200" enterkeyhint="send" placeholder="Skriv spørsmålet ditt …" aria-describedby="rb115CoachTrust" ${state.coachLiveSending?'disabled':''}></textarea><button type="submit" aria-label="Send spørsmål" ${state.coachLiveSending?'disabled':''}>${state.coachLiveSending?'<i></i>':icon('arrowRight')}</button></div><small id="rb115CoachTrust"><span>${icon('shield')} Råd, ikke diagnose</span><span>Ingen plan endres her</span><span class="rb115-keyboard-help">Enter sender · Shift + Enter gir ny linje</span></small></form></section></div>`;
  }
  ...
  function coachLiveStreamEvent(block){let text='',completed=false,error='';for(const line of String(block||'').split(/\r?\n/)){if(!line.startsWith('data:'))continue;const data=line.slice(5).trim();if(!data||data==='[DONE]'){if(data==='[DONE]')completed=true;continue}try{const item=JSON.parse(data);if(item.type==='delta'&&typeof item.text==='string')text+=item.text;else if(item.type==='completed')completed=true;else if(item.type==='failed')error=item.message||'Coach Live kunne ikke fullføre svaret.';else{const delta=item.response??item.delta?.content??item.choices?.[0]?.delta?.content??'';if(typeof delta==='string')text+=delta}}catch{}}return{text,completed,error}}
  function updateCoachLiveTurn(messageId){const node=qs(`[data-rb112-message="${CSS.escape(messageId)}"] .rb112-message-content`);const message=state.coachLiveMessages.find(row=>row.message_id===messageId);if(node&&message)node.innerHTML=message.role==='assistant'?coachLiveAnswerHtml(message.content):esc(message.content);const log=qs('[data-rb112-conversation]');if(log)log.scrollTop=log.scrollHeight}
  ...
  async function sendCoachLiveMessage(value){
    ...
    try{
      ...
      state.coachLiveThreadId=response.headers.get('x-runnerbear-thread-id')||state.coachLiveThreadId;if(!response.ok){const body=await response.json().catch(()=>({}));throw new Error(body.error||'Coach Live fikk ikke laget et svar.')}const reader=response.body?.getReader();if(!reader)throw new Error('Coach Live mangler svarstrøm.');const decoder=new TextDecoder();let buffer='';
      while(true){const{done,value:chunk}=await reader.read();buffer+=decoder.decode(chunk||new Uint8Array(),{stream:!done});const blocks=buffer.split(/\r?\n\r?\n/);buffer=blocks.pop()||'';for(const block of blocks){const event=coachLiveStreamEvent(block);if(event.error)throw new Error(event.error);if(event.text){const turn=state.coachLiveMessages.find(row=>row.message_id===assistantId);if(turn)turn.content+=event.text;updateCoachLiveTurn(assistantId)}}if(done)break}
      const tail=coachLiveStreamEvent(buffer),turn=state.coachLiveMessages.find(row=>row.message_id===assistantId);if(tail.error)throw new Error(tail.error);if(tail.text&&turn)turn.content+=tail.text;if(!turn?.content)throw new Error('Coach Live returnerte et tomt svar.');turn.pending=false;turn.status='completed';
    }catch(error){const turn=state.coachLiveMessages.find(row=>row.message_id===assistantId);if(turn){turn.pending=false;turn.status='failed';turn.retryable=true;turn.in_reply_to=userId;turn.content=error?.message||'Coach Live fikk ikke laget et svar. Spørsmålet er bevart, og du kan prøve igjen.'}state.coachLiveError=''}
  ...
  function bind(scope=document){
    ...
    qsa('[data-rb112-coach-form]').forEach(form=>{const textarea=form.elements.message,resize=()=>{textarea.style.height='auto';textarea.style.height=`${Math.min(textarea.scrollHeight,126)}px`};textarea.oninput=resize;textarea.onkeydown=e=>{if(e.key==='Enter'&&!e.shiftKey&&!e.isComposing){e.preventDefault();form.requestSubmit()}};form.onsubmit=e=>{e.preventDefault();const message=textarea.value;textarea.value='';resize();sendCoachLiveMessage(message)}});
    qsa('[data-rb1141-coach-retry]').forEach(b=>b.onclick=()=>{const failed=state.coachLiveMessages.find(row=>row.message_id===b.dataset.rb1141CoachRetry),user=failed&&state.coachLiveMessages.find(row=>row.message_id===failed.in_reply_to);if(!user?.content)return;if(failed)failed.retryable=false;trackEvent('coach_live_retry',{surface:state.coachLiveSurface});renderAll();sendCoachLiveMessage(user.content)});
