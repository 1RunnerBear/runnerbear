/* RunnerBear v11.2.0 · Coach Live within locked Design Direction 1.0
...
(function(){
  ...
  function trackEvent(name,detail={}){
    const payload={event:name,runnerbearVersion:'11.2.0',...detail};
  ...
  function runAutopilot(){
    ...
    window.RunnerBearCloudV11?.applySafeAuto?.().catch(error=>console.warn(JSON.stringify({event:'coach_loop_auto_failed',build:'11.2.0',message:error?.message||String(error)})));
  }
  ...
  function todayHtml(){
    ...
    return `<div id="rb107Today" class="rb107-surface rb119b rb119b-today"><div class="rb107-shell">${appBarHtml()}${raceMetaHtml()}${viewTitleHtml(terminal?'Dagens resultat':'Dagens økt',terminal?planStatus.label:'')}
      ${workoutHeroHtml(base)}${todayHealthCardHtml()}${todayCoachHtml(base,d,suggest)}${coachLiveCardHtml()}${weeklyPriorityHtml()}${weeklyReviewCardHtml()}${changeNoticeHtml(base)}${terminal?result:`${pending?pendingResultHtml(base):''}`}
      <div class="rb119b-sync ${sync.stale?'stale':''}"><i></i><span>${esc(sync.label)}</span></div>
    </div>${qs('.view.active')?.id==='today'&&state.workoutDetailOpen?workoutDetailModalHtml(planFor(state.workoutDetailDs)||base):''}${qs('.view.active')?.id==='today'&&state.workoutBankOpen?workoutBankModalHtml(planFor(state.workoutDetailDs)||base):''}${qs('.view.active')?.id==='today'&&state.coachReasonOpen?coachReasonModalHtml():''}${bodyResponseDetailModalHtml()}${weeklyReviewModalHtml()}${syncRepairModalHtml()}${coachLiveModalHtml()}</div>`;
  }
  ...
  function finishRender(id,startMark){
    ...
    document.body.classList.toggle('rb109-modal-open',state.goalManagerOpen||state.workoutDetailOpen||state.workoutBankOpen||!!state.movePreview||state.coachReasonOpen||state.bodyResponseOpen||state.intensityExplanationOpen||state.weeklyReviewOpen||state.syncRepairOpen||state.coachLiveOpen);
    bind($(id));
  ...
  async function init(){
    ...
    if(!engine()||!clarityModel()||!planIntegrity()||!adaptivePlan()){
      console.error(JSON.stringify({event:'runnerbear_fatal_init',build:'11.2.0',reason:'canonical core unavailable'}));
