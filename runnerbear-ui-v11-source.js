...
(function(){
  ...
  function canonicalSyncSummary(){
    ...
    if(action){const activation=errors.includes('PLAN_ACTIVATION_REQUIRED')||errors.includes('SOURCE_NOT_FOUND');return{tone:'action',label:activation?'Tredict trenger oppmerksomhet':'Tredict trenger kontroll',copy:activation?'RunnerBear-planen er lagret og følges opp automatisk når Tredict-kalenderen er aktiv.':'RunnerBear-planen er riktig. Synken fortsetter automatisk når Tredict er tilgjengelig.',action:false,count:latest.length}}
  ...
  function canonicalSyncBannerHtml(){if(!coachLoopUi())return'';const sync=canonicalSyncSummary(),repair=syncRepair(),needsAction=repair?.attention==='action';return`<section class="rb1028-sync-banner ${esc(sync.tone)}" aria-live="polite"><i aria-hidden="true"></i><span><b>${esc(sync.label)}</b><small>${esc(needsAction?repair.summary:sync.copy)}</small></span>${needsAction?'<button type="button" data-rb1031-sync-repair>Åpne kontroll</button>':''}</section>`}
