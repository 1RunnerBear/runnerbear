/* RunnerBear v11.4.1 · Reliability Patch within locked Design Direction 1.0
...
(function(){
  ...
  function trackEvent(name,detail={}){
    const payload={event:name,runnerbearVersion:'11.4.1',...detail};
  ...
  function runAutopilot(){
    ...
    window.RunnerBearCloudV11?.applySafeAuto?.().catch(error=>console.warn(JSON.stringify({event:'coach_loop_auto_failed',build:'11.4.1',message:error?.message||String(error)})));
  }
  ...
  function coachLiveMessageHtml(message){const pending=message.pending||message.status==='running'?' pending':'',failed=message.status==='failed',retry=failed&&message.retryable&&message.in_reply_to;return `<article class="rb112-message ${esc(message.role)}${pending}${failed?' failed':''}" data-rb112-message="${esc(message.message_id)}"><small>${message.role==='user'?'Du':failed?'Svar ikke levert':'Coach Live'}</small><div class="rb112-message-content">${esc(message.content||'')}</div>${pending?'<i aria-label="Coach Live skriver"><span></span><span></span><span></span></i>':''}${retry?`<button type="button" class="rb1141-coach-retry" data-rb1141-coach-retry="${esc(message.message_id)}">Prøv igjen</button>`:''}</article>`}
  ...
  async function init(){
    ...
    if(!engine()||!clarityModel()||!planIntegrity()||!adaptivePlan()){
      console.error(JSON.stringify({event:'runnerbear_fatal_init',build:'11.4.1',reason:'canonical core unavailable'}));
