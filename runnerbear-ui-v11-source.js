...
(function(){
  'use strict';
  const RELEASE=window.RunnerBearRelease||{build:'12.4.0',label:'One Truth · One Decision'},BUILD=RELEASE.build;
  ...
  async function adapt(p,reason){
    ...
    write(K.adjustments,all);try{await persistDayChoice(before,'day-'+reason)}catch(error){toast(error.message||'Valget kunne ikke lagres.');return false}const changed=basePlan(p.baseDs);queueTredictMutation(reason==='skip'?'plan:workout-cancelled':'plan:workout-adjusted',[changed],{previousRows,previousDate:p.ds,reason});addLog(`${p.title} er tilpasset: ${reason==='time'?'kortere tid':reason==='tired'?'sliten kropp':reason==='achilles'?'akillesrespons':'økten utgår'}.`,'manual',before);state.adaptOpen=false;state.workoutDetailOpen=false;toast('Valget er lagret i RunnerBear');renderAll();
  }
  ...
  const state={selectedDs:sessionStorage.getItem(K.selected)||'',planView:savedPlanView==='done'?'done':'plan',planLens:['week','long'].includes(savedPlanLens)?savedPlanLens:'week',openWeek:null,monthKey:'',monthOpen:false,planDayViewOpen:false,planDetailOpen:false,moveOpen:false,movePreview:null,moveSaving:false,adaptOpen:false,completedId:'',doneScroll:0,goalManagerOpen:false,goalEditor:'',goalInsightOpen:'',workoutDetailOpen:false,workoutDetailDs:'',workoutBankOpen:false,workoutBankReturnToDetail:false,coachReasonOpen:false,bodyResponseOpen:false,oneDecisionProposalOpen:false,oneDecisionResolving:false,intensityExplanationOpen:false,weeklyReviewOpen:false,syncRepairOpen:false};
  ...
  function oneDecisionHeroHtml(base){
    const current=oneDecision();if(!current)return'';const view=dailyPresentation(base),normal=['workout','rest'].includes(view?.mode),workout=prescription(base),target=targetFor(base),evidence=(current.evidence||[]).slice(0,2),fresh=runningDay()?.fresh(current),summary=normal?purposeFor(workout):fresh?current.summary:'Siste plan kan åpnes mens dagens vurdering fornyes automatisk.',action=view?.action||'view_plan';
    return `<section class="rb113-one-decision rb116-contextual ${normal?'calm':esc(current.tone||'watch')} ${esc(current.state)}" aria-labelledby="rb113DecisionTitle"><header><span class="rb113-decision-status"><i aria-hidden="true"></i>${normal?view.mode==='rest'?'Planlagt restitusjon':'Dagens oppgave':'Dagens avklaring'}</span></header><div class="rb113-decision-copy"><h2 id="rb113DecisionTitle">${esc(displayCopy(view?.headline||current.headline))}</h2><p>${esc(summary)}</p></div>${normal&&view.mode!=='rest'?`<div class="rb113-workout-context"><span>${icon(workoutIconName(base))}</span><span><small>Styring fra planen</small><b>${esc(target.hr)}</b><em>${esc(target.pace)}</em></span></div>`:!normal&&base?`<p class="rb123-planned">Planlagt: ${esc(displayCopy(workout.title))}</p>`:''}<div class="rb113-decision-actions"><button type="button" class="rb113-primary" data-rb113-decision-action="${esc(action)}" ${state.oneDecisionResolving?'disabled':''}>${state.oneDecisionResolving?'Lagrer …':esc(view?.label||'Se planen')}${icon('arrowRight')}</button>${normal&&base?.ds===today()&&!isTerminal(base)?'<button type="button" class="rb107-button secondary" data-rb123-adapt>Tilpass i dag</button>':''}</div>${normal?`<details class="rb121-workout-why"><summary>Hvorfor denne økten? ${icon('chevronRight')}</summary><p>${esc(keyAdvice(base))}</p>${evidence.length?`<ul>${evidence.map(row=>`<li>${esc(row.label)} · ${esc(row.source)}</li>`).join('')}</ul>`:''}<button type="button" class="rb107-button ghost" data-rb107-open-why>Se coachens grunnlag</button></details>`:''}</section>`;
  }
  ...
  function bodyResponseDetailModalHtml(){
    ...
    return `<div class="rb1020-modal rb111-body-modal" data-rb1020-modal-backdrop="body-response"><section class="rb1020-sheet rb111-body-sheet" role="dialog" aria-modal="true" aria-labelledby="rb111BodyTitle"><header><div><span>Kroppens respons</span><h2 id="rb111BodyTitle">Hvor mye tåler kroppen i dag?</h2><p>Bakken-prinsippene styrer hvordan du trener. Responsbildet setter dagens dose.</p></div><button type="button" data-rb1020-modal-close="body-response" aria-label="Lukk helsebildet">${icon('close')}</button></header><div class="rb1020-sheet-body"><section class="rb111-body-lead ${esc(body.state)}"><span>${icon(body.state==='as_planned'?'check':body.state==='wait_for_data'?'sync':'heartbeat')}</span><div><small>${esc(body.stateLabel)} · ${esc(body.confidence==='high'?'godt datagrunnlag':body.confidence==='medium'?'delvis datagrunnlag':'begrenset datagrunnlag')}</small><h3>${esc(body.recommendedAction?.label||'Planen står')}</h3><p>${esc(body.summary)}</p></div></section><section class="rb111-metric-grid">${metricRows.map(([label,copy,metric])=>`<article><header><small>${esc(label)}</small><b>${esc(bodyMetricTrend(metric,'current'))}</b></header><p>${esc(metric?.current==null?'Siste måling mangler. Historikken vises nedenfor.':copy)}</p><dl><div><dt>Personlig normal</dt><dd>${esc(bodyMetricTrend(metric,'baseline'))}</dd></div><div><dt>7 dager</dt><dd>${esc(bodyMetricTrend(metric,'trend7'))}</dd></div><div><dt>28 dager</dt><dd>${esc(bodyMetricTrend(metric,'trend28'))}</dd></div></dl></article>`).join('')}</section><section class="rb111-domains"><header><div><small>Beslutningsgrunnlag</small><h3>Dette bygger vurderingen på</h3></div></header>${(body.domains||[]).map(row=>`<div><span>${icon(domainIcons[row.id]||'info')}</span><div><b>${esc(row.label)}</b><small>${esc(row.summary)}</small></div><em class="${esc(row.status)}"><i></i>${esc(statusLabels[row.status]||row.status)}</em></div>`).join('')}</section>${body.checkIn?.required?`<section class="rb111-checkin"><small>Kort kroppssjekk</small><h3>${esc(body.checkIn.prompt)}</h3><div>${(body.checkIn.options||[]).map(option=>`<button type="button" data-rb111-checkin="${esc(option.value)}">${esc(option.label)}</button>`).join('')}</div></section>`:''}<section class="rb111-data-note"><span>${icon('info')}</span><p><b>Personlig trend, ikke diagnose.</b> Median og robust variasjon fra inntil 28 dager brukes som din normal. Ett lavt HRV-signal endrer ikke planen alene. Sist synkronisert ${esc(updated)}.</p></section></div></section></div>`;
  }
  ...
  function resultCardHtml(p,context='today'){
    ...
    return `<article class="rb107-card rb109-result-card ${context==='plan'?'compact':''}"><header class="rb109-result-head"><div class="rb109-result-mark">${icon('check')}</div><div><span class="rb107-overline">Registrert gjennomføring</span><h2>${esc(prescription(p).title)}</h2><p>Økten er lagret i historikken din.</p></div></header><div class="rb109-result-metrics">${metrics.slice(0,3).map(m=>`<div><span>${esc(m[0])}</span><b>${esc(m[1])}</b></div>`).join('')}</div><section class="rb109-coach-verdict"><span>Fra økten</span><p>${esc(x.review)}</p></section>${coachFeedbackHtml(p,x)}<div class="rb109-result-details"><details><summary>Se øktanalysen ${icon('chevronRight')}</summary>${analysisDetailsHtml(p,a)}</details></div></article>`;
  }
  ...
  function todayHtml(){
    ...
    return `<div id="rb107Today" class="rb107-surface rb119b rb119b-today"><div class="rb107-shell">${appBarHtml()}${raceMetaHtml()}${viewTitleHtml(terminal?'Dagens resultat':'Dagens økt',terminal?planStatus.label:'')}
      <div class="rb121-today-primary">${primary}${!terminal&&pending?pendingResultHtml(base):''}</div>${contextualHealthStripHtml()}${support}
  ...
  function planHtml(){
    ...
    const normalPlan=`${canonicalSyncBannerHtml()||historyNoticeHtml()}${contextualPlanNoticeHtml()}<div class="rb119b-segments" role="group" aria-label="Planvisning"><button class="${state.planLens==='week'?'active':''}" data-rb119b-plan-lens="week" aria-pressed="${state.planLens==='week'}">Uke</button><button class="${state.planLens==='long'?'active':''}" data-rb119b-plan-lens="long" aria-pressed="${state.planLens==='long'}">Langsiktig</button></div>${state.planLens==='week'&&week===current?weeklyPriorityHtml():''}${planListHtml(week,state.planLens)}${state.planLens==='week'?planOverviewHtml(week):''}${weeklyReview()?`<button class="rb119b-completed-link" data-rb1031-week-review>Forrige ukes oppsummering ${icon('chevronRight')}</button>`:''}<button class="rb119b-completed-link" data-rb107-plan-view="done">Gjennomførte økter og coachanalyser ${icon('chevronRight')}</button>`;
  ...
  function goalHeroFidelityHtml(goal,g,pred){
    ...
    return `<section class="rb119b-goal-hero" ${heroStyle(hero)}>${heroImage(hero)}<div><small>${esc(title)}</small><h2>${esc(distance)}</h2></div></section><div class="rb125-goal-meta">${icon('calendar')}<b>${esc(date)}</b><span>${goal?`${goalDays(goal)} dager igjen`:`~${roundHalf(policy().anchorKm||50)} km normaluke`}</span></div>`;
  }
  ...
  function moreHtml(){
    ...
    return `<div id="rb107More" class="rb107-surface rb119b rb119b-more"><div class="rb107-shell">${appBarHtml()}${viewTitleHtml('Innsikt og innstillinger','Fordyp deg når du trenger det.')}<section class="rb119b-tools"><h2>Treningsinnsikt</h2>${insight('threshold','Terskelutvikling','Utviklingen i sammenlignbare økter',thresholdHistoryCardHtml)}${insight('intensity','Intensitet og soner','Din styring av fart, puls og følelse',intensityCardHtml)}${insight('distribution','Fordeling av trening','Rolig arbeid og kontrollert kvalitet',intensityDistributionCardHtml)}</section><section class="rb119b-tools"><h2>Innstillinger</h2>${setting('profile','Treningsprofil',`${prefs.runDays.length} løpedager · volum automatisk`,trainingPreferencesHtml())}${setting('link','Datakilder','Status og automatisk oppdatering',dataSourcesHtml(sync))}${setting('plan','Coachens handlingsrom',mode==='autopilot'&&safeAuto?'Trygg autopilot aktiv':mode==='observer'?'Kun observasjon':'Forslag med din godkjenning',coachControl)}${setting('log','Endringshistorikk','Planvalg, endringer og angre',logHtml())}${setting('info','Om RunnerBear',`Versjon ${BUILD} · ${RELEASE.label}`,`<b>RunnerBear ${BUILD}</b><p>Garmin registrerer. RunnerBear tolker og planlegger. Tredict frakter godkjente endringer videre.</p>`)}</section></div>${qs('.view.active')?.id==='more'&&state.intensityExplanationOpen?intensityExplanationModalHtml():''}${syncRepairModalHtml()}</div>`;
  }
  ...
  function bind(scope=document){
    ...
    qsa('[data-rb123-adapt]').forEach(b=>b.onclick=()=>{state.adaptOpen=true;state.workoutDetailDs=today();state.workoutDetailOpen=true;renderAll();requestAnimationFrame(()=>qs('.rb1020-workout-sheet [data-rb1020-modal-close]')?.focus())});
