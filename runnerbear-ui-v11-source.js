...
(function(){
  'use strict';
  const RELEASE=window.RunnerBearRelease||{build:'12.2.1',label:'One Truth · One Decision'},BUILD=RELEASE.build;
  ...
  function raceFocusModel(){return raceFocus()?.model({snapshot:window.RunnerBearCloudV11?.snapshot?.(),goal:activeGoal()})}
  ...
  function todayHtml(){
    ...
    const support=calm()?.chooseSupport({transport:canonicalSyncBannerHtml(),change:changeSupportHtml(base),race:raceFocusEntryHtml(true),review:weeklyReviewCardHtml(),priority:weeklyPriorityHtml()})||'';
  ...
  function planOverviewHtml(week){
    const rows=weekRows(week),s=weekStats(week),plannedRuns=rows.filter(p=>!p.activityOnly&&['easy','quality','race'].includes(prescription(p).type)&&kmFor(p)>0),completedRuns=plannedRuns.filter(p=>isDone(p)&&sportKind(activityFor(p),p)==='run'),completedKm=roundHalf(completedRuns.reduce((sum,p)=>sum+(Number(activityFor(p)?.distance||0)?Number(activityFor(p).distance)/1000:Number(kmFor(p)||0)),0)),remainingKm=roundHalf(plannedRuns.filter(p=>p.ds>=today()&&!isTerminal(p)).reduce((sum,p)=>sum+kmFor(p),0)),plannedQuality=plannedRuns.filter(p=>['quality','race'].includes(prescription(p).type)),completedQuality=plannedQuality.filter(isDone),plannedEasy=plannedRuns.filter(p=>prescription(p).type==='easy'),completedEasy=plannedEasy.filter(p=>isDone(p)&&sportKind(activityFor(p),p)==='run').length;
  ...
  function goalPremiumSummaryHtml(goal,f,pred){
    const context=contextualCoach()?.surfaces?.goal,next=goalGates().next;
    return `<section class="rb119b-card rb121-goal-direction">${goal?.targetSeconds?`<p class="rb121-goal-target"><span>Måltid</span><b>${esc(fmtTime(goal.targetSeconds))}</b></p>`:''}<span class="rb107-overline">Retning mot målet</span><h2>${esc(context?.visible?context.headline:f.progress.label)}</h2><p>${esc(context?.visible?context.summary:f.progress.copy)}</p>${next?`<button class="rb107-button secondary" data-rb116-open-gate="${esc(next.plan.ds)}">Neste sjekkpunkt · ${esc(formatDate(next.plan.ds,{day:'numeric',month:'short'}))} ${icon('chevronRight')}</button>`:'<small>Neste sjekkpunkt kommer når planen gir et relevant grunnlag.</small>'}</section>`;
  }
  ...
  function raceFocusModalHtml(view){
    ...
    return `<div class="rb1020-modal rb122-modal" data-rb1020-modal-backdrop="race-focus"><section class="rb1020-sheet rb122-sheet" role="dialog" aria-modal="true" aria-labelledby="rb122RaceTitle"><header><div><span>Race Focus · ${available?esc(model.name):'Løpsforberedelser'}</span><h2 id="rb122RaceTitle">${available?esc(model.headline):'Grunnlaget oppdateres'}</h2></div><button type="button" data-rb1020-modal-close="race-focus" aria-label="Lukk løpsforberedelsene">${icon('close')}</button></header><div class="rb1020-sheet-body">${available?`
      ...
      ${model.blocked?`<section class="rb122-safety" role="status"><b>Dagens avklaring kommer først</b><p>${esc(model.safetyCopy)}</p><button type="button" class="rb107-button secondary" data-rb122-today>Se dagens coachråd</button></section>`:''}
