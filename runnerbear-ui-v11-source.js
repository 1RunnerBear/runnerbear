/* RunnerBear v11.1.0 · Body Response within locked Design Direction 1.0
...
(function(){
  ...
  function trackEvent(name,detail={}){
    const payload={event:name,runnerbearVersion:'11.1.0',...detail};
  ...
  function runAutopilot(){
    ...
    window.RunnerBearCloudV11?.applySafeAuto?.().catch(error=>console.warn(JSON.stringify({event:'coach_loop_auto_failed',build:'11.1.0',message:error?.message||String(error)})));
  }
  ...
  function finishRender(id,startMark){
    ...
    document.body.classList.toggle('rb109-modal-open',state.goalManagerOpen||state.workoutDetailOpen||state.workoutBankOpen||!!state.movePreview||state.coachReasonOpen||state.bodyResponseOpen||state.intensityExplanationOpen||state.weeklyReviewOpen||state.syncRepairOpen);
    bind($(id));
  ...
  function bind(scope=document){
    ...
    qsa('[data-rb1020-modal-close]').forEach(b=>b.onclick=()=>{const kind=b.dataset.rb1020ModalClose;if(kind==='workout')state.workoutDetailOpen=false;if(kind==='bank')closeWorkoutBank();if(kind==='move')state.movePreview=null;if(kind==='coach')state.coachReasonOpen=false;if(kind==='body-response')state.bodyResponseOpen=false;if(kind==='intensity')state.intensityExplanationOpen=false;if(kind==='goal-insight')state.goalInsightOpen='';if(kind==='weekly-review')state.weeklyReviewOpen=false;if(kind==='sync-repair')state.syncRepairOpen=false;renderAll()});
    qsa('[data-rb1020-modal-backdrop]').forEach(el=>el.onclick=e=>{if(e.target!==el)return;const kind=el.dataset.rb1020ModalBackdrop;if(kind==='workout')state.workoutDetailOpen=false;if(kind==='bank')closeWorkoutBank();if(kind==='move')state.movePreview=null;if(kind==='coach')state.coachReasonOpen=false;if(kind==='body-response')state.bodyResponseOpen=false;if(kind==='intensity')state.intensityExplanationOpen=false;if(kind==='goal-insight')state.goalInsightOpen='';if(kind==='weekly-review')state.weeklyReviewOpen=false;if(kind==='sync-repair')state.syncRepairOpen=false;renderAll()});
  ...
  async function init(){
    ...
    if(!engine()||!clarityModel()||!planIntegrity()||!adaptivePlan()){
      console.error(JSON.stringify({event:'runnerbear_fatal_init',build:'11.1.0',reason:'canonical core unavailable'}));
    ...
    document.addEventListener('keydown',e=>{if(e.key!=='Escape')return;if(state.workoutBankOpen){closeWorkoutBank();renderAll();return}if(state.goalManagerOpen||state.goalInsightOpen||state.workoutDetailOpen||state.movePreview||state.coachReasonOpen||state.bodyResponseOpen||state.intensityExplanationOpen||state.weeklyReviewOpen||state.syncRepairOpen){state.goalManagerOpen=false;state.goalEditor='';state.goalInsightOpen='';state.workoutDetailOpen=false;state.movePreview=null;state.coachReasonOpen=false;state.bodyResponseOpen=false;state.intensityExplanationOpen=false;state.weeklyReviewOpen=false;state.syncRepairOpen=false;renderAll()}});
    let storageFrame=0;
