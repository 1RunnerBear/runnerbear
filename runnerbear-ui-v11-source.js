/* RunnerBear v11.6.0 · Contextual Coach within locked Design Direction 1.0
...
(function(){
  ...
  function trackEvent(name,detail={}){
    const payload={event:name,runnerbearVersion:'11.6.0',...detail};
  ...
  function runAutopilot(){
    ...
    window.RunnerBearCloudV11?.applySafeAuto?.().catch(error=>console.warn(JSON.stringify({event:'coach_loop_auto_failed',build:'11.5.0',message:error?.message||String(error)})));
  }
  ...
  async function init(){
    ...
    if(!engine()||!clarityModel()||!planIntegrity()||!adaptivePlan()){
      console.error(JSON.stringify({event:'runnerbear_fatal_init',build:'11.5.0',reason:'canonical core unavailable'}));
