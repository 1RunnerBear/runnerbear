/* RunnerBear v11.0.0 · Coach Loop within locked Design Direction 1.0
...
(function(){
  ...
  function trackEvent(name,detail={}){
    const payload={event:name,runnerbearVersion:'11.0.0',...detail};
  ...
  function runAutopilot(){
    ...
    window.RunnerBearCloudV11?.applySafeAuto?.().catch(error=>console.warn(JSON.stringify({event:'coach_loop_auto_failed',build:'11.0.0',message:error?.message||String(error)})));
  }
  ...
  function saveReadinessState(patch,ds=today()){
    const all=read(K.readiness,{}),current=readinessState(ds);all[ds]={...current,...patch,updatedAt:new Date().toISOString()};write(K.readiness,all);if(coachLoopUi()){const value=all[ds],previous=effectiveSchedule().filter(p=>p.ds<ds&&isDone(p)).sort((a,b)=>a.ds.localeCompare(b.ds)).at(-1),isNextMorning=previous&&dayDiff(ds,previous.ds)===1,payload={sourceId:`morning-response:${ds}:${previous?.workoutId||'general'}`,localDate:isNextMorning?previous.ds:ds,responseDate:ds,responsePhase:isNextMorning?'next_morning':'post_workout',workoutId:previous?.workoutId||'',planRevisionId:window.RunnerBearCloudV11?.snapshot?.()?.planRevisionId||'',illness:value.reasons.includes('illness'),stress:value.reasons.includes('stress'),poor_sleep:value.reasons.includes('poor_sleep'),pain:value.reasons.includes('achilles')?3:0};window.RunnerBearCloudV11?.submitFeedback?.(payload).catch(error=>console.warn(JSON.stringify({event:'coach_loop_feedback_failed',build:'11.0.0',message:error?.message||String(error)})))}return all[ds];
  }
  ...
  async function init(){
    ...
    if(!engine()||!clarityModel()||!planIntegrity()||!adaptivePlan()){
      console.error(JSON.stringify({event:'runnerbear_fatal_init',build:'11.0.0',reason:'canonical core unavailable'}));
