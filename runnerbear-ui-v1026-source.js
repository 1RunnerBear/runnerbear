/* RunnerBear v10.26.0 · Coach Loop within locked Design Direction 1.0
...
(function(){
  ...
  const K={
    ...
    planRevision:'runnerbear_v10251_plan_revision',planMutations:'runnerbear_v10251_plan_mutations'
  };
  let eventScope=null;
  ...
  function trackEvent(name,detail={}){
    const payload={event:name,runnerbearVersion:'10.26.0',...detail};
  ...
  function migrateTrainingPreferences(){
    ...
    if(stored.preferencesVersion!==2){stored.preferencesVersion=2;changed=true}
    if(changed)write(K.profile,stored);
    if(sessionStorage.getItem(K.planView)==='overview')sessionStorage.setItem(K.planView,'plan');
  }
  const coachLoopUi=()=>window.RunnerBearCloudV1026?.snapshot?.()?.flags?.coach_loop_ui===true;
  ...
  function changeNoticeHtml(p,context='today'){
    const canonicalPlan=window.RunnerBearCloudV1026?.snapshot?.()?.activePlan;if(coachLoopUi()&&canonicalPlan?.reasonCode==='safe-autopilot'&&/redusert dose/i.test(p?.title||''))return`<article class="rb107-card rb117-change ${context==='plan'?'compact':''}"><header><span class="rb117-change-mark">${icon('swap')}</span><div><span class="rb107-overline">Coachen har justert</span><h3>${esc(p.title)}</h3></div><strong>Endret</strong></header><details><summary>Hvorfor?</summary><p>En liten, reverserbar doseendring ble gjort innenfor volumtak, låser og Coach Loop-policyen.</p></details><button class="rb117-undo" data-rb1026-undo-plan>Angre endringen</button></article>`;
  ...
  function garminWorkout(p){
    const x=prescription(p);if(!x||!['easy','quality','race'].includes(x.type)||Number(x.km||0)<=0)return null;const target=targetFor(p),canonicalId=String(p.workoutId||'').replace(/[^a-z0-9._-]+/gi,'-').toLowerCase(),stable=canonicalId?`rb-workout-${canonicalId}`:(planIntegrity()?.stableExternalId?.(p)||`rb-workout-${p.baseDs}`),planRevisionId=String(p.planRevisionId||window.RunnerBearCloudV1026?.snapshot?.()?.planRevisionId||'');
  ...
  function queueTredictMutation(type,plans=[],detail={}){
    ...
    if(window.RunnerBearCloudV1026?.snapshot?.()?.flags?.coach_loop_write===true)return mutation;
  ...
  function applyCoachMove(preview){
    ...
    write(K.moves,moves);matchCache.signature='';const plans=changed.map(basePlan).filter(Boolean),previousDates=Object.fromEntries(previousRows.map(row=>[row.baseDs,row.ds]));queueTredictMutation('plan:workout-replanned',plans,{previousRows,previousDates,reason:'move-coach-preview'});addLog(`Coachen flyttet ${plans.length} økter for å beskytte ukerytmen.`,'manual',before);state.selectedDs=preview.target.ds;trackEvent('plan_move_coach_applied',{affected:plans.length});renderAll();toast('Planen er oppdatert · avstand og stimulus er kontrollert',{label:'Angre',run:()=>undoSwap(before,changed)});return true;
  }
  function confirmMovePreview(mode){
    const preview=state.movePreview;if(!preview)return;state.movePreview=null;
    if(mode==='coach')return applyCoachMove(preview);
    return swapWorkout(basePlan(preview.source.baseDs),preview.target.ds,`${preview.sourceKind||'preview'}-direct`);
  }
  ...
  function runAutopilot(){
    ...
    window.RunnerBearCloudV1026?.applySafeAuto?.().catch(error=>console.warn(JSON.stringify({event:'coach_loop_auto_failed',build:'10.26.0',message:error?.message||String(error)})));
  }
  ...
  const state={selectedDs:sessionStorage.getItem(K.selected)||'',planView:savedPlanView==='done'?'done':'plan',planLens:['week','focus','long'].includes(savedPlanLens)?savedPlanLens:'week',openWeek:null,monthKey:'',planDayViewOpen:false,planDetailOpen:false,moveOpen:false,movePreview:null,adaptOpen:false,completedId:'',doneScroll:0,goalManagerOpen:false,goalEditor:'',workoutDetailOpen:false,workoutDetailDs:'',workoutBankOpen:false,coachReasonOpen:false,intensityExplanationOpen:false};
  ...
  function todayHealthCardHtml(){
    const h=healthSignal(),r=h.r||{},sleepStatus=r.sleep?.value?(r.sleep.baseline&&r.sleep.value<r.sleep.baseline*.85?'Lavere enn normalt':'Bra'):'Venter på data',hrvStatus=r.hrv?.value?(r.hrv.baseline&&r.hrv.value<r.hrv.baseline*.85?'Litt under normalen':'Normal'):'Venter på data',rhrDelta=r.rhr?.value&&r.rhr?.baseline?Math.round(r.rhr.value-r.rhr.baseline):null,rhrStatus=rhrDelta==null?'Venter på data':`${rhrDelta>0?'+':''}${rhrDelta} bpm`,headline=h.tone==='green'?'Innenfor normalen':h.tone==='yellow'?'Ett signal følges':'Flere signaler avviker';
    return `<article class="rb119b-card rb10251-health ${h.tone}"><header><div><span class="rb107-overline">Helsedata</span><h2>${esc(headline)}</h2></div><i aria-hidden="true"></i></header><div><span><small>Søvn</small><b>${esc(sleepStatus)}</b><em>${esc(formatSleep(r.sleep?.value))}</em></span><span><small>HRV</small><b>${esc(hrvStatus)}</b><em>${r.hrv?.value?`${Math.round(r.hrv.value)} ms`:'–'}</em></span><span><small>Hvilepuls</small><b>${esc(rhrStatus)}</b><em>${r.rhr?.value?`${Math.round(r.rhr.value)} bpm`:'–'}</em></span></div><p>${esc(h.copy)}</p></article>`;
  }
  function trend30(){
  ...
  function saveReadinessState(patch,ds=today()){
    const all=read(K.readiness,{}),current=readinessState(ds);all[ds]={...current,...patch,updatedAt:new Date().toISOString()};write(K.readiness,all);if(coachLoopUi()){const value=all[ds],previous=effectiveSchedule().filter(p=>p.ds<ds&&isDone(p)).sort((a,b)=>a.ds.localeCompare(b.ds)).at(-1),isNextMorning=previous&&dayDiff(ds,previous.ds)===1,payload={sourceId:`morning-response:${ds}:${previous?.workoutId||'general'}`,localDate:isNextMorning?previous.ds:ds,responseDate:ds,responsePhase:isNextMorning?'next_morning':'post_workout',workoutId:previous?.workoutId||'',planRevisionId:window.RunnerBearCloudV1026?.snapshot?.()?.planRevisionId||'',illness:value.reasons.includes('illness'),stress:value.reasons.includes('stress'),poor_sleep:value.reasons.includes('poor_sleep'),pain:value.reasons.includes('achilles')?3:0};window.RunnerBearCloudV1026?.submitFeedback?.(payload).catch(error=>console.warn(JSON.stringify({event:'coach_loop_feedback_failed',build:'10.26.0',message:error?.message||String(error)})))}return all[ds];
  }
  ...
  function readinessDecision(base=planFor(today())){
    const canonical=window.RunnerBearCloudV1026?.toLegacyDecision?.(base);if(canonical)return canonical;
  ...
  function workoutDetailModalHtml(p){
    ...
    return `<div class="rb1020-modal" data-rb1020-modal-backdrop="workout"><section class="rb1020-sheet rb1020-workout-sheet" role="dialog" aria-modal="true" aria-labelledby="rb1020WorkoutTitle"><header><div><span>${esc(status)} · ${esc(formatDate(p.ds,{weekday:'long',day:'numeric',month:'long'}))}</span><h2 id="rb1020WorkoutTitle">${esc(x.title)}</h2><p>${esc(typeLabel(x.type))}</p></div><button type="button" data-rb1020-modal-close="workout" aria-label="Lukk øktdetaljer">${icon('close')}</button></header><div class="rb1020-sheet-body">${a?`<section class="rb1020-detail-section"><span>${planStatus.code==='replaced'?'Faktisk aktivitet · erstattet':'Gjennomført'}</span><div class="rb1020-result-grid">${metrics.map(row=>`<div><small>${esc(row[0])}</small><b>${esc(row[1])}</b></div>`).join('')}</div></section>`:''}<section class="rb1020-detail-section"><span>Øktstruktur</span>${simpleStructureHtml(p)}</section>${workoutIntensityHtml(p)}<section class="rb1020-coach-key"><span>Coachens nøkkelråd</span><p>${esc(planStatus.code==='replaced'?replacementFor(p)?.classification?.coach?.consequence||'Planen videre står. Ingen treningsgjeld.':keyAdvice(p))}</p></section>${x.shoe||x.fuel?`<section class="rb1020-detail-section rb1020-practical"><span>Praktisk</span>${x.shoe?`<p><b>Sko</b>${esc(x.shoe)}</p>`:''}${x.fuel?`<p><b>Energi</b>${esc(x.fuel)}</p>`:''}</section>`:''}${flexHtml(p)}${!canOneOff(p)&&!flexible(p)?matchPickerHtml(p):''}${bodyCheck}${bankAction}${!done?garminStatusHtml(p):''}</div></section></div>`;
  }
  function workoutBankModalHtml(p){
    if(!p)return'';const intended=adaptivePlan()?.stimulusForWorkout?.(prescription(p))||'threshold',nextQuality=effectiveSchedule().find(row=>row.ds>p.ds&&['quality','race'].includes(prescription(row).type)),long=effectiveSchedule().find(row=>row.ds>=p.ds&&/langtur/i.test(row.title||'')),weekState=adaptiveWeekState(),goal=activeGoal(),ranked=adaptivePlan()?.rankWorkoutBank?.({plan:prescription(p),intendedStimulus:intended,weekMode:weekState.mode,healthTrend:healthSignal().tone==='red'?'negative':'normal',daysToNextQuality:nextQuality?dayDiff(nextQuality.ds,p.ds):99,daysToLongRun:long?dayDiff(long.ds,p.ds):99,phase:goal&&goalDays(goal)<35?'race':'build'})||[],recommended=ranked.filter(row=>row.stimulus===intended).slice(0,3),threshold=ranked.filter(row=>row.stimulus==='threshold'&&!recommended.includes(row)),xRows=ranked.filter(row=>row.stimulus==='x'),rowHtml=(row,label='')=>`<button data-rb10251-bank-select="${esc(row.id)}" data-base-ds="${esc(p.baseDs)}"><span><small>${esc(label||(row.stimulus==='x'?'X-element':'Terskel'))}</small><b>${esc(row.title)}</b><em>${esc(row.desc)}</em></span><strong>${row.suitabilityScore>=88?'Passer svært godt':row.suitabilityScore>=70?'Passer godt':'Vurderes med margin'}</strong>${icon('chevronRight')}</button>`;
    return `<div class="rb1020-modal rb10251-bank-modal" data-rb1020-modal-backdrop="bank"><section class="rb1020-sheet rb10251-bank" role="dialog" aria-modal="true" aria-labelledby="rb10251BankTitle"><header><div><span>RunnerBear Quality Bank</span><h2 id="rb10251BankTitle">Bytt kvalitetsøkt</h2><p>Coachen rangerer øktene uten å blande terskel og X-stimulus.</p></div><button type="button" data-rb1020-modal-close="bank" aria-label="Lukk treningsbanken">${icon('close')}</button></header><div class="rb1020-sheet-body"><section><h3>Anbefalt av coach</h3>${recommended.map(row=>rowHtml(row,'Anbefalt')).join('')}</section>${threshold.length?`<section><h3>Andre terskeløkter</h3>${threshold.map(row=>rowHtml(row,'Terskel')).join('')}</section>`:''}<section class="rb10251-x-bank"><h3>X-økter · eget stimulus</h3><p>Disse koster mer og velges bare når fase, helse og resten av uken støtter det.</p>${xRows.map(row=>rowHtml(row,'X-element')).join('')}</section></div></section></div>`;
  }
  function replaceQualityWorkout(p,workoutId){
  ...
  function movePreviewModalHtml(){
    ...
    return `<div class="rb1020-modal rb10251-move-modal" data-rb1020-modal-backdrop="move"><section class="rb1020-sheet rb10251-move-preview" role="dialog" aria-modal="true" aria-labelledby="rb10251MoveTitle"><header><div><span>Coachens flyttevurdering</span><h2 id="rb10251MoveTitle">Flytte ${esc(source.title)} til ${esc(formatDate(target.ds,{weekday:'long'}))}?</h2></div><button type="button" data-rb1020-modal-close="move" aria-label="Avbryt flytting">${icon('close')}</button></header><div class="rb1020-sheet-body"><div class="rb10251-move-route"><span><small>Fra</small><b>${esc(formatDate(source.ds,{weekday:'long',day:'numeric',month:'short'}))}</b></span>${icon('arrowRight')}<span><small>Til</small><b>${esc(formatDate(target.ds,{weekday:'long',day:'numeric',month:'short'}))}</b></span></div><p>${esc(warning)}</p>${extra?`<div class="rb10251-coach-proposal"><b>Coach foreslår å flytte ${extra} ekstra ${extra===1?'økt':'økter'}</b><span>Dette beskytter restitusjon og beholder treningsstimulus.</span></div>`:''}<div class="rb10251-move-actions"><button class="rb107-button" data-rb10251-move-confirm="coach">Bruk coachens forslag</button><button class="rb107-button secondary" data-rb10251-move-confirm="direct">Flytt kun denne</button><button class="rb107-button ghost" data-rb10251-move-cancel>Avbryt</button></div></div></section></div>`;
  }
  function coachReasonModalHtml(){
    const coach=coachBasis(),base=planFor(today()),positives=coach.positives.length?coach.positives:['Belastningsbildet gir ikke grunnlag for en offensiv endring.'],nextLong=effectiveSchedule().find(p=>p.ds>=today()&&/langtur/i.test(p.title||''));if(nextLong)positives.push(`Neste langtur er ${formatDate(nextLong.ds,{weekday:'long'})}.`);
    return `<div class="rb1020-modal" data-rb1020-modal-backdrop="coach"><section class="rb1020-sheet rb1020-reason-sheet" role="dialog" aria-modal="true" aria-labelledby="rb1020ReasonTitle"><header><div><span>Hvorfor denne økten?</span><h2 id="rb1020ReasonTitle">Derfor valgte coachen denne økten</h2></div><button type="button" data-rb1020-modal-close="coach" aria-label="Lukk coachvurderingen">${icon('close')}</button></header><div class="rb1020-sheet-body"><p class="rb1020-reason-lead">${esc(coach.message)}</p><section><h3>Det som taler for økten</h3><ul>${[...new Set(positives)].slice(0,5).map(x=>`<li>${esc(x)}</li>`).join('')}</ul></section><section><h3>Det vi følger med på</h3><ul>${coach.watch.map(x=>`<li>${esc(x)}</li>`).join('')}</ul></section><p class="rb10251-purpose"><b>Dagens mål</b>${esc(purposeFor(base))}</p><footer><span>${esc(coach.confidence)}</span><b>${esc(coach.sources.length?coach.sources.join(' · '):'plan og tilgjengelig treningshistorikk')}</b></footer></div></section></div>`;
  }
  function todayHtml(){
    ...
    return `<div id="rb107Today" class="rb107-surface rb119b rb119b-today"><div class="rb107-shell">${appBarHtml()}${raceMetaHtml()}${viewTitleHtml(terminal?'Dagens resultat':'Dagens økt',terminal?planStatus.label:'')}
      ${workoutHeroHtml(base)}${todayHealthCardHtml()}${todayCoachHtml(base,d,suggest)}${changeNoticeHtml(base)}${terminal?result:`${pending?pendingResultHtml(base):''}`}
      <div class="rb119b-sync ${sync.stale?'stale':''}"><i></i><span>${esc(sync.label)}</span></div>
    </div>${qs('.view.active')?.id==='today'&&state.workoutDetailOpen?workoutDetailModalHtml(planFor(state.workoutDetailDs)||base):''}${qs('.view.active')?.id==='today'&&state.workoutBankOpen?workoutBankModalHtml(planFor(state.workoutDetailDs)||base):''}${qs('.view.active')?.id==='today'&&state.coachReasonOpen?coachReasonModalHtml():''}</div>`;
  }
  ...
  function planOverviewHtml(week){
    const rows=weekRows(week),s=weekStats(week),plannedRuns=rows.filter(p=>['easy','quality','race'].includes(prescription(p).type)&&kmFor(p)>0),completedRuns=plannedRuns.filter(p=>isDone(p)&&sportKind(activityFor(p),p)==='run'),completedKm=roundHalf(completedRuns.reduce((sum,p)=>sum+(Number(activityFor(p)?.distance||0)?Number(activityFor(p).distance)/1000:Number(kmFor(p)||0)),0)),plannedQuality=plannedRuns.filter(p=>['quality','race'].includes(prescription(p).type)),completedQuality=plannedQuality.filter(isDone),quietDays=completedRuns.filter(p=>prescription(p).type==='easy').length;
    ...
    return `<div class="rb119b-plan-overview rb1025-plan-overview"><article class="rb119b-card rb1025-week-so-far"><header><div><h2>Uken så langt</h2><small>Faktisk gjennomført løping</small></div><strong>${completedKm} <span>/ ${s.km} km</span></strong></header><div><p><b>${completedRuns.length}</b> av ${plannedRuns.length} løpeøkter</p><p><b>${completedQuality.length}</b> av ${plannedQuality.length} kvalitetsøkter</p><p><b>${quietDays}</b> rolige dager</p></div></article><article class="rb119b-card rb1025-plan-phase"><header><h2>Planfase</h2><small>${esc(goal?.name||'Formbygging')}</small></header><strong>${esc(phaseTitle)}</strong><p>${goal?`${Math.max(0,Math.ceil(goalDays(goal)/7))} uker til hovedmålet`:'Retningen styres av kontinuitet og respons'}</p><footer><span>Neste nøkkel</span><b>${nextKey?`${esc(prescription(nextKey).title)} · ${esc(formatDate(nextKey.ds,{weekday:'long'}))}`:'Ingen ny nøkkeløkt i perioden'}</b></footer></article></div>`;
  }
  ...
  function planHtml(){
    ...
    </div>${qs('.view.active')?.id==='plan'&&state.workoutDetailOpen?workoutDetailModalHtml(planFor(state.workoutDetailDs)||selected):''}${qs('.view.active')?.id==='plan'&&state.workoutBankOpen?workoutBankModalHtml(planFor(state.workoutDetailDs)||selected):''}${qs('.view.active')?.id==='plan'&&state.movePreview?movePreviewModalHtml():''}</div>`;
  }
  ...
  function goalPremiumSummaryHtml(goal,f,pred){
    ...
    const items=[
      {icon:'flag',label:goal?distanceMeta(goal.distance).label:'Formmål',value:target?fmtTime(target):'Uten tid',copy:'mål'},
      {icon:'trend',label:'5 km-form',value:five?fmtTime(five):'Bygges',copy:five?'kapasitetsestimat':'trenger mer data'},
      {icon:'continuity',label:'Retning',value:direction,copy:f.progress.label},
      {icon:'calendar',label:'Neste gate',value:next?String(next.title).replace(/^gate\s*\d+\s*·?\s*/i,''):'Kontinuitet',copy:next?formatDate(next.plan.ds,{day:'numeric',month:'long'}):'neste beslutningspunkt'}
    ];
    return `<section class="rb119b-card rb10251-goal-summary"><header><span class="rb107-overline">Mot målet</span><h2>Det viktigste akkurat nå</h2></header><div>${items.map(item=>`<article><span>${icon(item.icon)}</span><small>${esc(item.label)}</small><b>${esc(item.value)}</b><em>${esc(item.copy)}</em></article>`).join('')}</div><p>${esc(f.progress.copy)}</p></section>`;
  }
  ...
  function saveTrainingPreferences(form){
    ...
    const previousRows=planRowsSnapshot(),before=snapshot(K.profile),stored=read(K.profile,{}),nextPreferences={baseKm,normalLow,normalHigh,maxKm,minRunDays:runDays.length,flexibleSessions:Math.min(2,alternativeDays.length),runDays,qualityDays,longRunDay,alternativeDays,preferencesConfigured:true,preferencesVersion:2};Object.assign(stored,nextPreferences);write(K.profile,stored);
    const legacy=read(K.legacyProfile,{});legacy.weekRhythm={...(legacy.weekRhythm||{}),runDays,qualityDays,longRunDay,crossDays:alternativeDays,allowCross:alternativeDays.length>0};write(K.legacyProfile,legacy);
    const result=adaptivePlan()?.replanFuture?.(previousRows,nextPreferences,{today:today()})||{rows:previousRows,changes:[]},moves=read(K.moves,{});
    for(const row of previousRows)if(row.ds>=today()&&!row.terminal)delete moves[row.baseDs];
    for(const row of result.rows||[])if(row.ds>=today()&&!row.terminal&&row.ds!==row.baseDs)moves[row.baseDs]=row.ds;
    write(K.moves,moves);matchCache.signature='';const changedIds=(result.changes||[]).map(x=>x.workoutId),changed=changedIds.map(basePlan).filter(Boolean),previousDates=Object.fromEntries(previousRows.map(row=>[row.baseDs,row.ds]));
    if(changed.length)queueTredictMutation('plan:preferences-replanned',changed,{previousRows,previousDates,reason:'training-preferences'});
    addLog(`Treningspreferansene er oppdatert. ${changed.length} kommende ${changed.length===1?'økt ble':'økter ble'} plassert på nytt.`,'manual',before);toast(changed.length?`Planen er oppdatert fra i dag · ${changed.length} økter flyttet`:'Preferansene er lagret · planen står');renderAll();
  }
  ...
  function finishRender(id,startMark){
    ...
    document.body.classList.toggle('rb109-modal-open',state.goalManagerOpen||state.workoutDetailOpen||state.workoutBankOpen||!!state.movePreview||state.coachReasonOpen||state.intensityExplanationOpen);
    bind($(id));
  ...
  function acceptReadinessRecommendation(){
    ...
    if(coach.canonical){window.RunnerBearCloudV1026?.resolveDecision?.('accept').then(()=>{toast('Coachens anbefaling er lagret i planen');renderAll()}).catch(error=>toast(error?.message||'Anbefalingen kunne ikke lagres'));return}
  ...
  function bind(scope=document){
    ...
    qsa('[data-rb10251-bank-open]').forEach(b=>b.onclick=()=>{const p=basePlan(b.dataset.rb10251BankOpen);if(!p)return;state.workoutDetailDs=p.ds;state.workoutBankOpen=true;trackEvent('quality_bank_opened',{date:p.ds});renderAll();requestAnimationFrame(()=>qs('.rb10251-bank [data-rb1020-modal-close]')?.focus())});
    ...
    qsa('[data-rb1020-modal-close]').forEach(b=>b.onclick=()=>{const kind=b.dataset.rb1020ModalClose;if(kind==='workout')state.workoutDetailOpen=false;if(kind==='bank')state.workoutBankOpen=false;if(kind==='move')state.movePreview=null;if(kind==='coach')state.coachReasonOpen=false;if(kind==='intensity')state.intensityExplanationOpen=false;renderAll()});
    qsa('[data-rb1020-modal-backdrop]').forEach(el=>el.onclick=e=>{if(e.target!==el)return;const kind=el.dataset.rb1020ModalBackdrop;if(kind==='workout')state.workoutDetailOpen=false;if(kind==='bank')state.workoutBankOpen=false;if(kind==='move')state.movePreview=null;if(kind==='coach')state.coachReasonOpen=false;if(kind==='intensity')state.intensityExplanationOpen=false;renderAll()});
    ...
    qsa('[data-rb1026-undo-plan]').forEach(b=>b.onclick=async()=>{b.disabled=true;try{await window.RunnerBearCloudV1026?.undoPlan?.();toast('Coachendringen er angret');renderAll()}catch(error){b.disabled=false;toast(error?.message||'Endringen kunne ikke angres')}});
  ...
  function init(){
    ...
    if(!engine()||!clarityModel()||!planIntegrity()||!adaptivePlan()){
      console.error(JSON.stringify({event:'runnerbear_fatal_init',build:'10.26.0',reason:'canonical core unavailable'}));
    ...
    document.addEventListener('keydown',e=>{if(e.key!=='Escape')return;if(state.goalManagerOpen||state.workoutDetailOpen||state.workoutBankOpen||state.movePreview||state.coachReasonOpen||state.intensityExplanationOpen){state.goalManagerOpen=false;state.goalEditor='';state.workoutDetailOpen=false;state.workoutBankOpen=false;state.movePreview=null;state.coachReasonOpen=false;state.intensityExplanationOpen=false;renderAll()}});
    let storageFrame=0;
