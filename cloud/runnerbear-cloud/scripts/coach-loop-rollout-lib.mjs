...
export function observationStateSql({userId='primary',coreActivatedAt=''}={}){
  ...
  return `WITH active AS (SELECT plan_revision_id FROM rb_plan_revisions WHERE user_id=${user} AND status='active')
    SELECT
      ...
      (SELECT COUNT(*) FROM rb_sync_operations o JOIN active a ON a.plan_revision_id=o.plan_revision_id LEFT JOIN rb_plan_revision_items i ON i.plan_revision_id=o.plan_revision_id AND i.workout_id=o.workout_id WHERE o.user_id=${user} AND o.status='failed_terminal' AND (i.local_date IS NULL OR i.local_date>=date('now'))) AS terminal_sync_error_count,
      (SELECT COUNT(*) FROM rb_sync_operations o JOIN active a ON a.plan_revision_id=o.plan_revision_id LEFT JOIN rb_plan_revision_items i ON i.plan_revision_id=o.plan_revision_id AND i.workout_id=o.workout_id WHERE o.user_id=${user} AND o.status='failed_retryable' AND (i.local_date IS NULL OR i.local_date>=date('now'))) AS retryable_sync_error_count,
