...
export function buildCoachBrief({plan={},decision=null,today='',now=new Date().toISOString()}={}){
  ...
  return{version:'coach-brief-1',planRevisionId,inputCursor:current?String(decision.inputCursor||''):'',generatedAt:now,validUntil:current?validUntil:null,freshness:current?'current':decision?'stale':'unavailable',attention,today:{workoutId:todayWorkout?.workoutId||'',title:current?(decision.explanation?.title||todayWorkout?.title||'Planen står'):'Vurderingen må fornyes',summary:current?(decision.explanation?.summary||'Dagens plan står.'):'RunnerBear viser ikke en eldre coachbeslutning som om den var gjeldende.',actionKind:current?decision.type:'wait_for_data',actionLabel:actionLabels[current?decision.type:'wait_for_data'],planChanged:Boolean(changed),affectedWorkoutIds:current?(decision.action?.affectedWorkoutIds||[]):[]},week:{priority,reason,nextChange,nextKeyWorkout:nextKey?{workoutId:nextKey.workoutId,localDate:nextKey.localDate,title:nextKey.title,workoutType:nextKey.workoutType}:null,keyWorkoutIds:weekKeys.map(row=>row.workoutId),qualityCount,longRunWorkoutId:longRun?.workoutId||'',watch:reasonCodes.map(code=>reasonLabels[code]).filter(Boolean).slice(0,3)}};
}
