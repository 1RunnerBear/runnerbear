...
export async function bodyResponseFor(db,userId,{plan={},today='',now=new Date().toISOString(),persist=true}={}){
  ...
  const [health,sync,checkIn,feedback,activity]=await Promise.all([
    ...
    db.prepare("SELECT source_id,occurred_at,payload_json FROM rb_training_events WHERE user_id=?1 AND event_type='feedback:workout' AND (json_type(payload_json,'$.activityId') IS NULL OR json_type(payload_json,'$.pain') IS NOT NULL OR json_type(payload_json,'$.pain_increased') IS NOT NULL OR json_type(payload_json,'$.illness') IS NOT NULL OR json_type(payload_json,'$.stress') IS NOT NULL OR json_type(payload_json,'$.poor_sleep') IS NOT NULL) ORDER BY occurred_at DESC LIMIT 1").bind(userId).first(),
