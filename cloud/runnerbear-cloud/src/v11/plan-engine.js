...
import { programQualityWeek } from './bakken-engine.js';
...
function legacyFor(type, index = 0) {
  if (type === 'quality') return { desc: 'Kontrollert Bakken-kvalitet.', detail: 'Stimulus, øktfamilie og dose velges av v11-motoren før planen aktiveres.', shoe: '', fuel: '' };
...
function template(type, date, km, index = 0, week = '') {
  ...
  return {
    ...
    title: quality ? 'Kvalitetsøkt · Bakken-motor' : long ? `${km} km rolig langtur` : `${km} km rolig`,
    intent: quality ? 'threshold' : long ? 'long' : 'easy',
    plannedDistanceM: km * 1000, lockLevel: 'none', source: 'runnerbear-v11.0',
    prescription: {
      version: 1,
      main: quality ? { kind: 'intervals', repetitions: 6, workSeconds: 360, recoverySeconds: 120 }
...
function reflowWeek(items, cfg, week, fromDate, trigger, priorHardDates = [], contextItems = []) {
  ...
  const used = new Set(); let future = []; const allDates = Array.from({ length: 7 }, (_, index) => addDays(week, index)).filter(date => date >= fromDate && !occupied.has(date));
  let qualityIndex = 0;
  ...
  const fixedFuture=fixed.filter(row=>row.localDate>=fromDate),programmed=programQualityWeek({ rows: [...fixedFuture,...future], allItems: [...contextItems, ...source], config: cfg, week, expectedQuality: targetMeta.expectedQualitySessions, responseMode: 'NORMAL' }),programmedById=new Map(programmed.map(row=>[row.workoutId,row])),resolvedFixed=fixed.map(row=>row.localDate>=fromDate?programmedById.get(row.workoutId)||row:row);
  future = future.map(row=>programmedById.get(row.workoutId)||row);
  ...
  for (const row of [...resolvedFixed.filter(row => row.localDate >= fromDate), ...future]) { row.plannedLoad = { ...(row.plannedLoad || {}), integrity }; if (trigger === 'training_preferences_changed') delete row.plannedLoad.manualMove; }
  return [...resolvedFixed, ...future].sort((a, b) => a.localDate.localeCompare(b.localDate) || a.slotIndex - b.slotIndex);
}
