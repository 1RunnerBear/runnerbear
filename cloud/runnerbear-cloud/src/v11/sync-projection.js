...
export function canonicalWorkoutProjection(item={},planRevisionId='',canonicalPlanId=''){
  const externalId=stableExternalId(item.workoutId),stimulus=String(item.intent||item.workoutType||'easy'),transportState={canonicalWorkoutId:externalId,date:item.localDate,title:item.title,stimulus,prescription:item.prescription||{},plannedDurationSeconds:item.plannedDurationSeconds??null,plannedDistanceM:Number(item.plannedDistanceM||0)},content={...transportState,plannedLoad:item.plannedLoad||{},canonicalPlanId,planRevisionId},fingerprint=deterministicFingerprint(transportState),notes=`[RB:${externalId}] [PLAN:${canonicalPlanId}] [REV:${planRevisionId}] [STIMULUS:${stimulus}] [FPR:${fingerprint}]`;
