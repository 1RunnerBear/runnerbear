...
export function canonicalWorkoutProjection(item={},planRevisionId='',canonicalPlanId=''){
  const externalId=stableExternalId(item.workoutId),stimulus=String(item.intent||item.workoutType||'easy'),desiredState={canonicalWorkoutId:externalId,date:item.localDate,title:item.title,stimulus,prescription:item.prescription||{},plannedDurationSeconds:item.plannedDurationSeconds??null,plannedDistanceM:Number(item.plannedDistanceM||0),plannedLoad:item.plannedLoad||{}},content={...desiredState,canonicalPlanId,planRevisionId},fingerprint=deterministicFingerprint(desiredState),notes=`[RB:${externalId}] [PLAN:${canonicalPlanId}] [REV:${planRevisionId}] [STIMULUS:${stimulus}] [FPR:${fingerprint}]`;
