...
export function buildWeeklyReview({plan={},activities=[],events=[],today=new Date().toISOString().slice(0,10),generatedAt=new Date().toISOString()}={}){
  ...
  for(const item of items){
    ...
    results.push({workoutId:item.workoutId,localDate:item.localDate,title:item.title,workoutType:item.workoutType,state,plannedDistanceM:finite(item.plannedDistanceM),actualDistanceM:isRun?activityDistance(activity):0,activityId:sourceId(activity),activitySport:sport||'',keyWorkout:quality(item)||long(item)});
  }
  const plannedDistanceM=plannedRuns.reduce((sum,row)=>sum+finite(row.plannedDistanceM),0),completedDistanceM=results.reduce((sum,row)=>sum+finite(row.actualDistanceM),0),completed=results.filter(row=>row.state==='completed').length,replaced=results.filter(row=>row.state==='replaced').length,missed=results.filter(row=>row.state==='missed').length,keyCompleted=results.filter(row=>row.keyWorkout&&['completed','replaced'].includes(row.state)).length,keyPlanned=results.filter(row=>row.keyWorkout).length,qualityCompleted=results.filter(row=>row.workoutType==='quality'&&row.state==='completed').length,alternatives=results.filter(row=>row.state==='replaced').length;
  let attention='normal',headline='Kontinuiteten er bevart',learning='Uken støtter videre kontrollert progresjon.',nextDirection='Neste uke følger planlagt rytme uten å jage ekstra kilometer.';
  if(!items.length){attention='watch';headline='Ukereviewen bygges';learning='RunnerBear mangler en komplett planreferanse for forrige uke.';nextDirection='Planen står mens datagrunnlaget kompletteres.'}
  else if(missed>0){attention='watch';headline=keyPlanned>keyCompleted?'En nøkkeløkt falt bort':'Uken ble tilpasset';learning='Tapte økter blir ikke treningsgjeld. Coachen beskytter neste relevante stimulus.';nextDirection='Fremtidige økter vurderes fra i dag; forrige uke endres ikke.'}
  else if(replaced>0){headline='Uken ble løst fleksibelt';learning='Alternativ trening teller som aerob støtte, men ikke som løpskilometer.';nextDirection='Neste nøkkeløkt beskyttes ut fra faktisk belastning.'}
  ...
  return{version:'weekly-review-1',planRevisionId:String(plan.planRevisionId||''),weekStart,weekEnd,generatedAt,attention,dataQuality,headline,learning,nextDirection,totals:{plannedDistanceM,completedDistanceM,plannedSessions:items.filter(row=>row.workoutType!=='rest').length,completedSessions:completed,replacedSessions:replaced,missedSessions:missed,plannedKeySessions:keyPlanned,completedKeySessions:keyCompleted,completedQualitySessions:qualityCompleted,alternativeSessions:alternatives},sessions:results,evidence:{activityIds:results.map(row=>row.activityId).filter(Boolean),feedbackEventIds:events.filter(row=>parseDate(row.local_date||row.localDate)>=weekStart&&parseDate(row.local_date||row.localDate)<=weekEnd).map(sourceId).filter(Boolean)}};
}
...
export function buildRealignmentProposal({plan={},activities=[],events=[],today=new Date().toISOString().slice(0,10),generatedAt=new Date().toISOString()}={}){
  ...
  let trigger='',evidenceId='',summary='',consequence='',rows=items,requiresInput=false;
  if(illness){
  ...
  return{version:'realignment-proposal-1',proposalId:`realign:${String(plan.planRevisionId||'none')}:${trigger}:${String(evidenceId||today).replace(/[^a-z0-9._:-]+/gi,'-')}`,planRevisionId:String(plan.planRevisionId||''),inputCursor:`${String(plan.planRevisionId||'')}:${trigger}:${evidenceId||today}`,generatedAt,validUntil:new Date(Date.parse(generatedAt)+6*3600000).toISOString(),trigger,status:requiresInput?'needs_input':changes.length?'proposed':'unchanged',summary,consequence,trainingDebt:false,autoEligible,protectedWorkoutIds,affectedWorkoutIds:changes.map(row=>row.workoutId),changes,rows};
}
